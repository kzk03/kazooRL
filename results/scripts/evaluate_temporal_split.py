...
def main():
    ...
    parser.add_argument("--test-data", required=True, help="2023年テストデータのパス")
    parser.add_argument(
        "--model-path",
        default="models/unified_rl/best_model.zip",
        help="訓練済みモデルのパス",
    )
    parser.add_argument(
        "--config-path", default="configs/unified_rl.yaml", help="設定ファイルのパス"
    )
    parser.add_argument(
        "--output-dir",
        default="outputs/temporal_evaluation",
        help="評価結果の出力ディレクトリ",
    )
