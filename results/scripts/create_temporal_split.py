...
def main():
    ...
    parser.add_argument('--input', default='data/backlog.json', 
                       help='入力データファイル')
    parser.add_argument('--irl-start', type=int, default=2019, 
                       help='IRL学習開始年')
    parser.add_argument('--irl-end', type=int, default=2021, 
                       help='IRL学習終了年')
    parser.add_argument('--rl-start', type=int, default=2022, 
                       help='RL訓練開始年')
    parser.add_argument('--rl-end', type=int, default=2022, 
                       help='RL訓練終了年')
    parser.add_argument('--test-start', type=int, default=2023, 
                       help='テスト開始年')
    parser.add_argument('--test-end', type=int, default=2023, 
                       help='テスト終了年')
    ...
    print(f"\n📊 データ分割サマリー:")
    print(f"   IRL用:    {len(irl_data):,} タスク ({len(irl_data)/total_split*100:.1f}%)")
    print(f"   RL訓練用: {len(rl_data):,} タスク ({len(rl_data)/total_split*100:.1f}%)")
    print(f"   テスト用: {len(test_data):,} タスク ({len(test_data)/total_split*100:.1f}%)")
