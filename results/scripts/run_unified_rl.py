...
def main():
    ...
    parser.add_argument("--timesteps", type=int, default=None, help="訓練ステップ数")
    parser.add_argument(
        "--eval-only", action="store_true", help="評価のみ実行（訓練はスキップ）"
    )
    parser.add_argument(
        "--quick",
        action="store_true",
    ...
    cmd = [sys.executable, str(script_path), f"--config-name={args.config}"]
