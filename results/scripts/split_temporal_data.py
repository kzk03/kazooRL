...
def create_split_datasets(year_data: Dict[str, List[Dict]], output_dir: str):
    ...
    return {"irl": irl_path, "training": rl_path, "test": test_path}
...
def update_config_file(new_paths: Dict[str, str], config_path: str):
    ...
    ] = "data/expert_trajectories_2019_2021.pkl"  # 必要に応じて更新
...
def create_migration_report(
    year_data: Dict[str, List[Dict]], new_paths: Dict[str, str], output_dir: str
):
    """移行レポートを作成"""
...
@click.option(
    "--input-file",
    default="data/backlog_with_2023.json",
    help="2023年データを含む入力ファイル",
)
@click.option(
    "--output-dir", default="data/temporal_split", help="分割データの出力ディレクトリ"
)
@click.option(
    "--backup-dir",
    default="backups/temporal_split",
    help="既存設定のバックアップディレクトリ",
)
@click.option(
    "--config-path",
    default="configs/unified_rl.yaml",
    help="更新する設定ファイルのパス",
)
@click.option("--dry-run", is_flag=True, help="実際の変更を行わずに確認のみ")
def main(
    input_file: str, output_dir: str, backup_dir: str, config_path: str, dry_run: bool
):
    """
    時系列データ分割スクリプト
    ...
    try:
        ...
        if dry_run:
            print("📊 分割結果プレビュー:")
            irl_count = (
                len(year_data["2019"]) + len(year_data["2020"]) + len(year_data["2021"])
            )
