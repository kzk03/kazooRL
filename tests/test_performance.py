...
class TestFeatureAnalysisPerformance(PerformanceTestSuite):
    ...
    def test_importance_analysis_scalability(self):
        ...
        for n_features in feature_counts:
            ...
            metrics = self._measure_performance(
                analyzer.analyze_feature_importance, weights, feature_names
            )
    ...
    def test_distribution_analysis_throughput(self):
        ...
        for n_samples, n_features in test_sizes:
            ...
            metrics = self._measure_performance(
                analyzer.analyze_feature_distributions, features
            )
            ...
            logger.info(
                f"分布分析スループット ({n_samples}×{n_features}): "
...
class TestFeatureDesignPerformance(PerformanceTestSuite):
    ...
    def test_task_feature_design_performance(self):
        ...
        for n_tasks in test_sizes:
            task_data = {
                ...
                "description": [
                    f"Description for task {i}" * 10 for i in range(n_tasks)
                ],
                "priority": np.random.choice(
                    ["low", "medium", "high"], n_tasks
                ).tolist(),
            }

            metrics = self._measure_performance(
                designer.design_enhanced_task_features, task_data
            )
    ...
    def test_developer_feature_design_memory_usage(self):
        ...
        metrics = self._measure_performance(
            designer.design_enhanced_developer_features, developer_data
        )
    ...
    def test_matching_feature_design_complexity(self):
        ...
        for n_tasks, n_developers in complexity_levels:
            ...
            metrics = self._measure_performance(
                designer.design_enhanced_matching_features, task_data, developer_data
            )
            ...
            logger.info(
                f"マッチング特徴量設計 ({n_tasks}×{n_developers}): "
                f"時間={metrics.execution_time:.3f}s"
            )
...
class TestFeatureOptimizationPerformance(PerformanceTestSuite):
    ...
    def test_scaling_performance_benchmark(self):
        ...
        for method in scaling_methods:
            ...
            for n_samples, n_features in benchmark_sizes:
                ...
                metrics = self._measure_performance(
                    scaler.fit_transform, features, method=method
                )
                ...
                size_category = (
                    "small"
                    if n_samples <= 1000
                    else "medium" if n_samples <= 10000 else "large"
                )
                threshold_key = f"{size_category}_dataset_time"

                self.assertLess(
                    metrics.execution_time,
                    self.benchmark_thresholds[threshold_key],
                    f"{method}スケーリングが遅すぎます: {metrics.execution_time:.2f}s",
                )

            logger.info(
                f"{method}スケーリングベンチマーク完了: {len(method_results)}サイズ"
            )
    ...
    def test_feature_selection_scalability(self):
        ...
        logger.info(
            f"特徴量選択スケーラビリティテスト完了: {len(scalability_results)}ポイント"
        )
...
class TestPipelinePerformance(PerformanceTestSuite):
    ...
    def test_full_pipeline_benchmark(self):
        ...
        for n_samples, n_features in benchmark_data_sizes:
            ...
            test_data = {"features": features, "labels": labels, "weights": weights}

            metrics = self._measure_performance(pipeline.run_full_pipeline, test_data)

            # データサイズに応じた性能要件チェック
            size_category = (
                "small"
                if n_samples <= 1000
                else "medium" if n_samples <= 5000 else "large"
            )
            threshold_key = f"{size_category}_dataset_time"
            ...
            logger.info(
                f"パイプラインベンチマーク ({n_samples}×{n_features}): "
                f"時間={metrics.execution_time:.2f}s, "
                f"メモリ={metrics.memory_usage_mb:.1f}MB, "
                f"スループット={metrics.throughput:.1f}/sec"
            )
    ...
    def test_pipeline_caching_performance(self):
        """パイプラインキャッシュ性能テスト"""
        config = {
            "pipeline": {"stages": ["analysis", "optimization"], "enable_cache": True},
        ...
        # 初回実行（キャッシュなし）
        metrics_first = self._measure_performance(pipeline.run_full_pipeline, test_data)

        # 2回目実行（キャッシュあり）
        metrics_cached = self._measure_performance(
            pipeline.run_full_pipeline, test_data
        )
        ...
        logger.info(
            f"キャッシュ性能: {speedup_ratio:.1f}x高速化 "
...
class TestLargeScaleDatasetPerformance(PerformanceTestSuite):
    ...
    def test_extreme_large_dataset_handling(self):
        ...
        for n_samples, n_features in large_sizes:
            ...
            expected_data_size = (
                n_samples * n_features * 8 / 1024 / 1024
            )  # float64で計算
            ...
            logger.info(
                f"大規模データセットテスト完了: {n_samples}×{n_features}, "
...
def generate_performance_report(
    test_results: Dict[str, List[PerformanceMetrics]], output_dir: Path
) -> str:
    """パフォーマンステスト結果レポートを生成"""
    ...
    for test_name, metrics_list in test_results.items():
        if metrics_list:
            ...
            # パフォーマンス評価
            time_class = (
                "good" if avg_time < 1.0 else "warning" if avg_time < 10.0 else "bad"
            )
            memory_class = (
                "good" if avg_memory < 50 else "warning" if avg_memory < 200 else "bad"
            )
            throughput_class = (
                "good"
                if avg_throughput > 1000
                else "warning" if avg_throughput > 100 else "bad"
            )
