...
class FeatureExtractor:
    ...
    def __init__(self, cfg):
        ...
        self.all_labels = ['bug', 'enhancement', 'documentation', 'question', 'help wanted']
    ...
    def get_features(self, task, developer, env) -> np.ndarray:
        ...
        # === カテゴリ2: 開発者の特徴 ===
        # 2.1 最近の活動量
        recent_activity_count = float(len(env.dev_action_history.get(developer.name, [])))
        ...
        for i, label_name in enumerate(self.all_labels):
            if label_vec[i] == 1.0:  # タスクがこのラベルを持っている場合のみ親和性を特徴量とする
