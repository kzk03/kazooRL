...
class GATFeatureExtractor:
    ...
    def _precompute_embeddings(self):
        ...
        try:
            # 開発者特徴量を取得
            dev_features = self.dev_network['dev_features_enhanced']  # [num_devs, feature_dim]
            edge_index = self.dev_network['dev_collaboration_edge_index']  # [2, num_edges]
            ...
            self.dev_embeddings = torch.matmul(torch.matmul(adj_matrix, dev_features), linear_transform)
    ...
    def print_statistics(self):
        ...
        if self.stats["total_calls"] > 0:
            success_rate = self.stats["successful_extractions"] / self.stats["total_calls"]
            print(f"GAT Feature Extraction Statistics:")
            print(f"  Total calls: {self.stats['total_calls']}")
            print(f"  Successful extractions: {self.stats['successful_extractions']} ({success_rate:.1%})")
