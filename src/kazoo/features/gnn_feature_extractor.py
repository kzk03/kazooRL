...
class GNNFeatureExtractor:
    ...
    def _load_gnn_model(self):
        ...
        try:
            ...
            # モデル読み込み
            model_path = Path(getattr(self.cfg.irl, 'gnn_model_path', 'data/gnn_model.pt'))
    ...
    def _create_id_mappings(self):
        ...
        self.task_id_to_idx = {str(task_id): idx for idx, task_id in enumerate(task_ids)}
        
        print(f"ID mappings created: {len(self.dev_id_to_idx)} devs, {len(self.task_id_to_idx)} tasks")
    ...
    def get_gnn_features(self, task, developer, env):
        ...
        try:
            ...
            all_task_sims = F.cosine_similarity(
                dev_emb.unsqueeze(0), 
                self.embeddings['task']
            )
            dev_expertise = torch.mean(torch.topk(all_task_sims, k=min(10, all_task_sims.size(0))).values).item()
            ...
            task_popularity = torch.mean(torch.topk(all_dev_sims, k=min(10, all_dev_sims.size(0))).values).item()
