...
class ImprovedPPOAgent:
    ...
    def __init__(self, obs_dim: int, act_space, config: Dict, device: str = "cpu"):
        ...
        # ネットワークの初期化
        self.policy = ImprovedActorCritic(obs_dim, act_space.n, network_config).to(
            self.device
        )
        self.policy_old = ImprovedActorCritic(obs_dim, act_space.n, network_config).to(
            self.device
        )
        ...
        if lr_schedule == "linear":
            self.scheduler = torch.optim.lr_scheduler.LinearLR(
                ...
                start_factor=1.0,
                end_factor=0.1,
                total_iters=config.get("total_timesteps", 500000)
                // config.get("rollout_len", 256),
            )
        else:
            self.scheduler = None
    ...
    ) -> Tuple[np.ndarray, torch.Tensor, None, torch.Tensor]:
        """行動選択と価値推定"""
        with torch.no_grad():
            # 状態の前処理
        ...
        for epoch in range(self.k_epochs):
            ...
            for start_idx in range(0, len(states), minibatch_size):
                ...
                if values_flat.shape != batch_rewards.shape:
                    print(
                        f"警告: 価値関数出力形状 {values_flat.shape} と報酬形状 {batch_rewards.shape} が一致しません"
                    )
                    # 形状を強制的に合わせる
