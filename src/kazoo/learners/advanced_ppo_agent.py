...
class AdvancedActorCritic(nn.Module):
    ...
    def _build_critic(self, hidden_dims: List[int], dropout_rate: float) -> nn.Module:
        """Criticネットワークの構築"""
        layers = []
...
class AdvancedPPOAgent:
    ...
    def __init__(
        self, obs_dim: int, action_dim: int, config: Dict, device: str = "cpu"
    ):
        self.obs_dim = obs_dim
        self.action_dim = action_dim
        ...
        # オプティマイザー
        self.optimizer = Adam(
            self.actor_critic.parameters(), lr=self.learning_rate, eps=1e-5
        )
        ...
        if self.use_adaptive_lr:
            self.scheduler = CosineAnnealingLR(
                self.optimizer,
                T_max=config.get("total_timesteps", 100000)
                // config.get("rollout_len", 2048),
    ...
    def update(self, rollout_buffer) -> Dict[str, float]:
        ...
        # 統計情報の更新
        num_updates = self.k_epochs * math.ceil(
            batch_size / self.config.get("minibatch_size", batch_size)
        )
