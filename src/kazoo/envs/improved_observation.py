...
class ImprovedObservationSpace:
    ...
    def __init__(self, config):
        ...
        # 正規化器
        self.scalers = {
            component: StandardScaler() for component in self.obs_components.keys()
        }
    ...
    def get_observation(
        self, agent_id: str, env_state: Dict, available_tasks: List[Dict]
    ) -> np.ndarray:
        """
        改良された観測ベクトルを生成
        ...
        graph_embeddings = self._extract_compressed_graph_embeddings(
            agent_id, env_state
        )
    ...
    def _extract_agent_features(self, agent_id: str, env_state: Dict) -> np.ndarray:
        ...
        try:
            ...
            # 活動パターン
            recent_activity = len(
                env_state.get("dev_action_history", {}).get(agent_id, [])
            )
            features[9] = min(recent_activity / 10.0, 1.0)

            # 現在の作業負荷
            current_workload = len(
                env_state.get("assignments", {}).get(agent_id, set())
            )
    ...
    def _extract_task_context_features(
        self, available_tasks: List[Dict], env_state: Dict
    ) -> np.ndarray:
        """タスクコンテキスト特徴の抽出"""
        ...
        try:
            ...
            # 緊急度分布
            urgency_levels = [
                self._calculate_task_urgency(task) for task in available_tasks[:10]
            ]
            if urgency_levels:
            ...
            # 協力度要求
            collab_requirements = [
                self._calculate_collaboration_requirement(task)
            ...
            learning_opportunities = [
                self._calculate_learning_opportunity(task)
                for task in available_tasks[:10]
            ]
    ...
    def _extract_environment_features(self, env_state: Dict) -> np.ndarray:
        ...
        try:
            ...
            total_assignments = sum(
                len(tasks) for tasks in env_state.get("assignments", {}).values()
            )
            total_developers = len(env_state.get("developers", {}))

            if total_developers > 0:
                features[0] = min(
                    total_assignments / (total_developers * 3.0), 1.0
                )  # 平均作業負荷
            ...
            active_devs = len(
                [
                    dev
                    for dev, actions in env_state.get("dev_action_history", {}).items()
                    if actions
                ]
            )
            features[4] = (
                min(active_devs / total_developers, 1.0) if total_developers > 0 else 0
            )
            ...
            if current_time and start_time:
                time_progress = (current_time - start_time).total_seconds() / (
                    365 * 24 * 3600
                )  # 年単位
            ...
            if env_state.get("assignments"):
                ...
                if workloads:
                    features[6] = np.std(workloads) / (
                        np.mean(workloads) + 1e-6
                    )  # 変動係数

            # システム効率性指標
            if len(env_state.get("completed_tasks", [])) > 0 and total_assignments > 0:
                features[7] = len(env_state["completed_tasks"]) / (
                    total_assignments + len(env_state["completed_tasks"])
                )
    ...
    def _extract_compressed_graph_embeddings(
        self, agent_id: str, env_state: Dict
    ) -> np.ndarray:
        """圧縮されたグラフ埋め込みの抽出"""
        ...
        try:
            # GNN埋め込みが利用可能な場合
            if (
                hasattr(env_state, "gnn_embeddings")
                and env_state.gnn_embeddings is not None
            ):
                # 開発者固有の埋め込みを取得
                if agent_id in env_state.gnn_embeddings:
                    ...
                    if len(raw_embedding) >= self.obs_components["graph_embeddings"]:
                        features = raw_embedding[
                            : self.obs_components["graph_embeddings"]
                        ]
                    else:
    ...
    def _generate_pseudo_graph_embedding(
        self, agent_id: str, env_state: Dict
    ) -> np.ndarray:
        """疑似グラフ埋め込みの生成"""
        features = np.zeros(self.obs_components["graph_embeddings"])
    ...
    def _normalize_observation(
        self, observation: np.ndarray, agent_id: str
    ) -> np.ndarray:
        """観測の正規化"""
        try:
            # 初回は正規化せずに返す
            ...
            for component, dim in self.obs_components.items():
                ...
                # 正規化器が学習済みの場合のみ適用
                try:
                    normalized_component = self.scalers[component].transform(
                        component_data
                    )
    ...
    def _calculate_learning_opportunity(self, task: Dict) -> float:
        """学習機会度計算"""
        # 複雑度ベース
        complexity = len(task.get("labels", [])) * 0.2 + min(
            task.get("comments_count", 0) / 10.0, 0.8
        )
