...
class ActionSpaceReducer:
    ...
    def get_candidate_developers(
        self, task: Dict, env_state: Dict, agent_id: str = None
    ) -> List[str]:
        """
        タスクに対する候補開発者を選出
        ...
        if remaining_devs:
            ...
            random_candidates = np.random.choice(
                remaining_devs, random_count, replace=False
            )
    ...
    def _get_activity_based_candidates(self, task: Dict, env_state: Dict) -> List[str]:
        ...
        if task is None:
            ...
            for dev_id, dev_info in developers.items():
                profile = dev_info["profile"]
                activity_score = profile.get("recent_commits", 0) + profile.get(
                    "recent_prs", 0
                )
...
class HierarchicalActionSpace:
    ...
    def get_action_space_for_task(
        self, task: Dict, env_state: Dict
    ) -> Tuple[List[str], Dict]:
        """
        特定のタスクに対する行動空間を取得
