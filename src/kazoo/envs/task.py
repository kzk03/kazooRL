...
class Task:
    def __init__(self, id, name, required_skills, complexity, deadline, dependencies=None):
        # --- 初期化メソッド ---
        self.id = id                   # タスクの一意なID
        self.name = name               # タスクの名前（例: "Fix login bug"）
        self.required_skills = required_skills # タスクを完了するために必要なスキル（リスト）
        self.complexity = complexity   # タスクの複雑さ（完了までにかかる時間などを決定）
        self.deadline = deadline       # タスクの完了期限
        self.dependencies = dependencies or [] # このタスクが依存する他のタスクのIDリスト
    ...
    def from_dict(cls, data):
        ...
        return cls(
            ...
            deadline=data.get("deadline", 100) # デッドラインがなければデフォルト値100を設定
        )
