...
class ImprovedOSSEnv(gym.Env):
    ...
    def step(self, actions: Dict[str, int]) -> Tuple[Dict, Dict, Dict, Dict, Dict]:
            ...
            if 0 <= action_idx < len(action_space):
                ...
                if selected_developer != "NO_OP" and self.backlog:
                    ...
                    reward, reward_breakdown = (
                        self.improved_reward_system.calculate_reward(
                            selected_developer,
                    ...
                    print(
                        f"Time {self.current_time.date()}: {selected_developer} started {selected_task.title}"
                    )
        ...
        for task in completed_tasks:
            if hasattr(task, "assigned_to") and task.assigned_to:
                env_state = self._get_env_state()
                completion_reward, completion_breakdown = (
                    self.improved_reward_system.calculate_reward(
    ...
    def _process_task_completion(self) -> List[Task]:
        ...
        for task in list(self.tasks_in_progress.values()):
            ...
            if np.random.rand() < completion_prob:
                ...
                # 割り当てから削除
                if (
                    hasattr(task, "assigned_to")
                    and task.assigned_to in self.assignments
                ):
    ...
    def _calculate_completion_probability(self, task: Task) -> float:
        ...
        if developer:
            ...
            # 総合確率
            adjusted_prob = (
                base_prob * rank_factor * experience_factor * workload_factor
            )
        ...
        for agent_id in self.agent_ids:
            ...
            if self.backlog and self.backlog[0] is not None:  # タスクがある場合のみ
                ...
                task_dict = (
                    ...
                    if hasattr(current_task, "__dict__")
                    else current_task
                )
                action_space, metadata = (
                    self.hierarchical_action.get_action_space_for_task(
                        task_dict, env_state
                    )
                )
            ...
            agent_id: {
                **base_info,
                ...
                "action_space_size": len(
                    self.current_action_mappings.get(agent_id, [])
                ),
            }
            for agent_id in self.agent_ids
        }
    ...
    def _update_stats(self, rewards: Dict, reward_breakdowns: Dict, actions: Dict):
        ...
        for agent_id, reward in rewards.items():
            ...
            for component, value in breakdown.items():
                self.episode_stats["reward_components"][
                    f"{agent_id}_{component}"
                ].append(value)
