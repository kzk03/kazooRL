...
class ObservationProcessor:
    ...
    def process_observation(
        self, raw_obs: Dict, env_state: Dict, agent_id: str
    ) -> np.ndarray:
        """
        生の観測データを処理済み特徴量に変換
        ...
        combined_features = np.concatenate(
            [task_features, dev_features, context_features, gnn_features]
        )
    ...
    def _extract_developer_features(self, agent_id: str, env_state: Dict) -> np.ndarray:
        ...
        if agent_id in developers:
            ...
            # 相対的な能力（全開発者との比較）
            all_ranks = [
                dev["profile"].get("rank", 5000) for dev in developers.values()
            ]
            all_commits = [
                dev["profile"].get("total_commits", 0) for dev in developers.values()
            ]

            if all_ranks:
                rank_percentile = (
                    sorted(all_ranks).index(profile.get("rank", 5000)) + 1
                ) / len(all_ranks)
                features[7] = rank_percentile

            if all_commits:
                commit_percentile = (
                    sorted(all_commits).index(profile.get("total_commits", 0)) + 1
                ) / len(all_commits)
