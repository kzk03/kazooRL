...
def optimize_meta_weights():
            ...
            try:
                ...
                for agent_name, model in system.models.items():
                    try:
                        ...
                        similarity_score = system._calculate_task_similarity(
                            task, agent_name
                        )
                        basic_score = (
                            0.4 * ppo_score
                ...
                # 他の手法のスコア
                contribution_scores = {
                    agent: min(system.author_contributions.get(agent, 0) / 200.0, 1.0)
                    for agent in system.models.keys()
                }
                similarity_scores = {
                    agent: system._calculate_task_similarity(task, agent)
                    for agent in system.models.keys()
                }
                temporal_scores = {
                    agent: system._calculate_temporal_match(task, agent)
                    for agent in system.models.keys()
                }
    ...
    for i, (pattern_name, result) in enumerate(sorted_results):
        ...
        print(
            f"| {i+1} | {pattern_name} | {accuracy*100:.1f}% | {weights[0]:.2f} | {weights[1]:.2f} | {weights[2]:.2f} | {weights[3]:.2f} |"
        )
