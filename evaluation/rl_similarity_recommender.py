...
class RLSimilarityEnvironment(gym.Env):
    ...
    def _get_observation(self):
        """現在の観測を生成"""
        # 1. TF-IDFによる類似度スコア
        tfidf_scores = self.similarity_recommender._predict_by_text_similarity(
        ...
        feature_scores = self.similarity_recommender._predict_by_features(
            self.current_task_data
        )
        feature_vector = []
        for dev in self.developers:
        ...
        # 3. タスクの基本特徴量
        basic_features = self.similarity_recommender.extract_basic_features(
            self.current_task_data
        )
        basic_vector = [
    ...
    def _calculate_reward(self, selected_dev):
        ...
        # 5. 開発者の専門性報酬
        dev_profile = self.similarity_recommender.developer_profiles.get(
            selected_dev, {}
        )
        ...
        task_labels = [
            ...
            for label in self.current_task_data.get("labels", [])
        ]
        if (
            task_labels
            and selected_dev in self.similarity_recommender.developer_profiles
        ):
            dev_label_prefs = self.similarity_recommender.developer_profiles[
                selected_dev
            ].get("label_preferences", {})
            label_match_score = 0.0

            for label in task_labels:
                if label in dev_label_prefs:
                    label_match_score += dev_label_prefs[label] / dev_profile.get(
                        "task_count", 1
                    )
...
class RLSimilarityRecommender:
    ...
    def train_base_models(self, data_path):
        ...
        # 学習ペア抽出
        training_pairs, developer_stats = (
            self.similarity_recommender.extract_training_pairs(training_data)
        )
        ...
        # 開発者プロファイル構築
        self.similarity_recommender.developer_profiles = (
            self.similarity_recommender.build_developer_profiles(training_pairs)
        )

        # TF-IDFモデル訓練
        self.similarity_recommender.train_text_similarity_model(
            self.similarity_recommender.developer_profiles
        )
    ...
    def train_rl_policy(self, training_pairs, total_timesteps=100000):
        ...
        def callback(locals_, globals_):
            if locals_["self"].num_timesteps % 10000 == 0:
                print(
                    f"   進捗: {locals_['self'].num_timesteps:,}/{total_timesteps:,} ステップ"
                )
            return True

        self.rl_agent.learn(total_timesteps=total_timesteps, callback=callback)
    ...
    def predict_with_rl(self, test_data):
        ...
        # テスト環境設定
        test_env = RLSimilarityEnvironment(self.similarity_recommender, [], self.config)
    ...
    def compare_with_baseline(self, test_data):
        ...
        # 1. 既存の類似度ベース予測
        print("   類似度ベース予測...")
        similarity_predictions, similarity_scores, test_assignments = (
            self.similarity_recommender.predict_assignments(
        ...
        comparison_results = {
            "similarity_method": {
                ...
                "metrics": self._evaluate_predictions(
                    similarity_predictions, test_assignments
                ),
            },
            "rl_method": {
    ...
    def save_comparison_results(self, comparison_results, output_dir):
        ...
        results = []
        all_task_ids = (
            set(comparison_results["similarity_method"]["predictions"].keys())
        ...
        for task_id in all_task_ids:
            result = {
                ...
                "similarity_prediction": comparison_results["similarity_method"][
                    "predictions"
                ].get(task_id),
                "rl_prediction": comparison_results["rl_method"]["predictions"].get(
                    task_id
                ),
                "similarity_correct": (
                    comparison_results["similarity_method"]["predictions"].get(task_id)
                    == comparison_results["test_assignments"].get(task_id)
                ),
                "rl_correct": (
                    comparison_results["rl_method"]["predictions"].get(task_id)
                    == comparison_results["test_assignments"].get(task_id)
                ),
            }
            ...
            if task_id in comparison_results["similarity_method"]["scores"]:
                ...
                result["similarity_combined_score"] = sim_score.get(
                    "combined_score", 0.0
                )
...
def main():
    ...
    parser.add_argument(
        "--timesteps", type=int, default=50000, help="強化学習の訓練ステップ数"
    )
