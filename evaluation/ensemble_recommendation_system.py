...
class EnsembleRecommendationSystem:
    ...
    def _calculate_task_specialization_match(self, task: Dict, author: str) -> float:
        ...
        # タスクの専門分野スコア
        task_spec_scores = {
            'bug_fix': 1.0 if any(kw in full_text for kw in ['bug', 'fix', 'error']) else 0.0,
            'feature': 1.0 if any(kw in full_text for kw in ['feature', 'enhancement']) else 0.0,
            'documentation': 1.0 if any(kw in full_text for kw in ['doc', 'readme']) else 0.0,
            'ui_ux': 1.0 if any(kw in full_text for kw in ['ui', 'ux', 'design']) else 0.0,
            'performance': 1.0 if any(kw in full_text for kw in ['performance', 'optimize']) else 0.0,
            'security': 1.0 if any(kw in full_text for kw in ['security', 'auth']) else 0.0,
        ...
        # マッチング度計算（コサイン類似度）
        dot_product = sum(author_spec[key] * task_spec_scores[key] for key in author_spec.keys())
    ...
    def ensemble_recommendation(self, task_features: torch.Tensor, task: Dict, k: int = 5) -> List[Tuple[str, float]]:
        """🚀 アンサンブル推薦 - 複数手法の統合"""
        agent_scores = {}
        ...
        for agent_name, model in self.models.items():
            try:
                ...
                # 3. 専門分野マッチングスコア
                specialization_score = self._calculate_task_specialization_match(task, agent_name)
    ...
    def adaptive_ensemble_recommendation(self, task_features: torch.Tensor, task: Dict, k: int = 5) -> List[Tuple[str, float]]:
    ...
    def evaluate_ensemble_system(self, method: str = "ensemble", sample_size: int = 500):
        """アンサンブルシステムの評価"""
        ...
        for k in [1, 3, 5]:
            ...
            for task, actual_author in tqdm(zip(eval_tasks, eval_ground_truth), 
                ...
                try:
                    ...
                    # 推薦方法の選択
                    if method == "ensemble":
                        recommendations = self.ensemble_recommendation(task_features, task, k)
                    elif method == "adaptive_ensemble":
                        recommendations = self.adaptive_ensemble_recommendation(task_features, task, k)
                    else:
...
def main():
    ...
    for method_key, method_name in methods:
        report_path = f"outputs/ensemble_fix/ensemble_{method_key}_report_{timestamp}.md"
        system.generate_ensemble_report(all_results[method_key], report_path, method_name)
    
    # 最良の結果を特定
    best_method = max(all_results.keys(), 
                     key=lambda x: all_results[x]['top_1']['accuracy'])
