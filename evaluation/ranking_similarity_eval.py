...
class RankingSimilarityEvaluator:
    ...
    def _setup_environment(self):
        ...
        self.env = SimpleTaskAssignmentEnv(
            cfg=cfg, backlog_data=training_backlog, dev_profiles_data=self.dev_profiles
        )
    ...
    def calculate_developer_similarity(self, actual_dev_profiles):
        ...
        for actual_dev, actual_vec in actual_vectors.items():
            ...
            for env_dev, env_vec in env_vectors.items():
                ...
                # ジャッカード類似度
                actual_skills = set(
                    skill for i, skill in enumerate(skill_list) if actual_vec[i] > 0
                )
                env_skills = set(
                    skill for i, skill in enumerate(skill_list) if env_vec[i] > 0
                )

                if len(actual_skills) > 0 or len(env_skills) > 0:
                    jaccard_sim = len(actual_skills & env_skills) / len(
                        actual_skills | env_skills
                    )
                else:
                    jaccard_sim = 0.0
        ...
        for task_data in test_tasks_with_assignments:
            try:
                ...
                if actual_dev in similarity_matrix:
                    ...
                    # 最も類似した開発者を見つける
                    best_match = max(
                        similarities.items(), key=lambda x: x[1]["combined"]
                    )
                ...
                ranking_results[task_id] = {
                    ...
                    "predicted_ranking": [
                        (env_dev, prob) for _, env_dev, prob in dev_predictions[:10]
                    ],
    ...
    def calculate_ranking_metrics(self, ranking_results):
        ...
        if best_match_ranks:
            ...
            metrics["top5_rank_ratio"] = sum(
                1 for r in best_match_ranks if r <= 5
            ) / len(best_match_ranks)
            metrics["top10_rank_ratio"] = sum(
                1 for r in best_match_ranks if r <= 10
            ) / len(best_match_ranks)

            print(f"   最類似開発者の平均ランク: {metrics['avg_best_match_rank']:.2f}")
            print(
                f"   最類似開発者の中央値ランク: {metrics['median_best_match_rank']:.1f}"
            )
        ...
        if best_match_similarities:
            ...
            print(
                f"   平均最類似度: {metrics['avg_best_match_similarity']:.3f} ± {metrics['similarity_std']:.3f}"
            )

        if top5_weighted_similarities:
            metrics["avg_top5_weighted_similarity"] = np.mean(
                top5_weighted_similarities
            )
            print(
                f"   Top-5重み付き類似度: {metrics['avg_top5_weighted_similarity']:.3f}"
            )
        ...
        if best_match_ranks and best_match_similarities:
            ...
            normalized_ranks = [
                1.0 - (r - 1) / 149 for r in best_match_ranks
            ]  # 150人中
            combined_scores = [
                r * s for r, s in zip(normalized_ranks, best_match_similarities)
            ]

            metrics["ranking_similarity_score"] = np.mean(combined_scores)
            print(
                f"   ランキング類似度総合スコア: {metrics['ranking_similarity_score']:.3f}"
            )
    ...
    def run_evaluation(self, output_dir="outputs"):
        ...
        # 2. ランキング予測
        predictions, ranking_results = self.predict_with_ranking(
            actual_assignments, actual_dev_profiles
        )
        ...
        with open(results_path, "w", encoding="utf-8") as f:
            ...
            json.dump(
                ranking_results, f, indent=2, ensure_ascii=False, default=convert_numpy
            )
