...
def get_active_developers_for_task(task, backlog_data, dev_profiles_data, activity_window_months=3):
    ...
    for other_task in backlog_data:
        ...
        if activity_start <= other_task_date < task_date:
            ...
            # PR作成者を追加
            if other_task.get("pull_request") or other_task.get("type") == "pull_request":
...
class SimpleRecommendationComponent:
            ...
            if task.get("assignees"):
                for assignee in task["assignees"]:
                    if assignee.get("login") and assignee["login"] in self.dev_profiles:
                        ...
                        if not stats[dev_name]['last_activity'] or task_date > stats[dev_name]['last_activity']:
    ...
    def calculate_simple_score(self, dev_name, task):
        ...
        if stats['last_activity']:
            days_since_last = (datetime.now().replace(tzinfo=None) - stats['last_activity'].replace(tzinfo=None)).days
    ...
    def _get_fallback_features(self, task, developer_name):
        ...
        try:
            ...
            if isinstance(labels, list) and labels:
                ...
                features[5] = 1.0 if any('bug' in label.lower() for label in label_names) else 0.0
                features[6] = 1.0 if any('enhancement' in label.lower() for label in label_names) else 0.0
                features[7] = 1.0 if any('documentation' in label.lower() for label in label_names) else 0.0
...
class HybridRecommendationSystem:
    ...
    def __init__(self, backlog_data, dev_profiles_data, irl_weights, ppo_model, env, feature_extractor):
        self.simple_component = SimpleRecommendationComponent(backlog_data, dev_profiles_data)
        self.rl_component = RLRecommendationComponent(irl_weights, ppo_model, env, feature_extractor, backlog_data)
        self.backlog_data = backlog_data
        self.dev_profiles = dev_profiles_data
    
    def recommend_developers(self, task, activity_window_months=3, num_recommendations=5, 
                           simple_weight=0.7, rl_weight=0.3):
        """
        ハイブリッド推薦を実行
        ...
        # 候補開発者リストを統合
        candidate_developers = list(task_author | actual_assignees | set(active_developers))
        
        # 候補が少ない場合は上位開発者を追加
        if len(candidate_developers) < 10:
            top_developers = sorted(
                self.dev_profiles.keys(), 
                key=lambda dev: self.dev_profiles.get(dev, {}).get('rank', float('inf'))
            )[:15]
        ...
        for dev_name in candidate_developers:
            ...
            rl_score = self.rl_component.calculate_rl_score(features) if features is not None else 0.0
            rl_scores[dev_name] = rl_score
            
            # ハイブリッドスコア
            hybrid_score = simple_weight * simple_score + rl_weight * rl_score
            
            developer_scores.append((dev_name, hybrid_score, {
...
def evaluate_hybrid_recommendations(backlog_data, dev_profiles_data, irl_weights, ppo_model, env, feature_extractor,
                                  activity_window_months=3, simple_weight=0.7, rl_weight=0.3):
    """ハイブリッド推薦システムを評価"""
    ...
    for task_idx, task in enumerate(tqdm(eval_tasks, desc="🎯 ハイブリッド推薦評価")):
        ...
        # 推薦実行
        try:
            recommendations_with_scores, candidate_info = hybrid_system.recommend_developers(
                task, activity_window_months, num_recommendations=5, 
                simple_weight=simple_weight, rl_weight=rl_weight
            )
            ...
            results["recommendation_details"].append({
                ...
                "scores": [details for dev_name, score, details in recommendations_with_scores],
...
def main():
    ...
    parser.add_argument("--irl-weights", required=True, help="学習済みIRL重みファイルのパス")
    parser.add_argument("--ppo-model", required=True, help="学習済みPPOモデルファイルのパス")
    parser.add_argument("--activity-months", type=int, default=3, help="活動期間の月数")
    parser.add_argument("--simple-weight", type=float, default=0.7, help="シンプルシステムの重み")
    parser.add_argument("--rl-weight", type=float, default=0.3, help="RLシステムの重み")
    parser.add_argument("--output", default="hybrid_recommendation_results.json", help="結果出力ファイル")
    ...
    results = evaluate_hybrid_recommendations(
        ...
        simple_weight=args.simple_weight, rl_weight=args.rl_weight
    )
    ...
    if total_tasks > 0:
        ...
        for i, detail in enumerate(results["recommendation_details"][:5]):
            ...
            if detail['scores']:
                hybrid_scores = [f"{s['hybrid_score']:.2f}" for s in detail['scores'][:3]]
                simple_scores = [f"{s['simple_score']:.2f}" for s in detail['scores'][:3]]
