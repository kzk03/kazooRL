...
def investigate_ndeloof_exclusion():
    ...
    print(f"   ndeloofが訓練済みモデルに含まれるか: {'✅' if 'ndeloof' in trained_models else '❌'}")
    ...
    if 'ndeloof' not in trained_models:
        ...
        if os.path.exists(ndeloof_model_path):
            ...
            # モデル読み込みを試行
            try:
                model_data = torch.load(ndeloof_model_path, map_location='cpu', weights_only=False)
                print(f"     モデル読み込み: ✅ 成功")
    ...
    if 'ndeloof' in trained_models:
        ...
        # ndeloofのタスクでのスコア分析
        ndeloof_tasks = [(i, task) for i, (task, author) in enumerate(zip(tasks, ground_truth)) if author == 'ndeloof']
            ...
            for i, (task_idx, task) in enumerate(sample_tasks, 1):
                ...
                try:
                    ...
                    for rank, (agent, score) in enumerate(sorted_agents[:5], 1):
                        ...
                        contribution = author_contribution.get(agent, 0)
                        print(f"       {rank}. {marker} {agent}: {score:.3f} ({contribution}タスク)")
                    ...
                    print(f"       ndeloofのカテゴリ: {'高貢献者' if 'ndeloof' in high_contributors else '中貢献者' if 'ndeloof' in medium_contributors else '低貢献者'}")
                    
                    # 各カテゴリでの順位
                    high_candidates = [(agent, score) for agent, score in agent_scores.items() if agent in high_contributors]
                    ...
                    print(f"       高貢献者内での順位: {ndeloof_high_rank}/{len(high_candidates)}")
                    print(f"       高貢献者上位2位: {[agent for agent, _ in high_candidates[:2]]}")
                    
                    # なぜ除外されたかの分析
                    if ndeloof_high_rank and ndeloof_high_rank > 2:
                        print(f"       🎯 除外理由: 高貢献者内で{ndeloof_high_rank}位のため、上位2位に入らなかった")
    ...
    if 'ndeloof' not in trained_models:
        ...
        print(f"\n   max_models制限の確認:")
        print(f"     現在の制限: 50モデル")
        print(f"     実際の重複開発者数: {len(set(ground_truth).intersection(set([f.replace('agent_', '').replace('.pth', '') for f in os.listdir('models/improved_rl/final_models') if f.endswith('.pth')])))}人")
...
if __name__ == "__main__":
    ...
    print(f"\n🎯 調査結果まとめ:")
    print(f"   ndeloofがモデルに含まれる: {'✅' if results['ndeloof_in_models'] else '❌'}")
