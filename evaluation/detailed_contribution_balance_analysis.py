...
def analyze_contribution_balance_detailed():
    ...
    for author in sorted(medium_contributors, key=lambda x: author_contribution[x], reverse=True):
        print(f"     {author}: {author_contribution[author]}タスク")
    
    print(f"\n   低貢献者 (1-9タスク): {len(low_contributors)}人")
    low_contrib_counts = Counter([author_contribution[author] for author in low_contributors])
    ...
    for i, (task, actual_author) in enumerate(zip(tasks[:sample_size], ground_truth[:sample_size])):
        ...
        try:
            ...
            if len(detailed_examples) < 10:
                actual_category = "高" if actual_author in high_contributors else \
                                "中" if actual_author in medium_contributors else "低"
    ...
    for i, example in enumerate(detailed_examples[:5], 1):
        print(f"\n   例{i}: {example['title']}")
        print(f"     実際の作成者: {example['actual_author']} ({example['actual_category']}貢献者, {example['actual_contribution']}タスク)")
        print(f"     推薦結果:")
        
        for j, (agent, score) in enumerate(example['recommendations'], 1):
            category = "高" if agent in high_contributors else \
                      "中" if agent in medium_contributors else "低"
            contribution = author_contribution.get(agent, 0)
            marker = "👑" if agent == example['actual_author'] else "  "
            print(f"       {j}. {marker} {agent} ({category}貢献者, {contribution}タスク, スコア: {score:.3f})")
    ...
    # 各カテゴリの実際の作成者分布
    actual_high = sum(1 for author in ground_truth if author in high_contributors and author in available_agents)
    actual_medium = sum(1 for author in ground_truth if author in medium_contributors and author in available_agents)
    actual_low = sum(1 for author in ground_truth if author in low_contributors and author in available_agents)
