...
def evaluate_simple_recommendations(backlog_data, dev_profiles_data, activity_window_months=3):
    """シンプル推薦システムを評価"""
    ...
    for task_idx, task in enumerate(tqdm(eval_tasks, desc="🎯 シンプル推薦評価")):
        ...
        # 推薦実行
        try:
            recommendations_with_scores, candidate_info = recommender.recommend_developers(
                task, activity_window_months
            )
            ...
            results["recommendation_details"].append({
                ...
                "scores": [score for dev_name, score in recommendations_with_scores],
...
def main():
    parser = argparse.ArgumentParser(description="シンプルなアクティブ開発者推薦システム評価")
    ...
    parser.add_argument("--output", default="simple_active_recommendation_results.json", help="結果出力ファイル")
