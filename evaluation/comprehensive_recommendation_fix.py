...
class ComprehensiveRecommendationSystem:
    ...
    def contribution_weighted_recommendation(
    ...
    def adaptive_balanced_recommendation(
        self, task_features: torch.Tensor, k: int = 5
    ) -> List[Tuple[str, float]]:
        """適応的バランス推薦（最適化版）"""
        # 各カテゴリで候補を収集
    ...
    def evaluate_recommendation_system(
        self, method: str = "adaptive_balanced", sample_size: int = 500
    ):
        """推薦システムの評価"""
            ...
            for task, actual_author in tqdm(
                ...
                try:
                    ...
                    if method == "contribution_weighted":
                        recommendations = self.contribution_weighted_recommendation(
                            task_features, k
                        )
                    elif method == "adaptive_balanced":
                        recommendations = self.adaptive_balanced_recommendation(
                            task_features, k
                        )
                    else:
...
def main():
    ...
    # 最良の結果でレポート生成
    best_method = max(
        all_results.keys(), key=lambda x: all_results[x]["top_3"]["accuracy"]
    )
