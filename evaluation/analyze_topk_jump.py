...
def load_sample_models(model_dir: str, actual_authors: List[str], max_models: int = 20) -> Dict[str, PPOPolicyNetwork]:
    """サンプルモデルを読み込み"""
...
def analyze_topk_jump():
    ...
    trained_models = load_sample_models("models/improved_rl/final_models", ground_truth, 20)
    ...
    for i, (task, actual_author) in enumerate(tqdm(zip(tasks[:sample_size], ground_truth[:sample_size]), desc="分析中")):
    ...
    if score_distributions:
        ...
        # スコア差が小さい場合の分析
        small_diff_count = sum(1 for item in ranking_analysis if item['score_diff'] < 0.1)
        print(f"   スコア差<0.1のタスク: {small_diff_count} ({small_diff_count/total_analyzed*100:.1f}%)")
    ...
    # Top-3に入った例を分析
    top3_examples = [item for item in ranking_analysis if 2 <= item['actual_rank'] <= 3][:5]
    
    print("### Top-3に入った例（2-3位の正解）:")
    for i, example in enumerate(top3_examples, 1):
        print(f"\n   例{i}: {example['actual_author']} ({example['actual_rank']}位)")
        print(f"     1位: {example['top1_agent']} (スコア: {example['top1_score']:.3f})")
        print(f"     実際: {example['actual_author']} (スコア: {example['actual_score']:.3f})")
    ...
    improvement_ratio = top3_accuracy/top1_accuracy if top1_accuracy > 0 else float('inf')
    print(f"   - 改善倍率: {'∞' if improvement_ratio == float('inf') else f'{improvement_ratio:.1f}'}倍")
