...
class SimilarityBasedRecommender:
    ...
    def extract_features_for_training(self, training_pairs):
        ...
        for i, pair in enumerate(training_pairs):
            try:
                ...
                dev_profile = all_dev_profiles.get(developer_name, {
                    'name': developer_name,
                    'skills': [],
                    'expertise': []
                })
        ...
        for dev_name, data in developer_profiles.items():
            if data['task_count'] > 0:
                ...
                data['avg_features'] = np.zeros_like(features[0] if features else np.zeros(62))
    ...
    def find_similar_developers(self, test_developers, top_k=5):
        ...
        for test_dev in test_developers:
            ...
            test_embedding = np.random.normal(0, 0.1, len(list(self.developer_embeddings.values())[0]))
            ...
            try:
                ...
                if actual_dev in similarity_mapping:
                    ...
                    for similar_dev, similarity_score in similar_devs:
                        ...
                        # 特徴量抽出
                        features = self.feature_extractor.get_features(task_obj, dev_obj, dummy_env)
                        ...
                        dev_scores.append((similar_dev, weighted_score, pred_proba, similarity_score))
                    
                    # 最高スコアの開発者を予測
                    if dev_scores:
                        dev_scores.sort(key=lambda x: x[1], reverse=True)
                        best_dev, best_score, best_proba, best_similarity = dev_scores[0]
    ...
    def evaluate_predictions(self, predictions, test_assignments, prediction_scores):
        ...
        metrics = {
            ...
            'confidence_std': np.std(prediction_confidences) if prediction_confidences else 0.0
        }
        
        print(f"   精度: {accuracy:.3f} ({correct_predictions}/{len(common_tasks)})")
        print(f"   平均類似度: {metrics['avg_similarity']:.3f} ± {metrics['similarity_std']:.3f}")
        print(f"   平均信頼度: {metrics['avg_confidence']:.3f} ± {metrics['confidence_std']:.3f}")
    ...
    def run_full_pipeline(self, data_path, output_dir="outputs"):
        ...
        # 4. 学習用特徴量抽出
        features, labels, developer_profiles = self.extract_features_for_training(training_pairs)
