...
class ImprovedGATRLEnvironment(gym.Env):
    ...
    def setup_feature_extractor(self):
        ...
        print(
            f"   GAT特徴量: {sum(1 for name in self.feature_extractor.feature_names if 'gat_' in name)}"
        )
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.training_pairs:
            ...
            self.current_pair = {
                ...
                "developer": (
                    self.active_developers[0] if self.active_developers else "unknown"
                ),
                "task_id": "dummy",
            }
...
class ImprovedGATRLRecommender:
    ...
    def train_improved_rl_agent(
        self, training_data, dev_profiles, total_timesteps=50000
    ):
        """改善版GAT強化学習エージェントを訓練"""
    ...
    def compare_with_baseline(
        self, gat_predictions, test_assignments, baseline_path=None
    ):
        """シンプル類似度ベースとの詳細比較"""
        print("📊 詳細ベースライン比較実行中...")
        ...
        gat_correct = sum(
            1
            for task_id in gat_common_tasks
        ...
        if baseline_path and Path(baseline_path).exists():
            try:
                ...
                print(
                    f"   精度: {baseline_accuracy:.3f} ({baseline_correct}/{baseline_total})"
                )

                improvement = gat_accuracy - baseline_accuracy
                print(
                    f"📈 改善度: {improvement:+.3f} ({improvement/baseline_accuracy*100:+.1f}%)"
                )
    ...
    def run_full_pipeline(self, data_path, baseline_path=None, total_timesteps=50000):
        ...
        # 4. 予測実行
        predictions, scores, test_assignments = self.predict_with_improved_gat_rl(
            test_data, dev_profiles
        )

        # 5. ベースラインとの比較
        metrics = self.compare_with_baseline(
            predictions, test_assignments, baseline_path
        )
...
def main():
    ...
    parser.add_argument("--baseline", help="ベースライン結果CSVパス")
    ...
    metrics = recommender.run_full_pipeline(args.data, args.baseline, args.timesteps)
