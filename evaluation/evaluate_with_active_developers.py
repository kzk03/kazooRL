...
def get_active_developers_for_task(task, backlog_data, dev_profiles_data, 
                                 activity_window_months=3, debug=False):
    """
    ...
    if debug:
        print(f"   デバッグ: タスク日付 {task_date}, 活動期間 {activity_start} - {task_date}")
    ...
    if debug:
        print(f"   デバッグ: 関連タスク数 {relevant_tasks}, アクティブ開発者数 {len(active_developers)}")
...
def analyze_developer_activity_patterns(backlog_data, dev_profiles_data):
    ...
    for dev, count in Counter(dev_task_counts).most_common(10):
        latest_activity = max(developer_activity[dev]) if developer_activity[dev] else None
...
class ActiveDeveloperRecommendationSystem:
    ...
    def _get_fallback_features(self, task, developer_name):
        ...
        try:
            ...
            features[1] = min(dev_profile.get('total_commits', 0) / 100.0, 1.0)  # コミット数
    ...
    def recommend_developers_with_activity(self, task, activity_window_months=3, num_recommendations=5):
        """
        ...
        active_developers = get_active_developers_for_task(
            task, self.backlog_data, self.dev_profiles, activity_window_months, debug=False
        )
        ...
        if len(candidate_developers) == 0:
            ...
            return [], {
                ...
                "is_pull_request": bool(task.get("pull_request") or task.get("type") == "pull_request")
            }
...
def evaluate_active_developer_recommendations(
    ...
    print(f"📊 評価対象タスク（担当者またはPR作成者あり）: {len(tasks_with_assignees)}/{len(backlog_data)}")
    ...
    for task_idx, task in task_progress:
        ...
        try:
            ...
            # アクティブ開発者に基づく推薦を実行
            recommendations_with_scores, candidate_info = recommender.recommend_developers_with_activity(
            ...
            if not recommendations_with_scores:
                if task_idx < 5:  # 最初の5つの失敗のみ詳細表示
                    ...
                    print(f"   候補開発者数: {candidate_info.get('final_candidates_count', 0)}")
    ...
    # 統計情報を計算
    if results["total_tasks"] > 0:
        results["activity_stats"]["avg_active_developers_per_task"] = total_active_developers / results["total_tasks"]
...
def main():
    ...
    parser.add_argument(
        "--activity-months", type=int, default=3, help="活動期間の月数（デフォルト: 3ヶ月）"
    )
    parser.add_argument(
        "--output", default="active_developer_recommendation_results_2023.json", help="結果出力ファイル"
    )
    ...
    if total_tasks > 0:
        ...
        print(f"活動期間: {args.activity_months}ヶ月")
        print(f"平均候補開発者数: {results['activity_stats']['avg_active_developers_per_task']:.1f}人")
        print(f"")
