...
def main():
    parser = argparse.ArgumentParser(
        description="学習済みモデルを2022年テストデータで評価"
    )
    parser.add_argument("--config", required=True, help="設定ファイルのパス")
    parser.add_argument(
        "--learned-weights", required=True, help="学習済み重みファイルのパス"
    )
    parser.add_argument(
        "--output", default="evaluation_results_2022.json", help="結果出力ファイル"
    )
    ...
    results = evaluate_recommendations(
        backlog_data, dev_profiles_data, feature_extractor, learned_weights
    )
