...
class ImprovedGATRLEnvironmentV2(gym.Env):
    ...
    def __init__(self, config, training_data, dev_profiles):
        ...
        if Path(irl_weights_path).exists():
            self.irl_weights = torch.tensor(
                np.load(irl_weights_path), dtype=torch.float32
            )
            print(
                f"✅ IRL重み読み込み: {irl_weights_path} ({tuple(self.irl_weights.shape)})"
            )
        else:
...
class ImprovedGATRLRecommenderV2:
    ...
    def train_improved_rl_agent_v2(
        self, training_data, dev_profiles, total_timesteps=100000
    ):
    ...
    def predict_with_improved_gat_rl_v2(self, test_data, dev_profiles):
        ...
        active_test_tasks = {
            task_id: dev
            for task_id, dev in test_assignments.items()
        ...
        for task_data in test_data:
            ...
            try:
                ...
                with torch.no_grad():
                    action_probs = (
                        self.rl_agent.policy.get_distribution(obs_tensor)
                        .distribution.probs.squeeze()
                        .cpu()
                        .numpy()
                    )
                ...
                if progress_count % 100 == 0:
                    print(
                        f"   進捗: {progress_count // len(test_env.active_developers)}/{len(active_test_tasks)}"
                    )
    ...
    def run_full_pipeline_v2(self, data_path, total_timesteps=100000):
        ...
        # 4. 予測実行
        predictions, prediction_scores, test_assignments = (
            self.predict_with_improved_gat_rl_v2(test_data, dev_profiles)
        )

        # 5. 評価
        metrics = self.evaluate_predictions_v2(
    ...
    def evaluate_predictions_v2(
        self, predictions, test_assignments, prediction_scores=None
    ):
        """予測結果の評価v2（Top-K評価含む）"""
        ...
        # Top-K評価
        topk_metrics = self._evaluate_topk_accuracy_v2(
    ...
    def _evaluate_topk_accuracy_v2(
...
def main():
    ...
    parser.add_argument("--timesteps", type=int, default=100000, help="訓練ステップ数")
