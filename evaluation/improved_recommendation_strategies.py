...
class ImprovedRecommendationSystem:
    ...
    def __init__(
        self, models: Dict[str, PPOPolicyNetwork], author_contributions: Dict[str, int]
    ):
        self.models = models
    ...
    def original_recommendation(
        self, task_features: torch.Tensor, k: int = 5
    ...
    def diversity_weighted_recommendation(
        self, task_features: torch.Tensor, k: int = 5, diversity_weight: float = 0.3
    ) -> List[Tuple[str, float]]:
        """多様性重み付き推薦"""
        agent_scores = {}
        for agent_name, model in self.models.items():
            try:
                ...
                max_recommendations = (
                    max(self.recommendation_history.values())
                    if self.recommendation_history
                    else 1
                )
                diversity_bonus = (
                    1 - recommendation_count / (max_recommendations + 1)
                ) * diversity_weight
    ...
    def contribution_balanced_recommendation(
        self, task_features: torch.Tensor, k: int = 5
    ) -> List[Tuple[str, float]]:
        """貢献量バランス推薦"""
        # 各カテゴリから候補を選出
    ...
    def temperature_scaled_recommendation(
        self, task_features: torch.Tensor, k: int = 5, temperature: float = 2.0
    ) -> List[Tuple[str, float]]:
        """温度スケーリング推薦（確率的選択）"""
        agent_scores = {}
        for agent_name, model in self.models.items():
        ...
        recommendations = [
            (agents[i], agent_scores[agents[i]]) for i in selected_indices
        ]
...
def evaluate_improved_strategies():
    ...
    for strategy_name, strategy_desc in strategies.items():
        ...
        for i, (task, actual_author) in enumerate(
            ...
            try:
                ...
                if strategy_name == "original":
                    recommendations = improved_system.original_recommendation(
                        task_features, 3
                    )
                elif strategy_name == "diversity_weighted":
                    recommendations = improved_system.diversity_weighted_recommendation(
                        task_features, 3
                    )
                elif strategy_name == "contribution_balanced":
                    recommendations = (
                        improved_system.contribution_balanced_recommendation(
                            task_features, 3
                        )
                    )
                elif strategy_name == "temperature_scaled":
                    recommendations = improved_system.temperature_scaled_recommendation(
                        task_features, 3
                    )
                elif strategy_name == "hybrid":
                    recommendations = improved_system.hybrid_recommendation(
                        task_features, 3
                    )
        ...
        evaluated_tasks = len(
            [
                task
                for task, author in zip(tasks[:sample_size], ground_truth[:sample_size])
        ...
        if all_recommendations:
            ...
            strategy_results["diversity_score"] = (
                unique_recommendations / total_recommendations
            )
        ...
        if total_recs > 0:
            high_pct = (
                strategy_results["contribution_distribution"]["high"] / total_recs * 100
            )
            medium_pct = (
                strategy_results["contribution_distribution"]["medium"]
                / total_recs
                * 100
            )
            low_pct = (
                strategy_results["contribution_distribution"]["low"] / total_recs * 100
            )
    ...
    for strategy_name, strategy_desc in strategies.items():
        ...
        if total_recs > 0:
            ...
            medium_pct = (
                result["contribution_distribution"]["medium"] / total_recs * 100
            )
        ...
        print(
            f"| {strategy_desc} | {result['top3_accuracy']:.3f} | {result['diversity_score']:.3f} | "
