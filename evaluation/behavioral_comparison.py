...
class BehavioralComparison:
    ...
    def calculate_metrics(self, actual_assignments, predictions, prediction_scores):
        ...
        metrics['mapping_accuracy'] = dev_mapping_matches / len(actuals) if actuals else 0
        
        # 重複開発者での精度（より公平な評価）
        if overlap_task_count > 0:
            metrics['overlap_accuracy'] = overlap_exact_matches / overlap_task_count
            print(f"   重複開発者精度: {overlap_exact_matches}/{overlap_task_count} = {metrics['overlap_accuracy']:.3f}")
        else:
            metrics['overlap_accuracy'] = 0.0
            print("   重複開発者精度: N/A（重複開発者なし）")
        
        print(f"   全体正確一致: {exact_matches}/{len(common_tasks)} = {metrics['exact_accuracy']:.3f}")
        print(f"   マッピング一致: {dev_mapping_matches}/{len(actuals)} = {metrics['mapping_accuracy']:.3f}")
            ...
            try:
                ...
                for k in [1, 3, 5, 10]:
                    if k <= len(self.env.developers):
                        ...
                        for task_id in common_tasks:
                            if task_id in prediction_scores:
                                ...
                                if actual_dev in self.env.developers:
                                    actual_idx = self.env.developers.index(actual_dev)
                                    top_k_actions = prediction_scores[task_id]['top_5_actions'][:k]
                        ...
                        top_k_scores[f'top_{k}_accuracy'] = top_k_correct / len(common_tasks)
                        if top_k_overlap_total > 0:
                            top_k_scores[f'top_{k}_overlap_accuracy'] = top_k_overlap_correct / top_k_overlap_total
                        else:
                            top_k_scores[f'top_{k}_overlap_accuracy'] = 0.0
                        
                        print(f"   Top-{k}精度: {top_k_correct}/{len(common_tasks)} = {top_k_scores[f'top_{k}_accuracy']:.3f}")
                        if top_k_overlap_total > 0:
                            print(f"   Top-{k}重複精度: {top_k_overlap_correct}/{top_k_overlap_total} = {top_k_scores[f'top_{k}_overlap_accuracy']:.3f}")
        ...
        # 開発者レベルの統計
        dev_stats = self._calculate_developer_stats(actual_assignments, predictions, common_tasks)
        metrics['developer_stats'] = dev_stats
        
        return metrics
    
    def _calculate_recommendation_metrics(self, actual_assignments, predictions, prediction_scores, common_tasks):
        """推薦システムとしての評価指標"""
        print("🎯 推薦システム指標計算中...")
        ...
        if confidence_scores:
            ...
            print(f"   平均予測信頼度: {metrics['avg_confidence']:.3f} ± {metrics['confidence_std']:.3f}")
        ...
        for task_id in common_tasks:
            if task_id in prediction_scores:
                ...
                if actual_dev in self.env.developers:
                    ...
                    try:
                        position = np.where(sorted_indices == actual_idx)[0][0] + 1  # 1-based
                        ranking_positions.append(position)
                        
                        # ランキング類似度計算（位置ベース）
                        # 1位なら1.0、最下位なら0.0に近づく
                        similarity = 1.0 - (position - 1) / (len(self.env.developers) - 1)
        ...
        if ranking_positions:
            ...
            metrics['ranking_top_1_ratio'] = sum(1 for p in ranking_positions if p == 1) / len(ranking_positions)
            metrics['ranking_top_5_ratio'] = sum(1 for p in ranking_positions if p <= 5) / len(ranking_positions)
            metrics['ranking_top_10_ratio'] = sum(1 for p in ranking_positions if p <= 10) / len(ranking_positions)
        ...
        if len(overlap_devs) >= 2:  # 相関計算には最低2人必要
            for task_id in common_tasks:
                if task_id in prediction_scores:
                    ...
                    if actual_dev in overlap_devs:
                        # 重複開発者のみでランキングを作成
                        overlap_indices = [self.env.developers.index(dev) for dev in overlap_devs]
                        
                        if 'probabilities' in prediction_scores[task_id]:
                            all_probs = prediction_scores[task_id]['probabilities']
                            overlap_probs = [all_probs[idx] if idx < len(all_probs) else 0.0 for idx in overlap_indices]
                        else:
                            # デフォルト確率
                            overlap_probs = [0.1] * len(overlap_indices)
                        
                        # 実際のランキング（実際の開発者が1位）
                        actual_ranking = [1 if dev == actual_dev else 2 for dev in overlap_devs]
                        
                        # 予測ランキング
                        predicted_ranking = np.argsort(np.argsort(overlap_probs)[::-1]) + 1
                        
                        if len(set(actual_ranking)) > 1 and len(set(predicted_ranking)) > 1:
                            # シンプルなスピアマン順位相関の実装
                            ...
                            correlation = simple_spearman(actual_ranking, predicted_ranking)
            ...
            if spearman_correlations:
                ...
                print(f"   平均スピアマン順位相関: {metrics['avg_spearman_correlation']:.3f}")
        ...
        for task_id in common_tasks:
            if task_id in prediction_scores:
                ...
                # 予測確率ベクトル
                if 'probabilities' in prediction_scores[task_id]:
                    predicted_vector = np.array(prediction_scores[task_id]['probabilities'])
                else:
                ...
                # コサイン類似度計算
                if np.linalg.norm(actual_vector) > 0 and np.linalg.norm(predicted_vector) > 0:
        ...
        if developer_similarity_scores:
            ...
            print(f"   平均コサイン類似度: {metrics['avg_cosine_similarity']:.3f} ± {metrics['cosine_similarity_std']:.3f}")
        ...
        if total_predictions > 0:
            ...
            metrics['prediction_concentration'] = max(predicted_dev_counts.values()) / total_predictions
    ...
    def _calculate_developer_stats(self, actual_assignments, predictions, common_tasks):
        ...
        for dev in dev_stats['actual_distribution']:
            ...
            dev_stats['per_developer_accuracy'][dev] = correct / len(dev_tasks) if dev_tasks else 0
    ...
    def _create_visualizations(self, metrics, output_dir, timestamp):
        ...
        try:
            ...
            print("⚠️ matplotlib/seabornがインストールされていません。可視化をスキップします。")
        except Exception as e:
            print(f"⚠️ 可視化エラー: {e}")
    ...
    def run_evaluation(self, output_dir="outputs"):
        ...
        # 評価指標計算
        metrics = self.calculate_metrics(actual_assignments, predictions, prediction_scores)
...
def main():
    ...
    # パスの存在確認
    for path_name, path_value in [('config', args.config), ('model', args.model), ('test-data', args.test_data)]:
