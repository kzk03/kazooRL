...
class MultiMethodRLEnvironment(gym.Env):
    ...
    def __init__(self, extraction_method="assignees"):
        ...
        # データ読み込み
        self.training_data, self.test_data = self.recommender.load_data(
            "data/backlog.json"
        )

        # 抽出方法に応じて学習データを準備
        self.training_pairs, self.developer_stats = (
            self.recommender.extract_training_pairs(
    ...
    def _train_base_models(self):
        ...
        # 開発者プロファイル構築
        self.developer_profiles = self.recommender.build_developer_profiles(
            self.training_pairs
        )
        ...
        print(
            f"   ベースモデル訓練完了: TF-IDF行列 {getattr(self.recommender, 'tfidf_matrix', 'N/A')}"
        )
    ...
    def step(self, action):
        ...
        reward = self._calculate_reward(
            predicted_developer, actual_developer, current_task
        )
        ...
        self.success_rate = (
            np.mean(self.recent_successes) if self.recent_successes else 0.0
        )
    ...
    def _get_observation(self):
        ...
        # 全特徴量を結合
        obs = np.concatenate([task_features, pool_stats, history_stats]).astype(
            np.float32
        )
    ...
    def _extract_pool_statistics(self):
        ...
        avg_activity = (
            np.mean(list(self.developer_stats.values())) if self.developer_stats else 0
        )
...
class MultiMethodRLRecommender:
    ...
    def _evaluate_method(self, method, agent, env, test_data):
        ...
        for task_data in test_data:
            ...
            # この抽出方法で開発者を抽出
            developers = recommender.extract_developers_from_task(
        ...
        # 結果集計
        accuracy = (
            correct_predictions / len(test_assignments) if test_assignments else 0.0
        )
    ...
    def compare_results(self):
                ...
                comparison_data.append(
                    {
                        ...
                        "coverage": result["total_predictions"]
                        / max(
                            1,
                            sum(r["total_predictions"] for r in self.results.values()),
                        )
                        * 100,
                    }
                )
        ...
        for data in comparison_data:
            print(
                f"{data['method']:12} | {data['accuracy']:.3f} | {data['predictions']:6} | {data['developers']:8} | {data['coverage']:6.1f}%"
            )
    ...
    def save_results(self, output_dir="outputs"):
        ...
        for method in self.extraction_methods:
            if method in self.results:
                # メトリクス保存
                metrics_path = (
                    output_dir / f"multi_method_{method}_metrics_{timestamp}.json"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="マルチメソッド強化学習タスク推薦システム"
    )
    parser.add_argument(
        "--timesteps", type=int, default=10000, help="各メソッドの訓練ステップ数"
    )
    ...
    print(
        f"   最大カバレッジ: {max(comparison_results, key=lambda x: x['predictions'])['method']}"
    )
