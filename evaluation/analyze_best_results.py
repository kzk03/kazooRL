...
def analyze_meta_ensemble_details():
    ...
    for i, (author, accuracy) in enumerate(sorted_authors[:10]):
        ...
        print(
            f"   {i+1}. {author}: {accuracy*100:.1f}% ({int(accuracy*total_attempts)}/{total_attempts}) [貢献{contribution}]"
        )

    # 成功事例分析
    print(f"\n### ✅ 成功事例 (Top-5)")
    correct_sorted = sorted(
        correct_predictions, key=lambda x: x["predicted_score"], reverse=True
    )
    for i, case in enumerate(correct_sorted[:5]):
        print(f"   {i+1}. タスク: {case['task_title']}")
        print(f"      タイプ: {case['task_type']}, 著者: {case['actual_author']}")
        print(
            f"      スコア: {case['predicted_score']:.3f}, 貢献: {case['contribution']}"
        )
        print()
    ...
    for i, case in enumerate(high_contrib_errors[:3]):
        ...
        print(
            f"      予測: {case['predicted_author']} (スコア{case['predicted_score']:.3f})"
        )
        print()
