...
def analyze_successful_predictions():
    ...
    print(f"\n   中貢献者リスト:")
    for author, count in sorted(medium_contributors, key=lambda x: x[1], reverse=True)[:10]:
        percentage = count / total_tasks * 100
    ...
    for i, (task, actual_author) in enumerate(tqdm(zip(tasks[:sample_size], ground_truth[:sample_size]), desc="推薦分析中")):
        ...
        try:
            ...
            if actual_author in top3_agents:
                actual_rank = next(rank for rank, (agent, _) in enumerate(sorted_agents, 1) if agent == actual_author)
    ...
    for k in ['top1', 'top3', 'top5']:
        ...
        for author, count in success_counter.most_common():
            ...
            success_rate = count / total_contribution * 100 if total_contribution > 0 else 0
            print(f"     {author}: {count}回成功 / {total_contribution}タスク ({success_rate:.1f}%)")
        ...
        for category, data in contribution_analysis.items():
            if data['total'] > 0:
                ...
                category_name = {'high': '高貢献者', 'medium': '中貢献者', 'low': '低貢献者'}[category]
                print(f"     {category_name}: {data['successes']}/{data['total']} ({success_rate:.2f}%)")
    ...
    if top3_successes:
        ...
        print(f"\n   成功の貢献量分布:")
        print(f"     高貢献者の成功: {high_contrib_successes}/{total_successes} ({high_contrib_successes/total_successes*100:.1f}%)")
        print(f"     中貢献者の成功: {medium_contrib_successes}/{total_successes} ({medium_contrib_successes/total_successes*100:.1f}%)")
        print(f"     低貢献者の成功: {low_contrib_successes}/{total_successes} ({low_contrib_successes/total_successes*100:.1f}%)")
    ...
    if top3_successes:
        ...
        # 成功の貢献量分布（再掲）
        contributions = [s['contribution'] for s in top3_successes]
        high_success_ratio = sum(1 for c in contributions if c >= 50) / len(contributions) * 100
        medium_success_ratio = sum(1 for c in contributions if 10 <= c < 50) / len(contributions) * 100
        low_success_ratio = sum(1 for c in contributions if c < 10) / len(contributions) * 100
