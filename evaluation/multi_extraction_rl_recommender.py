...
class MultiExtractionRLEnvironment(gym.Env):
    ...
    def __init__(self, similarity_recommender, training_data, config):
        ...
        self.method_to_id = {
            method: i for i, method in enumerate(self.extraction_methods)
        }
    ...
    def step(self, action):
        ...
        reward, info = self._calculate_reward(
            current_task, extraction_method, developer_idx
        )
    ...
    def _get_observation(self):
        ...
        # 全特徴量を結合
        obs = np.concatenate(
            [task_features, temporal_features, pool_stats, history_normalized]
        ).astype(np.float32)
    ...
    def _extract_pool_statistics(self, task_data):
        ...
        for method in self.extraction_methods:
            # 各方法での開発者数を取得
            developers = self.recommender.extract_developers_from_task(
            ...
            avg_priority = (
                np.mean([dev["priority"] for dev in developers]) if developers else 0
            )
    ...
    def _calculate_reward(self, task_data, extraction_method, developer_idx):
        ...
        # 実際の正解開発者を取得
        actual_developers = self.recommender.extract_developers_from_task(
        ...
        if extraction_method == "assignees" and selected_dev["priority"] == 1:
            ...
            extraction_method == "creators" and selected_dev["source"] == "user_creator"
        ):
...
class MultiExtractionRLRecommender:
    ...
    def train_base_models(self, data_path):
        ...
        training_pairs, developer_stats = (
            self.similarity_recommender.extract_training_pairs(
        ...
        # 開発者プロファイル構築
        learned_profiles = self.similarity_recommender.build_developer_profiles(
            training_pairs
        )
    ...
    def train_rl_agent(self, training_pairs, total_timesteps=50000):
        ...
        self.rl_agent = PPO(
            ...
            device="auto",
            seed=42,
            policy_kwargs={"net_arch": [256, 256, 128]},  # より深いネットワーク
        )
        ...
        for task_data in test_data:
            ...
            try:
                ...
                temp_env = MultiExtractionRLEnvironment(
                    self.similarity_recommender, [task_data], self.config
                )
    ...
    def evaluate_predictions(self, predictions, test_data):
        ...
        print(
            f"   マルチ抽出RL精度: {accuracy:.3f} ({correct_predictions}/{len(common_tasks)})"
        )
    ...
    def save_results(self, metrics, predictions, prediction_scores, output_dir):
        ...
        for task_id, predicted_dev in predictions.items():
            result = {"task_id": task_id, "predicted_developer": predicted_dev}
...
def main():
    parser = argparse.ArgumentParser(
        description="マルチ抽出方法対応の強化学習推薦システム"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--timesteps", type=int, default=50000, help="強化学習訓練ステップ数"
    )
    ...
    metrics = recommender.run_full_pipeline(
        args.data, total_timesteps=args.timesteps, output_dir=args.output
    )
