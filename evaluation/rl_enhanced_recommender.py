...
class HybridRecommenderEnvironment(gym.Env):
    ...
    def __init__(self, similarity_model, training_data, config):
        ...
        self.action_space = gym.spaces.MultiDiscrete(
            [self.num_developers, 5]  # 開発者選択  # 信頼度レベル (0-4)
        )
    ...
    def _calculate_reward(self, selected_dev, confidence_weight):
        ...
        try:
            ...
            # 予測確率
            proba = self.similarity_model.rf_classifier.predict_proba(features_scaled)[
                0
            ]
...
class RLEnhancedRecommender:
    ...
    def hybrid_predict(self, test_data, alpha=0.7):
        ...
        for task_data in test_data:
            ...
            similarity_predictions[task_id] = {"dev": best_dev, "score": 0.5}
...
def main():
    ...
    parser.add_argument(
        "--similarity_model", required=True, help="学習済み類似度モデルパス"
    )
    parser.add_argument(
        "--train_rl", action="store_true", help="強化学習ポリシーを訓練"
    )
    ...
    training_data = [
        task for task in all_data if task.get("created_at", "").startswith("2022")
    ]
    test_data = [
        task for task in all_data if task.get("created_at", "").startswith("2023")
    ]
