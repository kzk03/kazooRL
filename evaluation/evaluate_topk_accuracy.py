...
def evaluate_topk_accuracy(
    ...
    for i, (task, actual_author) in enumerate(
        tqdm(zip(tasks, ground_truth), desc="Top-K評価中")
    ):
        try:
            # タスク特徴量抽出
    ...
    for k in k_values:
        ...
        available_topk_accuracy = (
            available_topk_matches / available_total if available_total > 0 else 0
        )
        ...
        print(
            f"   Top-{k}利用可能精度: {available_topk_accuracy:.3f} ({available_topk_matches}/{available_total})"
        )
...
"""
    ...
    for k in [1, 3, 5]:
        if f"top_{k}" in topk_results:
            ...
            improvement = (
                available_accuracy / random_expected if random_expected > 0 else 0
            )
...
def main():
    ...
    parser.add_argument(
        "--max-models", type=int, default=50, help="読み込む最大モデル数"
    )
    parser.add_argument(
        "--output-dir",
    ...
    try:
        ...
        trained_models = load_trained_models(
            args.model_dir, ground_truth, args.max_models
        )
        ...
        for k in [1, 3, 5]:
            if f"top_{k}" in topk_results:
                ...
                available_accuracy = result.get("available_accuracy", 0)
                print(
                    f"   - Top-{k}精度: {available_accuracy:.3f} ({available_accuracy*100:.1f}%)"
                )
