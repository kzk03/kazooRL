...
class SimpleSimilarityRecommender:
    ...
    def extract_developers_from_task(self, task_data, method='all'):
        ...
        if method in ['creators', 'all']:
            # 3. Issue/PR作成者（新しい方法）
            if 'user' in task_data and task_data['user'] and 'login' in task_data['user']:
    ...
    def extract_training_pairs(self, training_data, extraction_method='all'):
        ...
        for task_data in training_data:
            ...
            # 複数の方法で開発者を抽出
            developers = self.extract_developers_from_task(task_data, method=extraction_method)
    ...
    def build_developer_activity_timeline(self, training_data, extraction_method='all'):
        ...
        3. all: すべての関与形態を統合
        """
        print(f"📅 開発者アクティビティタイムライン構築中... (方法: {extraction_method})")
        ...
        print(f"   アクティビティタイムライン: {len(self.developer_activity_timeline)} ヶ月")
    ...
    def create_temporal_candidate_pool(self, task_data, learned_profiles):
        ...
        active_developers = self.get_active_developers_for_date(created_at, lookback_months=6)
        
        print(f"   📅 {created_at[:10]} 時点の直近6ヶ月アクティブ開発者: {len(active_developers)} 人")
        ...
        if len(temporal_pool) < 3:
            ...
            for i in range(7, 12):  # 7-11ヶ月前まで拡張（最大1年前）
                extra_devs = self.get_active_developers_for_date(created_at, lookback_months=i)
                for dev_name in extra_devs:
                    if dev_name not in temporal_pool:
                        if dev_name in learned_profiles:
                            temporal_pool[dev_name] = learned_profiles[dev_name]
                            temporal_pool[dev_name]['temporal_active'] = False  # 間接的にアクティブ
                        else:
                            temporal_pool[dev_name] = self._create_default_profile(dev_name)
    ...
    def extract_task_text(self, task_data):
        ...
        if 'labels' in task_data:
            labels = [label.get('name', '') if isinstance(label, dict) else str(label) for label in task_data['labels']]
    ...
    def extract_basic_features(self, task_data):
        ...
        features['is_enhancement'] = 1 if any('enhancement' in label or 'feature' in label for label in labels) else 0
        features['is_documentation'] = 1 if any('doc' in label for label in labels) else 0
        features['is_question'] = 1 if any('question' in label for label in labels) else 0
        features['is_help_wanted'] = 1 if any('help' in label for label in labels) else 0
    ...
    def train_feature_model(self, training_pairs):
        ...
        for pair in training_pairs:
            ...
            feature_vector = [task_features[key] for key in sorted(task_features.keys())]
        ...
        if features:
            ...
            # 特徴重要度
            feature_names = sorted(self.extract_basic_features(training_pairs[0]['task_data']).keys())
            ...
            try:
                ...
                if combined_scores:
                    ...
                    prediction_scores[task_id] = {
                        ...
                        'all_scores': dict(sorted_scores)  # 全開発者のスコア（ソート済み）
                    }
    ...
    def predict_assignments_with_pool(self, test_data, developer_profiles, test_assignments):
        """テストベース候補プールでの割り当て予測"""
        print("🤖 テストベース割り当て予測中...")
    ...
    def predict_assignments_temporal(self, test_data, learned_profiles):
        ...
        for task_data in test_data:
            ...
            try:
                # その時点の動的候補プールを作成
                temporal_pool = self.create_temporal_candidate_pool(task_data, learned_profiles)
                
                # 統計収集
                pool_size = len(temporal_pool)
                temporal_stats['pool_sizes'].append(pool_size)
                temporal_stats['min_pool_size'] = min(temporal_stats['min_pool_size'], pool_size)
                temporal_stats['max_pool_size'] = max(temporal_stats['max_pool_size'], pool_size)
                ...
                # 時系列アクティビティによる重み付け
                combined_scores = self._apply_temporal_weights(combined_scores, temporal_pool, task_data)
                ...
                if combined_scores:
                    ...
                    prediction_scores[task_id] = {
                        ...
                        'pool_size': pool_size,
                        'temporal_active': temporal_pool.get(best_dev, {}).get('temporal_active', False),
                        'created_at': task_data.get('created_at', ''),
        ...
        print(f"   候補プールサイズ範囲: {temporal_stats['min_pool_size']}-{temporal_stats['max_pool_size']}")
    ...
    def _predict_by_text_similarity(self, task_data):
        ...
        if hasattr(self, 'tfidf_matrix') and hasattr(self, 'dev_names'):
            ...
            if task_text.strip():
                ...
                # 類似度計算
                similarities = cosine_similarity(task_vector, self.tfidf_matrix).flatten()
    ...
    def _predict_by_features(self, task_data):
        ...
        for dev_name, profile in self.developer_profiles.items():
            if profile.get('source') == 'training':
                ...
                similarity_score = self._calculate_feature_similarity(task_features, profile)
    ...
    def _calculate_feature_similarity(self, task_features, dev_profile):
        ...
        # タスクの長さ特徴量との類似度
        title_len_diff = abs(task_features['title_length'] - dev_profile.get('avg_title_length', 50))
        body_len_diff = abs(task_features['body_length'] - dev_profile.get('avg_body_length', 200))
        comments_diff = abs(task_features['comments_count'] - dev_profile.get('avg_comments_count', 2))
        ...
        # 重み付き統合スコア
        score = (0.2 * title_sim + 0.3 * body_sim + 0.2 * comments_sim + 0.3 * label_affinity)
    ...
    def _combine_predictions(self, text_scores, feature_scores):
        ...
        for dev in all_devs:
            ...
            combined_scores[dev] = (text_weight * text_score + 
    ...
    def evaluate_predictions(self, predictions, test_assignments, prediction_scores):
        ...
        # Top-K評価 (K=3, 5)
        topk_metrics = self._evaluate_topk_accuracy(common_tasks, test_assignments, prediction_scores)
        ...
        metrics = {
            ...
            'avg_combined_score': np.mean(prediction_confidences) if prediction_confidences else 0.0,
            ...
            'score_std': np.std(prediction_confidences) if prediction_confidences else 0.0
        };
        ...
        print(f"   平均統合スコア: {metrics['avg_combined_score']:.3f} ± {metrics['score_std']:.3f}")
        print(f"   平均テキストスコア: {metrics['avg_text_score']:.3f}")
        print(f"   平均特徴量スコア: {metrics['avg_feature_score']:.3f}")
        
        # 開発者別評価
        dev_metrics = self._evaluate_by_developer(predictions, test_assignments, common_tasks)
        metrics['developer_metrics'] = dev_metrics
        
        return metrics
    
    def _evaluate_topk_accuracy(self, common_tasks, test_assignments, prediction_scores):
        """Top-K accuracy評価"""
        top3_correct = 0
        top5_correct = 0
    ...
    def _evaluate_by_developer(self, predictions, test_assignments, common_tasks):
        ...
        print("   開発者別精度:")
        sorted_devs = sorted(dev_metrics.items(), key=lambda x: x[1]['total'], reverse=True)
        for dev, stats in sorted_devs[:10]:
            print(f"     {dev}: {stats['accuracy']:.3f} ({stats['correct']}/{stats['total']})")
    ...
    def run_full_pipeline(self, data_path, output_dir="outputs", use_temporal=True, extraction_method='all'):
        """
        完全パイプラインの実行
        ...
        # 2. 学習ペア抽出（新しい抽出方法を使用）
        training_pairs, developer_stats = self.extract_training_pairs(training_data, extraction_method)
        ...
        if use_temporal:
            # 時系列ベース動的候補プール
            predictions, prediction_scores, test_assignments, temporal_stats = self.predict_assignments_temporal(
                test_data, learned_profiles
            )
        else:
            # 従来のテストベース候補プール
            test_developers, test_assignments = self.extract_test_developers(test_data)
            self.developer_profiles = self.create_test_based_candidate_pool(learned_profiles, test_developers)
        ...
        # 8. 評価
        metrics = self.evaluate_predictions(predictions, test_assignments, prediction_scores)
        ...
        # 9. 結果保存
        self.save_results(metrics, predictions, prediction_scores, test_assignments, output_dir)
    ...
    def save_results(self, metrics, predictions, prediction_scores, test_assignments, output_dir):
        """結果の保存"""
        output_dir = Path(output_dir)
        output_dir.mkdir(exist_ok=True)
    ...
    def load_existing_profiles(self, profile_path="data/dev_profiles.json"):
        ...
        try:
            ...
            for dev_name, profile in existing_profiles.items():
                # 基本的な統計情報を抽出
                if profile.get('total_contributions', 0) > 0:  # 最低1つは貢献している開発者
                    expanded_profiles[dev_name] = {
                        ...
                        'avg_is_bug': profile.get('label_affinities', {}).get('bug', 0.1),
                        'avg_is_enhancement': profile.get('label_affinities', {}).get('enhancement', 0.1),
                        'avg_is_documentation': profile.get('label_affinities', {}).get('documentation', 0.05),
    ...
    def _generate_fallback_profiles(self):
        ...
        common_names = [
            ...
            "ivan", "julia", "kevin", "lisa", "mike", "nancy", "oscar", "penny",
            "quinn", "robert", "sara", "tom", "ursula", "victor", "wendy", "xavier",
            "yuki", "zoe", "alex", "jordan", "taylor", "casey", "jamie", "riley"
        ]
    ...
    def expand_candidate_pool(self, learned_profiles, existing_profiles, min_activity_threshold=1):
        ...
        for dev_name, profile in existing_profiles.items():
            if dev_name not in expanded_pool and profile.get('task_count', 0) >= min_activity_threshold:
                ...
                expanded_pool[dev_name] = {
                    ...
                    'label_preferences': profile.get('label_preferences', defaultdict(int)),
    ...
    def _create_default_profile(self, dev_name):
        ...
        return {
            ...
            'avg_is_enhancement': 0.1 + ((name_hash + 1) % 3) * 0.05,  # 0.1-0.2の機能親和性
            'avg_is_documentation': 0.05 + ((name_hash + 2) % 3) * 0.025,  # 0.05-0.1のドキュメント親和性
            'source': 'default',
            'priority': 'low'
        }
    
def main():
    parser = argparse.ArgumentParser(description='シンプル類似度ベースのタスク推薦システム')
    ...
    if metrics:
        for key, value in metrics.items():
            if isinstance(value, (int, float)) and key not in ['developer_metrics', 'temporal_stats']:
        ...
        if 'temporal_stats' in metrics:
            ...
            print(f"   候補プールサイズ範囲: {stats.get('min_pool_size', 0)}-{stats.get('max_pool_size', 0)}")
