...
class AdvancedEnsembleSystem:
        ...
        for author, tasks in self.author_task_history.items():
            ...
            for task in tasks:
                ...
                important_keywords = [
                    ...
                    'performance', 'optimize', 'security', 'auth',
                    'api', 'endpoint', 'test', 'spec', 'docker',
                    'compose', 'build', 'deploy', 'config'
                ]
    ...
    def _extract_task_features(self, task: Dict) -> torch.Tensor:
        ...
        extended_keywords = [
            ...
            "docker", "compose", "build", "deploy", "config",
            "test", "spec", "coverage", "ci", "cd"
        ]
        
        for keyword in extended_keywords:
            features.append(1 if keyword in f"{title} {body} {label_text}".lower() else 0)
    ...
    def ultra_advanced_ensemble_recommendation(self, task_features: torch.Tensor, task: Dict, k: int = 5) -> List[Tuple[str, float]]:
    ...
    def meta_ensemble_recommendation(self, task_features: torch.Tensor, task: Dict, k: int = 5) -> List[Tuple[str, float]]:
    ...
    def evaluate_advanced_system(self, method: str = "ultra_advanced", sample_size: int = 500):
        """高度システムの評価"""
            ...
            for task, actual_author in tqdm(zip(eval_tasks, eval_ground_truth), 
                ...
                try:
                    ...
                    if method == "ultra_advanced":
                        recommendations = self.ultra_advanced_ensemble_recommendation(task_features, task, k)
                    elif method == "meta_ensemble":
                        recommendations = self.meta_ensemble_recommendation(task_features, task, k)
                    else:
