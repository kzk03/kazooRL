...
class ImprovedRLTrainer:
    ...
    def _load_data(self):
        ...
        if not os.path.exists(self.config.env.backlog_path):
            raise FileNotFoundError(
                f"Backlog file not found: {self.config.env.backlog_path}"
            )

        if not os.path.exists(self.config.env.dev_profiles_path):
            raise FileNotFoundError(
                f"Dev profiles file not found: {self.config.env.dev_profiles_path}"
            )
        ...
        print(
            f"✅ Loaded {len(self.backlog)} tasks and {len(self.dev_profiles)} developer profiles"
        )
    ...
    def train(self):
        ...
        while global_step < total_timesteps:
            ...
            # 進捗バーの更新
            avg_reward = (
                np.mean(list(episode_rewards.values())) if episode_rewards else 0
            )
            pbar.set_postfix(
                {
                    "Episode": episode,
    ...
    def _collect_rollout(self, initial_obs, rollout_len):
        ...
        for step in range(rollout_len):
            ...
            for agent_id, agent_obs in obs.items():
                action, log_prob, _, value = self.agents[agent_id].get_action_and_value(
                    agent_obs
                )
            ...
            for agent_id in self.env.agent_ids:
                ...
                rollout_data["dones"][agent_id].append(
                    terminateds[agent_id] or truncateds[agent_id]
                )
    ...
    def _prepare_agent_rollout(self, rollout_data, agent_id):
        ...
        observations = torch.stack(
            [torch.FloatTensor(obs) for obs in rollout_data["observations"][agent_id]]
        )
    ...
    def _evaluate_agents(self, global_step):
        ...
        # 評価結果の表示
        avg_rewards = {
            agent_id: np.mean(rewards) for agent_id, rewards in eval_rewards.items()
        }
    ...
    def _update_training_stats(self, episode_rewards, episode_length, update_stats):
        ...
        # エージェント更新統計の平均
        if update_stats:
            avg_policy_loss = np.mean(
                [stats.get("policy_loss", 0) for stats in update_stats.values()]
            )
            avg_value_loss = np.mean(
                [stats.get("value_loss", 0) for stats in update_stats.values()]
            )
            avg_lr = np.mean(
                [stats.get("learning_rate", 0) for stats in update_stats.values()]
            )
    ...
    def _log_training_progress(self, episode, global_step, episode_rewards):
        ...
        print(
            f"Episode {episode:4d} | Step {global_step:6d} | "
...
@hydra.main(
    config_path="../../configs", config_name="improved_rl_training", version_base=None
)
