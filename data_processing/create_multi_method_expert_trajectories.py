...
from evaluation.simple_similarity_recommender import \
    SimpleSimilarityRecommender
...
class MultiMethodExpertTrajectoryGenerator:
        ...
        for method in self.extraction_methods:
            ...
            results[method] = {
                ...
                "unique_developers": len(
                    set([traj["developer"] for traj in expert_trajectories])
                ),
                "total_actions": sum(
                    [len(traj["actions"]) for traj in expert_trajectories]
                ),
            }
        ...
        for developer, actions in developer_trajectories.items():
            if len(actions) >= 2:  # 最低2つのアクションが必要
                ...
                trajectory = {
                    ...
                    / max(1, developer_stats.get(developer, 1)),
                }
    ...
    def _calculate_timespan(self, actions):
        ...
        try:
            first_time = datetime.fromisoformat(
                actions[0]["timestamp"].replace("Z", "+00:00")
            )
            last_time = datetime.fromisoformat(
                actions[-1]["timestamp"].replace("Z", "+00:00")
            )
    ...
    def compare_trajectories(self, results):
        ...
        for method in self.extraction_methods:
            if method in results:
                ...
                print(
                    f"{method:12} | {result['trajectories']:6} | {result['unique_developers']:8} | {result['total_actions']:12} | {avg_actions:15.1f}"
                )
        ...
        for method in self.extraction_methods:
            if method in results:
                ...
                coverage = (
                    ...
                    / max(1, sum(r["total_actions"] for r in results.values()))
                    * 100
                )
    ...
    def load_and_analyze_trajectories(self, method):
        ...
        try:
            ...
            print(
                f"   アクティビティスコア: 平均 {np.mean(activity_scores):.3f}, 標準偏差 {np.std(activity_scores):.3f}"
            )

            # 時間スパン分析
            timespans = [
                traj["timespan"] for traj in trajectories if traj["timespan"] > 0
            ]
            if timespans:
                print(
                    f"   時間スパン: 平均 {np.mean(timespans):.1f} 日, 最大 {max(timespans)} 日"
                )
...
def main():
    ...
    parser.add_argument(
        "--data", default="data/backlog.json", help="バックログデータパス"
    )
    parser.add_argument(
        "--config",
        default="configs/multi_method_training.yaml",
        help="設定ファイルパス",
    )
    parser.add_argument("--analyze", action="store_true", help="既存軌跡の分析のみ実行")
    parser.add_argument(
        "--method", choices=["assignees", "creators", "all"], help="特定の方法のみ処理"
    )
