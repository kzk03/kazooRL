...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('irl_integration.log'),
...
class DataLoader:
    ...
    def load_backlog_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"バックログデータを読み込みました: {len(backlog_data)} 件")
            return backlog_data
    ...
    def prepare_feature_data(self, backlog_data: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        """特徴量エンジニアリング用データの準備"""
        # タスクデータの抽出
        task_data = {
            'task_id': [],
        ...
        for item in backlog_data:
            if 'task_info' in item:
                ...
                task_data['estimated_hours'].append(task_info.get('estimated_hours', 8.0))
                task_data['labels'].append(task_info.get('labels', []))
                task_data['created_at'].append(task_info.get('created_at', datetime.now()))
                task_data['deadline'].append(task_info.get('deadline', datetime.now() + timedelta(days=7)))
            
            if 'developer_info' in item:
                dev_info = item['developer_info']
                developer_data['developer_id'].append(dev_info.get('id', 0))
                developer_data['commits_count'].append(dev_info.get('commits_count', 100))
                developer_data['expertise_languages'].append(dev_info.get('expertise_languages', ['python']))
                developer_data['recent_activity_days'].append(dev_info.get('recent_activity_days', 30))
                developer_data['pr_merge_rate'].append(dev_info.get('pr_merge_rate', 0.8))
                developer_data['code_review_count'].append(dev_info.get('code_review_count', 50))
                developer_data['bug_introduction_rate'].append(dev_info.get('bug_introduction_rate', 0.02))
...
class BackwardCompatibilityManager:
    ...
    def convert_legacy_features(self, legacy_features: np.ndarray, feature_names: List[str]) -> Tuple[np.ndarray, List[str]]:
        """レガシー特徴量を新システム形式に変換"""
    ...
    def convert_legacy_config(self, legacy_config_path: str) -> Dict[str, Any]:
        ...
        try:
            with open(legacy_config_path, 'r', encoding='utf-8') as f:
                if legacy_config_path.endswith('.yaml') or legacy_config_path.endswith('.yml'):
            ...
            new_config = {
                'pipeline': {
                    ...
                    'cache_expiry_hours': legacy_config.get('cache', {}).get('expiry_hours', 24)
                },
                'analysis': {
...
class PerformanceMonitor:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_stage_time': np.mean(list(self.stage_times.values())) if self.stage_times else 0.0
        }
...
class IRLFeatureIntegrationSystem:
    ...
    def load_and_prepare_data(self) -> Tuple[Dict[str, Any], Dict[str, Any], torch.Tensor, torch.Tensor, np.ndarray]:
        """データの読み込みと準備"""
    ...
    def run_feature_engineering(self, task_data: Dict[str, Any], developer_data: Dict[str, Any], 
                              irl_weights: np.ndarray) -> Dict[str, Any]:
        """特徴量エンジニアリングの実行"""
        ...
        try:
            ...
            if os.path.exists(self.config.feature_pipeline_config):
                with open(self.config.feature_pipeline_config, 'r', encoding='utf-8') as f:
                ...
                pipeline_config = {
                    ...
                    'design': {'task_features': {'enabled': True}, 'developer_features': {'enabled': True}},
                    'optimization': {'scaling': {'enabled': True}, 'selection': {'enabled': True}}
                }
            ...
            if not pipeline_result.success:
                raise Exception(f"特徴量パイプライン実行失敗: {pipeline_result.message}")
            
            # 品質監視の実行
            if hasattr(pipeline_result, 'data') and 'final_features' in pipeline_result.data:
            ...
            self.performance_monitor.record_stage_time('feature_engineering', execution_time)
            
            self.logger.info(f"特徴量エンジニアリングが完了しました (実行時間: {execution_time:.2f}秒)")
    ...
    def run_irl_training(self, features: np.ndarray, labels: torch.Tensor, 
                        graph_data: torch.Tensor) -> Dict[str, Any]:
        """IRL訓練の実行"""
        ...
        try:
            ...
            try:
                ...
                # モック結果
                self.logger.warning("既存IRL訓練システムが利用できません。モック結果を使用します。")
                training_result = {
                    'model_path': os.path.join(self.config.model_output_dir, 'irl_model.pt'),
                    'training_loss': [0.5, 0.3, 0.2, 0.15, 0.1],
                    'validation_accuracy': 0.85,
                    'learned_weights': np.random.randn(features.shape[1] if len(features.shape) > 1 else 100)
                }
            
            execution_time = (datetime.now() - stage_start_time).total_seconds()
            self.performance_monitor.record_stage_time('irl_training', execution_time)
            
            self.logger.info(f"IRL訓練が完了しました (実行時間: {execution_time:.2f}秒)")
    ...
    def run_comprehensive_evaluation(self, model_path: str, features: np.ndarray, 
        ...
        try:
            ...
            try:
                ...
                # モック結果
                self.logger.warning("既存評価システムが利用できません。モック結果を使用します。")
                evaluation_results = {
                    'accuracy': 0.85,
                    'precision': 0.83,
                    'recall': 0.87,
            ...
            self.logger.info(f"包括的評価が完了しました (実行時間: {execution_time:.2f}秒)")
    ...
    def _calculate_detailed_metrics(self, features: np.ndarray, labels: torch.Tensor) -> Dict[str, float]:
        """詳細メトリクスの計算"""
        metrics = {}
        ...
        try:
            ...
            if len(features.shape) > 1:
                ...
                metrics['feature_correlation_mean'] = np.mean(np.abs(np.corrcoef(features.T)))
    ...
    def run_ab_testing(self, baseline_features: np.ndarray, enhanced_features: np.ndarray, 
                      labels: torch.Tensor) -> Dict[str, Any]:
        """A/Bテストの実行"""
    ...
    def run_full_integration(self) -> IntegrationResult:
        ...
        try:
            ...
            task_data, developer_data, graph_data, labels, irl_weights = self.load_and_prepare_data()
            result.stages_executed.append("data_loading")
            
            # 2. 特徴量エンジニアリング
            if self.config.enable_feature_pipeline:
                self.logger.info("ステージ2: 特徴量エンジニアリング")
                feature_result = self.run_feature_engineering(task_data, developer_data, irl_weights)
            ...
            if self.config.enable_irl_training:
                ...
                training_result = self.run_irl_training(enhanced_features, labels, graph_data)
            ...
            if self.config.enable_comprehensive_evaluation:
                ...
                evaluation_result = self.run_comprehensive_evaluation(model_path, enhanced_features, labels)
            ...
            if hasattr(self, 'ab_tester') and len(enhanced_features) > 0:
                ...
                ab_result = self.run_ab_testing(baseline_features, enhanced_features, labels)
            ...
            # 特徴量品質指標の計算
            if result.feature_engineering_result and 'quality_metrics' in result.feature_engineering_result:
                result.feature_quality_metrics = result.feature_engineering_result['quality_metrics']
            ...
            result.computational_efficiency = {
                ...
                'stage_balance': np.std(list(performance_summary['stage_execution_times'].values()))
            }
