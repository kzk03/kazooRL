...
def analyze_graph_data():
    ...
    if os.path.exists(training_graph_path):
        try:
            training_graph = torch.load(training_graph_path, map_location='cpu', weights_only=False)
            ...
            for edge_type, edge_data in training_graph.items():
                if 'edge_index' in str(type(edge_data)) or (isinstance(edge_data, dict) and 'edge_index' in edge_data):
    ...
    if os.path.exists(collab_graph_path):
        try:
            collab_graph = torch.load(collab_graph_path, map_location='cpu', weights_only=False)
            print(f"\n✅ 協力ネットワークグラフ読み込み成功")
