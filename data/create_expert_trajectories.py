...
def map_event_to_action(event):
    ...
    if event_type == "issue_assigned":
        return Action.ASSIGN_TASK, {"issue_number": event["number"], "assignee": event.get("actor")}
    if event_type == "pr_opened":
        return Action.SUBMIT_PULL_REQUEST, {"pr_number": event["number"], "author": event.get("actor")}
    if event_type == "pr_review_approved":
        return Action.APPROVE_PULL_REQUEST, {"pr_number": event["number"], "reviewer": event.get("actor")}
    if event_type == "pr_merged":
        return Action.MERGE_PULL_REQUEST, {"pr_number": event["number"], "merger": event.get("actor")}
    return None, None
...
def main():
    ...
    print(f"Expert trajectories saved to {output_path}. Total trajectories: {len(trajectories)}")
