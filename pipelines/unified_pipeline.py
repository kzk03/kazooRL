...
class KazooUnifiedPipeline:
    ...
    def __init__(self):
        ...
        self.log_file = self.project_root / "outputs" / f"pipeline_{self.start_time.strftime('%Y%m%d_%H%M%S')}.log"
    ...
    def train_gat(self):
        ...
        if not self.run_command(
            ...
            self.log("協力関係GATトレーニングに失敗、通常のGNNトレーニングにフォールバック")
            return self.run_command(
                "python training/gat/train_gnn.py",
                "GNNトレーニング",
                self.project_root
            )
    ...
    def train_rl(self):
        ...
        return self.run_command(
            "python training/rl/train_oss.py",
            "RLトレーニング",
            self.project_root
        )
