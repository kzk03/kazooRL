...
try:
    ...
    from analysis.feature_design.task_feature_designer import \
        TaskFeatureDesigner
...
class IRLPipelineIntegrator:
            ...
            for weight_file in expected_weight_files:
                if os.path.exists(f"data/{weight_file}"):
                    ...
                    if weights.shape[0] != self.config.original_feature_count:
                        ...
                        self.logger.info(
                            f"{weight_file}: 検証OK ({weights.shape[0]}次元)"
                        )
    ...
    def _check_feature_count_compatibility(self) -> bool:
        ...
        if self.config.feature_compatibility_mode == "strict":
            return (
                self.config.target_feature_count == self.config.original_feature_count
            )
    ...
    def _build_feature_mappings(self):
        ...
        try:
            ...
            self.feature_mappings = {
                "legacy_features": legacy_feature_names[
            ...
            self.logger.info(
                f"特徴量マッピング構築完了: {len(self.feature_mappings['legacy_features'])}個の既存特徴量"
            )
    ...
    def _extract_legacy_features(
        ...
        try:
            ...
            np.random.seed(42)
            features[:, 3:] = np.random.randn(
                n_samples, self.config.original_feature_count - 3
            )
    ...
    def _integrate_feature_sets(
        self, original_features: np.ndarray, enhanced_features: Optional[np.ndarray]
    ...
    def _gradual_integration(
        self, original_features: np.ndarray, enhanced_features: np.ndarray
    ) -> np.ndarray:
        """段階的統合"""
        # 次元を合わせる
    ...
    def _evaluate_performance(
        ...
        try:
            ...
            if legacy_weights is not None:
                # 既存重みでの予測値比較
                original_pred = np.dot(
                    original_features, legacy_weights[: original_features.shape[1]]
                )
    ...
    def _adjust_weights_for_integration(
        self, legacy_weights: np.ndarray, target_dim: int
    ) -> np.ndarray:
        """統合用の重み調整"""
    ...
    def _analyze_importance_change(
        ...
        try:
            ...
            integrated_importance_normalized = integrated_importance / np.sum(
                integrated_importance
            )

            # 変化量計算
            min_dim = min(
                len(legacy_importance_normalized), len(integrated_importance_normalized)
            )
            importance_change = {}
            ...
            for i in range(min_dim):
                ...
                change = (
                    integrated_importance_normalized[i]
            ...
            if len(integrated_importance_normalized) > min_dim:
                for i in range(min_dim, len(integrated_importance_normalized)):
                    ...
                    importance_change[feature_name] = integrated_importance_normalized[
                        i
                    ]
    ...
    def save_integration_result(
        self, result: IRLIntegrationResult, output_dir: str = "outputs/integration"
    ) -> str:
        """統合結果の保存"""
        try:
            ...
            json_data = {
                ...
                "config_used": (
                    asdict(result.config_used) if result.config_used else None
                ),
            }
            ...
            if result.integrated_features is not None:
                features_path = os.path.join(
                    output_dir, f"{result.integration_id}_features.npy"
                )
            ...
            if result.feature_names:
                names_path = os.path.join(
                    output_dir, f"{result.integration_id}_feature_names.txt"
                )
    ...
    def load_integration_result(self, result_path: str) -> IRLIntegrationResult:
        ...
        try:
            ...
            result = IRLIntegrationResult(
                ...
                backward_compatibility_preserved=json_data.get(
            ...
            if self.config.integration_strategy == "gradual":
                ...
                phases = [
                    ...
                    },
                    {
                        ...
                        "success_criteria": {
                            "accuracy": current_performance.get("accuracy", 0.8) * 1.05
                        },
                    },
                    {
                        "phase": 3,
                        ...
                        "success_criteria": {
                            "accuracy": current_performance.get("accuracy", 0.8) * 1.1
                        },
                    },
                    {
                        "phase": 4,
                ...
                migration_plan["estimated_duration_days"] = sum(
...
class IRLIntegrationUtils:
    ...
    def validate_data_compatibility(
        task_data: Dict[str, Any], developer_data: Dict[str, Any]
    ) -> Dict[str, bool]:
        """データ互換性の検証"""
        validation_results = {}
        ...
        validation_results["task_data_format_valid"] = (
            isinstance(task_data.get("task_id"), list)
            and len(task_data.get("task_id", [])) > 0
        )

        validation_results["developer_data_format_valid"] = (
            isinstance(developer_data.get("developer_id"), list)
            and len(developer_data.get("developer_id", [])) > 0
        )
