...
def analyze_collaboration_detection():
    ...
    for collab_count in sorted(collab_distribution.keys()):
        if collab_count <= 10:  # 0-10人まで表示
            ...
            if collab_count == 11:  # 初回のみ表示
                print(
                    f"  - 11人以上: {sum(collab_distribution[c] for c in collab_distribution if c > 10)}名"
                )
            break
    ...
    for dev, profile in profiles.items():
        collaborators = profile.get("collaborators", [])
        if (
            len(collaborators) >= 2 and examples_shown < 3
        ):  # 2人以上の協力者がいる例を3つ表示
            print(f"\n  開発者: {dev}")
            ...
            print(
                f"    協力者: {', '.join(collaborators[:5])}{'...' if len(collaborators) > 5 else ''}"
            )
            examples_shown += 1
    ...
    print(
        "  - match_collaborated_with_task_author: タスク作成者との過去の協力履歴 (0 or 1)"
    )
    print("  - match_collaborator_overlap_count: タスク担当者との協力履歴重複数")
    print(
        "  - match_has_prior_collaboration: タスク関連開発者との協力履歴の有無 (0 or 1)"
    )
...
def demonstrate_feature_calculation():
    ...
    # サンプル開発者とタスクの例
    sample_dev_profile = {"collaborators": ["alice", "bob", "charlie", "dave"]}

    # ケース1: タスク作成者との協力履歴あり
    task1_author = "alice"
    collaborated_with_author = (
        1.0 if task1_author in sample_dev_profile["collaborators"] else 0.0
    )
    ...
    task2_author = "eve"
    collaborated_with_author = (
        1.0 if task2_author in sample_dev_profile["collaborators"] else 0.0
    )
    ...
    print(
        f"  協力履歴のある担当者: {list(assignee_set.intersection(collaborator_set))}"
    )
    ...
    has_prior_collaboration = (
        1.0 if len(task_related_devs.intersection(collaborator_set)) > 0 else 0.0
    )
    print(f"\\nケース4: タスク関連開発者との総合判定")
    print(f"  タスク関連開発者: {list(task_related_devs)}")
    print(
        f"  協力履歴のある人: {list(task_related_devs.intersection(collaborator_set))}"
    )
