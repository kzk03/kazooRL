...
def debug_active_developers():
    ...
    for i, task in enumerate(tasks_with_assignees[:5]):
        ...
        for activity_days in [30, 90, 180]:
                ...
                if activity_start <= other_task_date <= activity_end:
                    ...
                    if other_task.get("assignees"):
                        for assignee in other_task["assignees"]:
                            if (
                                assignee.get("login")
                                and assignee["login"] in dev_profiles
                            ):
                    ...
                    if (
                        ...
                        and author["login"] in dev_profiles
                    ):
            ...
            actual_in_active = [
                assignee
                for assignee in actual_assignees
                if assignee in active_developers
            ]
    ...
    monthly_stats = defaultdict(
        lambda: {"tasks": 0, "developers": set(), "assignees": set()}
    )
    ...
    for month in sorted(monthly_stats.keys())[-12:]:  # 直近12ヶ月
        ...
        print(
            f"  {month}: {stats['tasks']:3d}タスク, {len(stats['assignees']):2d}担当者, {len(stats['developers']):2d}作成者, {total_devs:2d}総開発者"
        )
        if stats["assignees"]:
    ...
    for assignee in sorted(actual_assignees_all):
        if assignee in assignee_activity:
            ...
            print(
                f"  {assignee:15s}: {dates[0].date()} ～ {dates[-1].date()} ({len(dates)}タスク)"
            )
        else:
