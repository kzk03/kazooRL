...
class FeatureQualityMonitor:
    ...
    def __init__(
        ...
        self.output_dir = (
            Path(output_dir) if output_dir else Path("./quality_monitoring")
        )
    ...
    def start_monitoring(
        ...
        try:
            ...
            logger.info(
                f"品質監視開始: {len(self.baseline_metrics)} メトリクス設定完了"
            )
            return self
    ...
    def monitor_current_quality(
        ...
        if not self.is_monitoring:
            raise ValueError(
                "監視が開始されていません。start_monitoring()を先に実行してください。"
            )
        ...
        try:
            ...
            monitoring_result = {
                ...
                "recommendations": self._generate_recommendations(
                    current_metrics, quality_changes
                ),
            }
            ...
            logger.info(
                f"品質監視完了: 品質スコア={monitoring_result['overall_quality_score']:.3f}"
            )
        ...
        for historical_entry in recent_metrics:
            ...
            if "stability" in hist_metrics:
                ...
                stability_score = 1.0 - min(
                    1.0, np.mean(np.abs(current_std - np.mean(current_std)))
                )
    ...
    def _calculate_redundancy(
        ...
        try:
            ...
            # 高い相関（>0.8）の割合を冗長性とする
            high_correlation_ratio = np.sum(np.abs(correlations) > 0.8) / len(
                correlations
            )
    ...
    def _calculate_correlation_quality(
        self,
        features: np.ndarray,
        labels: Optional[np.ndarray] = None,
        feature_names: Optional[List[str]] = None,
    ...
    def _calculate_missing_values(
        ...
        try:
            # NaNやInfの割合を計算
            missing_ratio = (
                np.isnan(features).sum() + np.isinf(features).sum()
            ) / features.size
    ...
    def _calculate_outliers(
        ...
        try:
            ...
            for i in range(features.shape[1]):
                ...
                outliers = (features[:, i] < lower_bound) | (
                    features[:, i] > upper_bound
                )
    ...
    def _determine_alert_severity(
        self, deviation: float, thresholds: Dict[str, float]
    ...
    def _handle_alerts(self, alerts: List[QualityAlert]) -> None:
        ...
        for alert in alerts:
            try:
                # アラートをログに記録
                logger.warning(
                    f"Quality Alert [{alert.severity.value.upper()}]: {alert.message}"
                )
    ...
    def _generate_recommendations(
        self, metrics: Dict[str, QualityMetric], quality_changes: Dict[str, float]
    ) -> List[str]:
        """品質改善の推奨事項を生成"""
        recommendations = []
        ...
        for metric_name, metric in metrics.items():
            if metric.value < metric.threshold:
                low_quality_metrics.append(
                    (metric_name, metric.value, metric.threshold)
                )
    ...
    def _save_baseline_metrics(self) -> None:
        ...
        try:
            baseline_data = {
                ...
                "metrics": {
                    name: asdict(metric)
                    for name, metric in self.baseline_metrics.items()
                },
            }
    ...
    def generate_quality_report(
        self, include_visualizations: bool = True, report_format: str = "html"
    ) -> str:
        """品質レポートを生成
    ...
    def _generate_html_report(
        self, report_file: Path, include_visualizations: bool
    ) -> None:
        """HTMLレポートを生成"""
    ...
    def _build_html_report_content(self, include_visualizations: bool) -> str:
        ...
        # 最新のメトリクス
        latest_metrics = (
            self.historical_metrics[-1]["metrics"] if self.historical_metrics else {}
        )
            ...
            "summary": {
                ...
                "latest_quality_score": (
                    self._calculate_overall_quality_score(
                        self.historical_metrics[-1]["metrics"]
                    )
    ...
    def _generate_text_report(self, report_file: Path) -> None:
        ...
        if self.historical_metrics:
            ...
            content.extend(["現在の品質メトリクス:", "-" * 30])

            for metric_name, metric in latest_metrics.items():
                status = "OK" if metric.value >= metric.threshold else "NG"
                content.append(
                    f"{metric_name}: {metric.value:.3f} (閾値: {metric.threshold:.3f}) [{status}]"
                )
    ...
    def stop_monitoring(self) -> Dict[str, Any]:
        ...
        final_report_path = self.generate_quality_report(
            include_visualizations=True, report_format="html"
        )
