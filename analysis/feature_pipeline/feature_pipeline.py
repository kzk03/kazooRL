...
from feature_optimization import DimensionReducer, FeatureScaler, FeatureSelector
from gat_optimization import GATIntegratedOptimizer, GATInterpreter, GATOptimizer
...
class FeaturePipeline:
    ...
    def __init__(self, config_path: Optional[str] = None, cache_dir: Optional[str] = None):
        ...
        self.cache_dir = Path(cache_dir) if cache_dir else Path('./feature_pipeline_cache')
        ...
        logger.info(f"FeaturePipeline初期化完了: config={config_path}, cache={self.cache_dir}")
    ...
    def initialize_components(self) -> 'FeaturePipeline':
        ...
        try:
            ...
            if self.config.get('analysis', {}).get('correlation_analysis'):
                self.components['analyzers']['correlation'] = FeatureCorrelationAnalyzer()
            
            if self.config.get('analysis', {}).get('distribution_analysis'):
                self.components['analyzers']['distribution'] = FeatureDistributionAnalyzer()
            ...
            if self.config.get('gat_enhancement', {}).get('integration'):
                self.components['gat_components']['integrated_optimizer'] = GATIntegratedOptimizer()
    ...
    def run_full_pipeline(self, data: Dict[str, Any], 
    ...
    def _execute_analysis_stage(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'correlation' in self.components['analyzers']:
            ...
            correlation_result = correlation_analyzer.analyze_feature_correlations(features)
        ...
        if 'distribution' in self.components['analyzers']:
            ...
            distribution_result = distribution_analyzer.analyze_feature_distributions(features)
            result['analysis_results']['distribution'] = distribution_result
        
        result['output_data'] = {'analysis_completed': True}
        result['metrics'] = {
            'analyzed_features': features.shape[1] if hasattr(features, 'shape') else len(features),
    ...
    def _execute_design_stage(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 特徴量組み合わせの生成
        combinations = self.config.get('design', {}).get('feature_combinations', ['basic'])
    ...
    def _execute_optimization_stage(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'scaler' in self.components['optimizers']:
            ...
            scaling_methods = self.config.get('optimization', {}).get('scaling_methods', ['standard'])
        ...
        if 'selector' in self.components['optimizers']:
            ...
            selection_methods = self.config.get('optimization', {}).get('selection_methods', ['univariate'])
            ...
            for method in selection_methods:
                try:
                    ...
                    selection_results[method] = {
                        'success': True,
                        'selected_features': min(50, features.shape[1] if hasattr(features, 'shape') else 50),
                        'selection_ratio': 0.6
                    }
                except Exception as e:
                    selection_results[method] = {
        ...
        if 'reducer' in self.components['optimizers']:
            ...
            reduction_methods = self.config.get('optimization', {}).get('reduction_methods', ['pca'])
    ...
    def _execute_gat_enhancement_stage(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'integrated_optimizer' in self.components['gat_components']:
            integrated_optimizer = self.components['gat_components']['integrated_optimizer']
    ...
    def _generate_performance_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'evaluation' in results.get('stage_results', {}):
            ...
            perf_comparison = eval_results.get('evaluation_results', {}).get('performance_comparison', {})
            summary['final_performance_score'] = perf_comparison.get('gat_enhanced_performance')
        
        return summary
    
    def _generate_pipeline_report(self, data: Dict[str, Any], evaluation_result: Dict[str, Any]) -> str:
        """パイプライン実行レポートを生成"""
            ...
            with open(report_path, 'w', encoding='utf-8') as f:
                ...
                if hasattr(self, 'execution_results'):
                    for pipeline_id, result in self.execution_results.items():
                        ...
                        f.write(f"性能サマリ: {result.get('performance_summary', {})}\n\n")
                
                # 評価結果
                f.write("【評価結果詳細】\n")
                for key, value in evaluation_result.get('evaluation_results', {}).items():
                    f.write(f"{key}: {value}\n")
    ...
    def clear_cache(self, older_than_hours: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"キャッシュクリア完了: {len(cleared_files)}/{total_files} ファイル削除")
            return result
