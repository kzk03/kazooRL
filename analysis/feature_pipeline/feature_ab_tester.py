...
class FeatureABTester:
    ...
    def register_feature_set(self, 
                           ...
                           description: Optional[str] = None) -> 'FeatureABTester':
        """特徴量セットを登録
    ...
    def create_ab_test(self, config: ABTestConfig) -> str:
        ...
        if config.baseline_features not in self.feature_sets:
            raise ValueError(f"ベースライン特徴量セットが見つかりません: {config.baseline_features}")
        
        if config.treatment_features not in self.feature_sets:
            raise ValueError(f"改善版特徴量セットが見つかりません: {config.treatment_features}")
        
        # サンプルサイズの検証
        baseline_size = self.feature_sets[config.baseline_features]['features'].shape[0]
        treatment_size = self.feature_sets[config.treatment_features]['features'].shape[0]
        
        if baseline_size < config.minimum_sample_size or treatment_size < config.minimum_sample_size:
            logger.warning(f"サンプルサイズが不足しています: "
    ...
    def run_ab_test(self, test_id: str, models: Optional[Dict[str, Any]] = None) -> ABTestResult:
        """A/Bテストを実行
        ...
        try:
            ...
            # 効果サイズを計算
            effect_sizes = self._calculate_effect_sizes(baseline_metrics, treatment_metrics)
            ...
            logger.info(f"A/Bテスト実行完了: {test_id} (有意性: {test_result.is_significant})")
            return test_result
    ...
    def _evaluate_unsupervised_metrics(self, 
                                     features: np.ndarray,
    ...
    def _calculate_precision(self, 
        ...
        """適合率を計算"""
        model = models.get('random_forest', self.default_models['random_forest'])
        scores = cross_val_score(model, features, labels, cv=5, scoring='precision_macro')
        return np.mean(scores)
    ...
    def _calculate_roc_auc(self, 
        ...
        try:
            model = models.get('random_forest', self.default_models['random_forest'])
            scores = cross_val_score(model, features, labels, cv=5, scoring='roc_auc_ovr')
            return np.mean(scores)
        except:
            # マルチクラス問題で失敗した場合
            return 0.5
    
    def _calculate_cross_val_accuracy(self, 
                                    features: np.ndarray,
                                    labels: np.ndarray,
                                    models: Dict[str, Any]) -> float:
        """クロスバリデーション精度を計算"""
        model = models.get('logistic_regression', self.default_models['logistic_regression'])
        scores = cross_val_score(model, features, labels, cv=10, scoring='accuracy')
        return np.mean(scores)
    
    def _calculate_feature_importance_sum(self, 
                                        features: np.ndarray,
                                        labels: np.ndarray,
                                        models: Dict[str, Any]) -> float:
        """特徴量重要度の合計を計算"""
    ...
    def _calculate_prediction_time(self, 
                                 features: np.ndarray,
                                 labels: np.ndarray,
                                 models: Dict[str, Any]) -> float:
        """推論時間を計算"""
    ...
    def _calculate_effect_sizes(self, 
        ...
        for metric_name in baseline_metrics:
            if metric_name in treatment_metrics:
                ...
                # Cohen's d の簡略版
                if baseline_value != 0:
                    effect_size = (treatment_value - baseline_value) / abs(baseline_value)
                else:
    ...
    def _calculate_confidence_intervals(self, 
                                      baseline_metrics: Dict[str, float],
                                      treatment_metrics: Dict[str, float],
                                      significance_level: float) -> Dict[str, Tuple[float, float]]:
        """信頼区間を計算"""
        confidence_intervals = {}
    ...
    def _generate_recommendation(self, 
        ...
        if test_result.is_significant:
            ...
            if improvements:
                ...
                recommendations.append(f"改善が認められた指標: {', '.join(improvements)}")
            else:
                recommendations.append("統計的有意性は認められましたが、実用的な効果サイズには達していません。")
        else:
            # 有意な差が認められない場合
            recommendations.append(
        ...
        # 追加の推奨事項
        max_effect_metric = max(test_result.effect_sizes.items(), key=lambda x: abs(x[1]))
        recommendations.append(
    ...
    def _perform_detailed_analysis(self, 
                                 ...
                                 test_result: ABTestResult) -> Dict[str, Any]:
        """詳細分析を実行"""
        analysis = {}
        ...
        if baseline_features.shape[1] > 1 and treatment_features.shape[1] > 1:
            ...
            analysis['correlation_analysis'] = {
                'baseline_mean_correlation': np.mean(np.abs(baseline_corr[baseline_mask])),
                'treatment_mean_correlation': np.mean(np.abs(treatment_corr[treatment_mask]))
            }
    ...
    def _determine_best_performing_group(self, test_result: ABTestResult) -> str:
        """最高性能グループを判定"""
        positive_effects = sum(1 for effect in test_result.effect_sizes.values() if effect > 0)
    ...
    def _find_consistent_improvements(self, test_result: ABTestResult) -> List[str]:
        ...
        for metric_name, effect_size in test_result.effect_sizes.items():
            if effect_size > 0.05:  # 5%以上の改善
                statistical_result = test_result.statistical_results.get(metric_name, {})
    ...
    def _save_test_result(self, test_result: ABTestResult) -> None:
        ...
        try:
            ...
            result_dict['test_config']['test_type'] = test_result.test_config.test_type.value
    ...
    def generate_comparison_report(self, 
        ...
        report_file = self.output_dir / f"ab_test_report_{test_id}_{timestamp}.{report_format}"
        
        try:
            if report_format == 'html':
                self._generate_html_comparison_report(test_result, report_file, include_visualizations)
    ...
    def _generate_html_comparison_report(self, 
                                       test_result: ABTestResult,
                                       report_file: Path,
                                       include_visualizations: bool) -> None:
        """HTML比較レポートを生成"""
        html_content = self._build_html_comparison_content(test_result, include_visualizations)
        
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write(html_content)
    
    def _build_html_comparison_content(self, 
                                     test_result: ABTestResult,
        ...
        if test_result.detailed_analysis:
            ...
            # 特徴量数比較
            feature_comparison = test_result.detailed_analysis.get('feature_count_comparison', {})
            ...
            # パフォーマンス要約
            performance_summary = test_result.detailed_analysis.get('performance_summary', {})
    ...
    def _generate_json_comparison_report(self, 
                                       test_result: ABTestResult,
        ...
        report_data = {
            ...
            'summary': {
                ...
                'best_performing_group': test_result.detailed_analysis.get('performance_summary', {}).get('best_performing_group'),
                'total_metrics_improved': sum(1 for effect in test_result.effect_sizes.values() if effect > 0),
                'total_metrics_degraded': sum(1 for effect in test_result.effect_sizes.values() if effect < 0),
                'largest_improvement': max(test_result.effect_sizes.values()) if test_result.effect_sizes else 0,
                'largest_degradation': min(test_result.effect_sizes.values()) if test_result.effect_sizes else 0
            },
        ...
        report_data['test_result']['test_config']['test_type'] = test_result.test_config.test_type.value
    ...
    def _generate_comparison_charts(self, test_result: ABTestResult) -> Dict[str, str]:
        ...
        try:
            ...
            ax1.bar(x - width/2, baseline_values, width, label='ベースライン', alpha=0.8)
            ...
            chart_path = self.output_dir / f"comparison_chart_{test_result.test_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    ...
    def clean_up_old_tests(self, days_old: int = 30) -> Dict[str, Any]:
        ...
        for test_id, test_result in self.completed_tests.items():
            test_end_time = datetime.fromisoformat(test_result.end_time) if test_result.end_time else datetime.now()
