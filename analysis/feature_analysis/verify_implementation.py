...
def main():
    ...
    feature_names = [f"task_feature_{i}" if i < 3 else f"dev_feature_{i}" if i < 6 else f"match_feature_{i}" for i in range(n_features)]
    ...
    try:
        ...
        print(f"✅ 重要度分析完了")
        print(f"   - 重要度ランキング: {len(importance_results['importance_ranking'])}項目")
        print(f"   - カテゴリ別分析: {len(importance_results['category_importance'])}カテゴリ")
        
        # TOP3を表示
        print("   重要度TOP3:")
        for i, (name, weight, importance) in enumerate(importance_results['importance_ranking'][:3], 1):
            print(f"     {i}. {name:20s} | 重み:{weight:6.3f} | 重要度:{importance:6.3f}")
        ...
        correlation_results = correlation_analyzer.analyze_correlations(correlation_threshold=0.3)
        print(f"✅ 相関分析完了")
        print(f"   - 相関行列: {correlation_results['correlation_matrix'].shape}")
        print(f"   - 高相関ペア: {len(correlation_results['high_correlation_pairs'])}ペア")
        print(f"   - 冗長特徴量候補: {len(correlation_results['redundant_features'])}個")
        ...
        print(f"   - 外れ値検出: {len(distribution_results['outlier_detection'])}特徴量")
