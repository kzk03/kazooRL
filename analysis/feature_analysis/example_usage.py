...
def load_real_irl_data():
    ...
    # IRL重みファイルを探す
    weights_files = ["learned_weights_bot_excluded.npy", "learned_weights_test.npy"]
...
def demonstrate_feature_analysis():
    ...
    try:
        ...
        for i, (name, weight, importance) in enumerate(
            importance_results["importance_ranking"][:10], 1
        ):
            print(
                f"  {i:2d}. {name:35s} | 重み:{weight:8.5f} | 重要度:{importance:8.5f}"
            )

        print("\n📋 カテゴリ別重要度:")
        for category, stats in importance_results["category_importance"].items():
            print(
                f"  {category:20s}: 平均重要度 {stats['mean_importance']:.5f} ({stats['count']}個)"
            )
        ...
        if correlation_results["high_correlation_pairs"]:
            print("\n🔍 高相関ペア TOP5:")
            for i, (feat1, feat2, corr) in enumerate(
                correlation_results["high_correlation_pairs"][:5], 1
            ):
        ...
        if correlation_results["redundant_features"]:
            ...
            for item in correlation_results["redundant_features"][:3]:
                print(
                    f"   削除候補: {item['redundant_feature'][:30]:30s} (相関: {item['correlation']:.3f})"
                )
        ...
        print(
            f"   - 深刻なスケール不均衡: {'はい' if scale_info['scale_imbalance_severe'] else 'いいえ'}"
        )
        ...
        if correlation_results["redundant_features"]:
            print(
                f"   1. {len(correlation_results['redundant_features'])}個の冗長特徴量候補の削除を検討"
            )
        ...
        # 分布の改善
        extreme_skew_count = sum(
            1
            for stats in distribution_results["distribution_stats"].values()
