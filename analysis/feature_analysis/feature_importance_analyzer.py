...
class FeatureImportanceAnalyzer:
    ...
    def __init__(self, irl_weights_path: str, feature_names: List[str]):
        ...
        logger.info(
            f"FeatureImportanceAnalyzer初期化完了: {len(self.feature_names)}特徴量"
        )
        ...
        try:
            if not self.irl_weights_path.exists():
                raise FileNotFoundError(
                    f"重みファイルが見つかりません: {self.irl_weights_path}"
                )
    ...
    def _analyze_by_category(self) -> Dict[str, Dict[str, float]]:
        ...
        for category, indices in self.feature_categories.items():
            ...
            stats_dict = {
                ...
                "weight_range": [
                    float(np.min(category_weights)),
                    float(np.max(category_weights)),
                ],
            }
    ...
    def _compare_basic_vs_gat(self) -> Dict[str, Any]:
        ...
        try:
            u_stat, u_p = stats.mannwhitneyu(
                basic_importance, gat_importance, alternative="two-sided"
            )
    ...
    def _get_top_features_in_indices(
        self, indices: List[int], top_k: int
    ) -> List[Dict[str, Any]]:
    ...
    def generate_importance_report(self, output_path: Optional[str] = None) -> str:
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"重み範囲: [{np.min(self.weights):.6f}, {np.max(self.weights):.6f}]\n"
            )
            ...
            for i, (name, weight, importance) in enumerate(ranking[:20], 1):
                f.write(
                    f"{i:2d}. {name:40s} | 重み:{weight:8.5f} | 重要度:{importance:8.5f}\n"
                )
            f.write("\n")
            ...
            if "error" not in comparison:
                ...
                if (
                    "statistical_test" in comparison
                    and "error" not in comparison["statistical_test"]
                ):
                    test = comparison["statistical_test"]
                    f.write(f"\n統計的比較 ({test['test']}):\n")
                    f.write(f"  p値: {test['p_value']:.6f}\n")
                    f.write(
                        f"  有意差: {'あり' if test['significant_difference'] else 'なし'}\n"
                    )
    ...
    def visualize_importance(
        self, output_dir: Optional[str] = None, show_plot: bool = True
    ) -> str:
        """重要度分析の可視化
        ...
        # 1. 重み分布ヒストグラム
        axes[0, 0].hist(
            self.weights, bins=30, alpha=0.7, color="skyblue", edgecolor="black"
        )
        ...
        names = [
            item[0][:20] + "..." if len(item[0]) > 20 else item[0] for item in ranking
        ]
        ...
        axes[0, 2].bar(
            range(len(categories)), mean_importance, alpha=0.7, color="lightcoral"
        )
        ...
        if category_weights_data:
            ...
            axes[1, 1].set_xticklabels(
                category_labels, rotation=45, ha="right", fontsize=8
            )
