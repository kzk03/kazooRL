...
from kazoo.analysis.feature_analysis import (FeatureCorrelationAnalyzer,
...
def test_feature_importance_analyzer():
    ...
    try:
        ...
        for i, (name, weight, importance) in enumerate(
            results["importance_ranking"][:5], 1
        ):
...
def test_feature_correlation_analyzer():
    ...
    if results["high_correlation_pairs"]:
        ...
        for i, (feat1, feat2, corr) in enumerate(
            results["high_correlation_pairs"][:5], 1
        ):
...
def test_feature_distribution_analyzer():
    ...
    # 可視化テスト
    fig_path = analyzer.visualize_distributions(
        "outputs", show_plot=False, max_features=12
    )
    print(f"✅ 可視化完了: {fig_path}")
