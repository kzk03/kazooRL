...
class FeatureCorrelationAnalyzer:
    ...
    def analyze_correlations(self, correlation_threshold: float = 0.8) -> Dict[str, Any]:
        """相関分析を実行
        ...
        results = {
            ...
            'high_correlation_pairs': self._find_high_correlations(correlation_threshold),
            'redundant_features': self._identify_redundant_features(correlation_threshold),
    ...
    def _identify_redundant_features(self, threshold: float = 0.8) -> List[Dict[str, Any]]:
        """冗長特徴量特定
        ...
        for feat1, feat2, corr in high_corr_pairs:
            ...
            redundant_candidates.append({
                ...
                'correlation': corr,
                'redundant_corr_count': feature_corr_count.get(redundant_feature, 0),
    ...
    def generate_correlation_report(self, output_path: Optional[str] = None, 
                                  correlation_threshold: float = 0.8) -> str:
        """相関分析レポートを生成
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"相関ペア総数: {len(self.feature_names) * (len(self.feature_names) - 1) // 2}\n\n")
    ...
    def visualize_correlations(self, output_dir: Optional[str] = None, 
                             show_plot: bool = True, max_features: int = 50) -> str:
        """相関分析の可視化
        ...
        axes[1, 0].hist(abs_correlations, bins=50, alpha=0.7, color='lightcoral', edgecolor='black')
        ...
        if category_corrs:
            ...
            axes[1, 1].bar(range(len(categories)), mean_corrs, alpha=0.7, color='lightgreen')
            axes[1, 1].set_xticks(range(len(categories)))
            axes[1, 1].set_xticklabels([cat[:15] for cat in categories], rotation=45, ha='right', fontsize=8)
        ...
        axes[1, 2].bar(stats_labels, stats_values, alpha=0.7, color=['red', 'orange', 'blue'])
    ...
    def get_feature_correlations(self, feature_name: str, top_k: int = 10) -> List[Tuple[str, float]]:
        """指定特徴量との相関が高い特徴量を取得
