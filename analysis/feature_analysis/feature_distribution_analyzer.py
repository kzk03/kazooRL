...
class FeatureDistributionAnalyzer:
        ...
        for i, name in enumerate(self.feature_names):
            ...
            try:
                ...
                if len(clean_values) <= 5000:
                    ...
                    # サンプル数が多い場合はサブサンプリング
                    sample_indices = np.random.choice(len(clean_values), 5000, replace=False)
                    shapiro_stat, shapiro_p = stats.shapiro(clean_values[sample_indices])
                
                # Kolmogorov-Smirnov検定
                ks_stat, ks_p = stats.kstest(clean_values, 'norm', 
    ...
    def _detect_outliers(self) -> Dict[str, Dict[str, Any]]:
        ...
        for i, name in enumerate(self.feature_names):
            ...
            iqr_outlier_mask = (clean_values < lower_bound) | (clean_values > upper_bound)
    ...
    def _detect_scale_imbalance(self) -> Dict[str, float]:
        ...
        # スケール不均衡の指標を計算
        scale_stats = {
            'std_ratio_max_min': float(np.max(feature_stds) / np.min(feature_stds[feature_stds > 0])) if np.any(feature_stds > 0) else 1.0,
            'range_ratio_max_min': float(np.max(feature_ranges) / np.min(feature_ranges[feature_ranges > 0])) if np.any(feature_ranges > 0) else 1.0,
            'mean_ratio_max_min': float(np.max(feature_means) / np.min(feature_means[feature_means > 0])) if np.any(feature_means > 0) else 1.0,
            'std_cv': float(np.std(feature_stds) / np.mean(feature_stds)) if np.mean(feature_stds) > 0 else 0.0,
            'range_cv': float(np.std(feature_ranges) / np.mean(feature_ranges)) if np.mean(feature_ranges) > 0 else 0.0,
            'features_with_zero_std': int(np.sum(feature_stds == 0)),
            'features_with_large_std': int(np.sum(feature_stds > np.mean(feature_stds) + 3 * np.std(feature_stds))),
            'scale_imbalance_severe': float(np.max(feature_stds) / np.min(feature_stds[feature_stds > 0])) > 1000 if np.any(feature_stds > 0) else False
        }
        ...
        for category, indices in self.feature_categories.items():
            ...
            if means:
                category_stats[category] = {
                    ...
                    'scale_ratio': float(np.max(stds) / np.min(stds)) if np.min(stds) > 0 else np.inf
                }
    ...
    def generate_distribution_report(self, output_path: Optional[str] = None) -> str:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write("【スケール不均衡分析】\n")
            f.write(f"標準偏差比（最大/最小）: {scale_imbalance['std_ratio_max_min']:.2f}\n")
            f.write(f"レンジ比（最大/最小）: {scale_imbalance['range_ratio_max_min']:.2f}\n")
            f.write(f"平均値比（最大/最小）: {scale_imbalance['mean_ratio_max_min']:.2f}\n")
            f.write(f"分散ゼロの特徴量数: {scale_imbalance['features_with_zero_std']}\n")
            f.write(f"深刻なスケール不均衡: {'はい' if scale_imbalance['scale_imbalance_severe'] else 'いいえ'}\n\n")
            ...
            normal_count_shapiro = sum(1 for test in normality_tests.values() if test.get('is_normal_shapiro', False))
            normal_count_ks = sum(1 for test in normality_tests.values() if test.get('is_normal_ks', False))
            
            f.write(f"Shapiro-Wilk検定で正規分布: {normal_count_shapiro}/{len(normality_tests)}\n")
            f.write(f"Kolmogorov-Smirnov検定で正規分布: {normal_count_ks}/{len(normality_tests)}\n")
            
            # 外れ値サマリー
            f.write("\n【外れ値検出サマリー】\n")
            outlier_detection = analysis_results['outlier_detection']
            high_outlier_features = [name for name, result in outlier_detection.items() 
            ...
            # 歪度の絶対値でソート
            sorted_features = sorted(distribution_stats.items(), 
                                   key=lambda x: abs(x[1]['skewness']) if not np.isnan(x[1]['skewness']) else 0, 
                                   reverse=True)
            
            f.write("歪度が大きい特徴量 TOP10:\n")
            for i, (name, stats) in enumerate(sorted_features[:10], 1):
                f.write(f"{i:2d}. {name:30s} | 歪度:{stats['skewness']:7.3f} | 尖度:{stats['kurtosis']:7.3f} | CV:{stats['cv']:7.3f}\n")
        
        logger.info(f"分布分析レポート生成完了: {output_path}")
        return str(output_path)
    
    def visualize_distributions(self, output_dir: Optional[str] = None, 
                              show_plot: bool = True, max_features: int = 20) -> str:
        """分布分析の可視化
        ...
        # 1. 歪度分布
        skewnesses = [stats['skewness'] for stats in distribution_stats.values() 
                     if not np.isnan(stats['skewness'])]
        axes[0, 0].hist(skewnesses, bins=30, alpha=0.7, color='skyblue', edgecolor='black')
        ...
        # 2. 尖度分布
        kurtoses = [stats['kurtosis'] for stats in distribution_stats.values() 
                   if not np.isnan(stats['kurtosis'])]
        axes[0, 1].hist(kurtoses, bins=30, alpha=0.7, color='lightcoral', edgecolor='black')
        ...
        # 3. 変動係数分布
        cvs = [stats['cv'] for stats in distribution_stats.values() 
               if not np.isnan(stats['cv']) and not np.isinf(stats['cv'])]
        if cvs:
            # 極端な値をクリップ
            cvs_clipped = np.clip(cvs, 0, np.percentile(cvs, 95))
            axes[0, 2].hist(cvs_clipped, bins=30, alpha=0.7, color='lightgreen', edgecolor='black')
        ...
        # 4. ゼロ比率分布
        zero_ratios = [stats['zero_ratio'] for stats in distribution_stats.values() 
                      if not np.isnan(stats['zero_ratio'])]
        axes[1, 0].hist(zero_ratios, bins=30, alpha=0.7, color='orange', edgecolor='black')
        ...
        if category_distributions:
            ...
            mean_skewnesses = [category_distributions[cat]['mean_skewness'] for cat in categories]
            
            axes[1, 1].bar(range(len(categories)), mean_skewnesses, alpha=0.7, color='purple')
            axes[1, 1].set_xticks(range(len(categories)))
            axes[1, 1].set_xticklabels([cat[:10] for cat in categories], rotation=45, ha='right', fontsize=8)
        ...
        if max_features > 0:
            ...
            fig2.suptitle(f'個別特徴量分布 (歪度上位{n_show}個)', fontsize=16, fontweight='bold')
            ...
            for i, (name, stats) in enumerate(sorted_features[:n_show]):
                ...
                if len(clean_values) > 0:
                    axes2[row, col].hist(clean_values, bins=30, alpha=0.7, edgecolor='black')
                    axes2[row, col].set_title(f'{name[:20]}\n歪度:{stats["skewness"]:.2f}', fontsize=10)
                    axes2[row, col].grid(True, alpha=0.3)
                else:
                    axes2[row, col].text(0.5, 0.5, 'No Data', ha='center', va='center', 
