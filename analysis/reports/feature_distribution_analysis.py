    ...
    try:
        ...
        if config_path.exists():
            ...
            cfg = OmegaConf.create({
                ...
                'irl': {
                    'use_gat': True  # GATを有効化して正確な特徴量名取得
                }
            })
        ...
        if len(feature_names) != n_features:
            print(f"⚠️  次元数調整: 特徴量名{len(feature_names)}次元 → IRL重み{n_features}次元")
            if len(feature_names) > n_features:
        ...
        feature_names = [
            ...
            'task_label_bug', 'task_label_enhancement', 'task_label_documentation',
            'task_label_question', 'task_label_help wanted',
            'task_priority_score', 'task_urgency_indicator', 'task_complexity_estimate',
            'task_comment_count', 'task_participant_count', 'task_file_count',
            'task_line_count', 'task_branch_age', 'task_commit_frequency',
            'task_test_coverage', 'task_documentation_quality',
            
            # 開発者特徴量 (15次元)
            'dev_recent_activity_count', 'dev_current_workload', 'dev_total_lines_changed',
            'dev_collaboration_network_size', 'dev_comment_interactions', 'dev_cross_issue_activity',
            'dev_expertise_score', 'dev_reputation', 'dev_response_time',
            'dev_code_quality', 'dev_test_coverage', 'dev_documentation_score',
            'dev_leadership_score', 'dev_mentoring_activity', 'dev_innovation_index',
            ...
            # GAT統計特徴量 (5次元)
            'gat_similarity', 'gat_dev_expertise', 'gat_task_popularity',
            'gat_collaboration_strength', 'gat_network_centrality',
...
def generate_realistic_feature_data(feature_names, n_samples=1000):
    ...
    print(f"📊 現実的な特徴量データを生成中... ({n_samples}サンプル, {len(feature_names)}次元)")
...
def analyze_feature_distributions(features, feature_names, weights):
    ...
    print(f"正の重み: {np.sum(weights > 0)}個 ({np.sum(weights > 0)/len(weights)*100:.1f}%)")
    print(f"負の重み: {np.sum(weights < 0)}個 ({np.sum(weights < 0)/len(weights)*100:.1f}%)")
    print(f"ゼロの重み: {np.sum(weights == 0)}個 ({np.sum(weights == 0)/len(weights)*100:.1f}%)")
    
    # 基本特徴量とGAT特徴量の分類
    # 実際の特徴量名（25次元）+ feature_XX（37次元）の場合、feature_XXをGAT特徴量として扱う
    basic_feature_names = [name for name in feature_names if not (name.startswith('gat_') or 'gat_' in name or name.startswith('feature_'))]
    gat_feature_names = [name for name in feature_names if (name.startswith('gat_') or 'gat_' in name or name.startswith('feature_'))]
    ...
    categories = {
        ...
        'マッチング特徴量': [name for name in feature_names if name.startswith('match_')],
        'GAT統計特徴量': [name for name in feature_names if name.startswith('gat_') and 'emb_' not in name],
        'GAT埋め込み': [name for name in feature_names if ('gat_dev_emb_' in name or 'gat_emb_' in name or name.startswith('feature_'))]
    }
    ...
    for major_category, names in major_categories.items():
        ...
        # 大分類内の特徴量インデックスを取得
        category_indices = [feature_names.index(name) for name in names if name in feature_names]
        category_weights = weights[category_indices] if len(category_indices) > 0 else []
        
        if len(category_weights) > 0:
            print(f"   重み統計:")
            print(f"     - 平均: {np.mean(category_weights):.6f}")
            print(f"     - 標準偏差: {np.std(category_weights):.6f}")
            print(f"     - 範囲: [{np.min(category_weights):.6f}, {np.max(category_weights):.6f}]")
            print(f"     - 正の重み: {np.sum(np.array(category_weights) > 0)}個 ({np.sum(np.array(category_weights) > 0)/len(category_weights)*100:.1f}%)")
            print(f"     - 負の重み: {np.sum(np.array(category_weights) < 0)}個 ({np.sum(np.array(category_weights) < 0)/len(category_weights)*100:.1f}%)")
            ...
            if major_cat_data:
                ...
                for idx, (_, row) in enumerate(top_major.iterrows(), 1):
                    print(f"     {idx}. {row['feature'][:45]:45s} | 重み:{row['irl_weight']:8.5f}")
    ...
    for category, names in categories.items():
        ...
        if len(category_weights) > 0:
            print(f"   重み統計 - 平均: {np.mean(category_weights):.6f}, "
            ...
            if name in feature_names:
                ...
                if feature_idx < len(weights):
                    ...
                    stats = {
                        ...
                        'outliers_pct': np.mean(np.abs(values - np.mean(values)) > 3 * np.std(values)) * 100,
    ...
    if stats_report:
        ...
        for idx, (_, row) in enumerate(top_features.iterrows(), 1):
            feature_display = format_feature_with_japanese(row['feature'], japanese_names)
            print(f"  {idx:2d}. {feature_display[:55]:55s} | "
                  f"重み:{row['irl_weight']:8.5f} | "
                  f"重要度:{row['importance']:8.5f} | "
                  f"カテゴリ:{row['category']}")
        
        # 基本特徴量内でのランキング
        basic_stats = stats_df[~(stats_df['feature'].str.contains('gat_') | stats_df['feature'].str.startswith('feature_'))]
        ...
        # GAT特徴量内でのランキング
        gat_stats = stats_df[stats_df['feature'].str.contains('gat_') | stats_df['feature'].str.startswith('feature_')]
...
def create_visualizations(features, feature_names, weights, stats_df):
    ...
    if not stats_df.empty:
        plt.subplot(2, 3, 2)
        category_weights = stats_df.groupby('category')['irl_weight'].agg(['mean', 'std'])
        category_weights.plot(kind='bar', y='mean', yerr='std', ax=plt.gca(), 
    ...
    if not stats_df.empty:
        ...
        plt.barh(range(len(top_features)), top_features['irl_weight'], color=colors, alpha=0.7)
        plt.yticks(range(len(top_features)), 
                  [name[:20] + '...' if len(name) > 20 else name for name in top_features['feature']])
        plt.xlabel('IRL重み')
    ...
    plt.boxplot(sample_features, labels=[name[:10] for name in feature_names[:min(5, len(feature_names))]])
    ...
    plt.subplot(2, 3, 6)
    if not stats_df.empty:
        scatter = plt.scatter(stats_df['irl_weight'], stats_df['importance'], 
                            c=stats_df.index, cmap='viridis', alpha=0.7)
        plt.xlabel('IRL重み')
        plt.ylabel('重要度（絶対値）')
        plt.title('重み vs 重要度')
        plt.grid(True, alpha=0.3)
    ...
    if not stats_df.empty:
        ...
        for i, category in enumerate(categories):
            ...
            plt.hist(cat_data['irl_weight'], bins=10, alpha=0.7, 
            ...
            plt.barh(range(len(top_cat)), top_cat['irl_weight'], 
                    color=colors, alpha=0.7)
            plt.yticks(range(len(top_cat)), 
                      [name[:15] for name in top_cat['feature']])
...
def save_detailed_report(stats_df, output_dir):
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        if not stats_df.empty:
            # 大分類別サマリー
            f.write("【大分類別サマリー】\n")
            basic_features = stats_df[~(stats_df['feature'].str.contains('gat_') | stats_df['feature'].str.startswith('feature_'))]
            gat_features = stats_df[stats_df['feature'].str.contains('gat_') | stats_df['feature'].str.startswith('feature_')]
            ...
            if not basic_features.empty:
                ...
                f.write(f"  重み範囲: [{basic_features['irl_weight'].min():.6f}, {basic_features['irl_weight'].max():.6f}]\n")
            ...
            if not gat_features.empty:
                ...
                f.write(f"  重み範囲: [{gat_features['irl_weight'].min():.6f}, {gat_features['irl_weight'].max():.6f}]\n")
                f.write(f"  正の重み: {(gat_features['irl_weight'] > 0).sum()}個\n")
                f.write(f"  負の重み: {(gat_features['irl_weight'] < 0).sum()}個\n")
            f.write("\n")
            
            f.write("【詳細カテゴリ別サマリー】\n")
            category_summary = stats_df.groupby('category').agg({
            ...
            if not basic_features.empty:
                ...
                for idx, (_, row) in enumerate(top_basic.iterrows(), 1):
                    feature_display = format_feature_with_japanese(row['feature'], japanese_names)
                    f.write(f"{idx:2d}. {feature_display[:50]:50s} | "
            ...
            for category in ['タスク特徴量', '開発者特徴量', 'マッチング特徴量']:
                ...
                if not cat_features.empty:
                    f.write(f"\n{category}:\n")
                    top_cat = cat_features.nlargest(min(10, len(cat_features)), 'importance')
                    for idx, (_, row) in enumerate(top_cat.iterrows(), 1):
                        feature_display = format_feature_with_japanese(row['feature'], japanese_names)
                        f.write(f"  {idx:2d}. {feature_display[:45]:45s} | "
                               f"重み:{row['irl_weight']:8.4f} | "
                               f"重要度:{row['importance']:8.4f}\n")
            ...
            for _, row in top_features.iterrows():
                feature_display = format_feature_with_japanese(row['feature'], japanese_names)
                f.write(f"{feature_display[:50]:50s} | "
                       f"重み:{row['irl_weight']:8.4f} | "
                       f"重要度:{row['importance']:8.4f} | "
                       f"カテゴリ:{row['category']}\n")
            
            f.write("\n【負の重み特徴量】\n")
            negative_features = stats_df[stats_df['irl_weight'] < 0].sort_values('irl_weight')
...
def main():
    ...
    if len(weights) != len(feature_names):
        print(f"⚠️  次元数不一致: 重み{len(weights)}次元 vs 特徴量{len(feature_names)}次元")
    ...
    if not stats_df.empty:
        ...
        feature_display = format_feature_with_japanese(top_feature['feature'], japanese_names)
        print(f"   - 最重要特徴量: {feature_display} (重み: {top_feature['irl_weight']:.4f})")
        ...
        # カテゴリ別重要度
        cat_importance = stats_df.groupby('category')['importance'].mean().sort_values(ascending=False)
        print(f"   - 最重要カテゴリ: {cat_importance.index[0]} (平均重要度: {cat_importance.iloc[0]:.4f})")
        ...
        if not basic_features.empty:
            ...
            top_basic_feature = basic_features.loc[basic_features['importance'].idxmax()]
            top_basic_display = format_feature_with_japanese(top_basic_feature['feature'], japanese_names)
            print(f"     - 最重要: {top_basic_display} (重み: {top_basic_feature['irl_weight']:.4f})")
            ...
            for idx, (_, row) in enumerate(top_basic_3.iterrows(), 1):
                feature_display = format_feature_with_japanese(row['feature'], japanese_names)
                print(f"       {idx}. {feature_display[:40]:40s} (重み: {row['irl_weight']:7.4f})")
        ...
        if not gat_features.empty:
            ...
            top_gat_display = format_feature_with_japanese(top_gat_feature['feature'], japanese_names)
            print(f"     - 最重要: {top_gat_display} (重み: {top_gat_feature['irl_weight']:.4f})")
