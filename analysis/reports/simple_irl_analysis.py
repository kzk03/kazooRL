...
def create_simple_interpretation():
    ...
    if len(gat_weights) > 0:
        ...
        print(f"  → 協力関係の方が {np.mean(np.abs(gat_weights))/np.mean(np.abs(base_weights)):.1f}倍重要！")
    
    # 基本特徴量の重要なものを分析
    print(f"\n✅ 重要視される開発者の特徴 (基本情報):")
    positive_base = [(i, base_weights[i], base_features[i]) for i in range(len(base_weights)) if base_weights[i] > 0.5]
    ...
    print(f"\n❌ 避けられる開発者の特徴 (基本情報):")
    negative_base = [(i, base_weights[i], base_features[i]) for i in range(len(base_weights)) if base_weights[i] < -0.5]
...
def create_simple_visualization(weights, base_features, base_weights, gat_weights):
    ...
    ax1.set_yticklabels([name[:12] + "..." if len(name) > 12 else name for name in base_features], fontsize=10)
    ax1.set_xlabel('Importance Weight', fontsize=12)
    ax1.set_title('Basic Features Importance\n(Blue: Important, Red: Avoided, Gray: Neutral)', fontsize=14, pad=20)
    ...
    for i, (bar, weight) in enumerate(zip(bars, base_weights)):
        if abs(weight) > 0.3:  # 重要度が高いもののみ表示
            ax1.text(weight + (0.05 if weight > 0 else -0.05), i, f'{weight:.2f}', 
    ...
    ax3.hist(base_weights, bins=15, alpha=0.7, color='blue', edgecolor='black', label='Basic Features')
    if len(gat_weights) > 0:
        ax3.hist(gat_weights, bins=15, alpha=0.7, color='red', edgecolor='black', label='GAT Features')
    ...
    for i, (bar, weight) in enumerate(zip(bars, top_weights)):
        ax4.text(weight + (0.05 if weight > 0 else -0.05), i, f'{weight:.2f}', 
                va='center', ha='left' if weight > 0 else 'right', fontsize=9, fontweight='bold')
    
    # 5. GAT特徴量の詳細
    if len(gat_weights) > 0:
        ax5 = plt.subplot(2, 3, 6)
        gat_colors = ['red' if w < 0 else 'blue' if w > 0.5 else 'gray' for w in gat_weights]
        ...
        for idx in top_gat:
            ax5.bar(idx, gat_weights[idx], color='gold', alpha=0.9, edgecolor='black', linewidth=2)
    
    plt.tight_layout(pad=3.0)
    
    # 保存
    output_path = Path("outputs") / f"simple_irl_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
...
def print_practical_advice(base_weights, base_features, gat_weights):
    ...
    print("🎯 開発者選択で最も重要なのは:")
    if len(gat_weights) > 0 and np.mean(np.abs(gat_weights)) > np.mean(np.abs(base_weights)):
    ...
    # 基本情報の重要ポイント
    important_base = [(i, base_weights[i], base_features[i]) for i in range(len(base_weights)) if abs(base_weights[i]) > 0.5]
...
def create_summary_report():
    ...
    stats = {
        ...
        "GAT特徴量平均重要度": np.mean(np.abs(gat_weights)) if len(gat_weights) > 0 else 0,
        "最大重み": np.max(weights),
        "最小重み": np.min(weights)
    }
