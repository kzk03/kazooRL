...
def analyze_irl_weights():
    ...
    feature_names.extend(
        ["match_skill_intersection_count", "match_file_experience_count"]
    )
    ...
    for i, name in enumerate(feature_names):
        if any(
            ...
            for keyword in [
                ...
                "cross_issue_activity",
                "collaborated_with_task_author",
                "collaborator_overlap_count",
...
def analyze_weights_by_category(weights, feature_names):
    ...
    for i, name in enumerate(feature_names):
        ...
        elif name.startswith("dev_") and not any(
            keyword in name
            for keyword in [
                "collaboration_network_size",
                "comment_interactions",
    ...
    for category, cat_weights in categories.items():
        if cat_weights:
            ...
            print(
                f"   {category:<12}: 平均重み={mean_weight:.6f}, 最大重み={max_weight:.6f}, 個数={len(cat_weights)}"
            )
...
def create_weight_visualization(weights, feature_names, collaboration_indices):
    ...
    # 協力関係特徴量をハイライト
    colors = [
        "red" if i in collaboration_indices else "blue" for i in range(len(weights))
    ]
    ...
    plt.ylabel("重み")
    plt.xticks(
        range(len(collab_names)),
        [name.replace("match_", "").replace("dev_", "") for name in collab_names],
        rotation=45,
        ha="right",
    )
    plt.axhline(y=0, color="black", linestyle="-", alpha=0.3)
    ...
    for i, (bar, weight) in enumerate(zip(bars, collab_weights)):
        plt.text(
            ...
            bar.get_height() + 0.001 * np.sign(weight),
            f"{weight:.4f}",
            ha="center",
            va="bottom" if weight >= 0 else "top",
            fontsize=9,
        )
...
def analyze_collaboration_impact():
    ...
    for dev_name in sample_devs:
        ...
        print(
            f"    協力者: {', '.join(list(collaborators)[:3])}{'...' if len(collaborators) > 3 else ''}"
        )
        ...
        for task_idx, task in enumerate(sample_tasks):
            ...
            collaborated_with_author = (
                1.0 if task_author and task_author in collaborators else 0.0
            )
            collaborator_overlap_count = len(
                assignee_logins.intersection(collaborators)
            )
            ...
            has_prior_collaboration = (
                1.0 if len(task_related_devs.intersection(collaborators)) > 0 else 0.0
            )
