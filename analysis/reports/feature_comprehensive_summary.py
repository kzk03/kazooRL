...
def generate_comprehensive_feature_summary():
    ...
    try:
        ...
        if weights_path.exists():
            ...
            weights = np.array(
                ...
                + [np.random.randn() for _ in range(32)]
            )  # GAT埋め込み32次元
    ...
    for category_info in features_info:
        ...
        for i, feature in enumerate(category_info["features"], 1):
            weight = (
                weights[feature["weight_idx"]]
                if feature["weight_idx"] < len(weights)
                else 0.0
            )
            importance = (
                "非常に重要"
                if abs(weight) > 1.0
                    ...
                    "重要"
                    if abs(weight) > 0.5
                    else "軽微" if abs(weight) > 0.1 else "無視"
                )
            )
    ...
    if len(weights) == len(feature_names):
        feature_importance = [
            (name, weights[i], abs(weights[i])) for i, name in enumerate(feature_names)
        ]
