...
def analyze_irl_weights_detailed():
    ...
    if len(weights) != len(feature_info):
        print(f"⚠️ 重み数({len(weights)})と特徴量定義数({len(feature_info)})が一致しません")
        # 重みの数に合わせて調整
        if len(weights) < len(feature_info):
    ...
    print(f"\n📊 重みの符号分析:")
    print(f"  - 正の重み: {len(positive_weights)}個 (平均: {positive_weights.mean():.4f})")
    print(f"  - 負の重み: {len(negative_weights)}個 (平均: {negative_weights.mean():.4f})")
    ...
    negative_sorted = negative_indices[np.argsort(weights[negative_indices])]  # 昇順（最も負の値）
...
def create_detailed_visualization(weights, feature_info):
    ...
    ax1.set_yticklabels([name[:15] + "..." if len(name) > 15 else name for name in top_20_names], fontsize=8)
    ...
    ax2.bar(x_pos, means, yerr=stds, capsize=5, alpha=0.7, color=['skyblue', 'lightcoral'][:len(categories)])
    ...
    ax3.axvline(weights.mean(), color='red', linestyle='--', label=f'Mean: {weights.mean():.3f}')
    ...
    # 保存
    output_path = Path("outputs") / f"irl_detailed_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
