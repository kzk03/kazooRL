...
def generate_summary_report():
    ...
    if any(
        ...
        if feature_names[list(base_weights).index(w)] == "プロフィール文の有無"
    ):
    ...
    if any(
        w > 0.5
        for w in base_weights
        if "フォロー" in feature_names[list(base_weights).index(w)]
    ):
    ...
    if any(
        w < -0.5
        for w in base_weights
        if "年間リポジトリ" in feature_names[list(base_weights).index(w)]
    ):
        print("4. 質を重視する")
        print("   • リポジトリの数より質を重視")
...
def save_report_to_file(weights, base_weights, gat_weights, feature_names):
    ...
    report_path = (
        output_dir
        / f"irl_summary_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    )
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        base_sorted = sorted(
            enumerate(base_weights), key=lambda x: abs(x[1]), reverse=True
        )
