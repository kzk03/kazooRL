...
def visualize_gat_analysis(gat_model, graph_data, irl_weights):
    ...
    if gat_model and isinstance(gat_model, dict):
        ...
        first_linear = None
        for name, param in gat_model.items():
            if 'lin' in name.lower() and 'weight' in name and isinstance(param, torch.Tensor):
    ...
    # 保存
    output_path = Path("outputs") / f"gat_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
