...
def analyze_gat_features():
    ...
    gat_stat_weights = weights[
        gat_start_idx : gat_start_idx + len(gat_statistical_features)
    ]
    gat_emb_weights = weights[
        gat_start_idx + len(gat_statistical_features) : gat_start_idx + num_gat_features
    ]
    ...
    for i, (feature_name, weight) in enumerate(
        zip(gat_statistical_features, gat_stat_weights), 1
    ):
    ...
    if len(gat_stat_weights) > 0:
        ...
        print(
            f"   最大: {np.max(gat_stat_weights):8.4f} ({gat_statistical_features[np.argmax(gat_stat_weights)]})"
        )
        print(
            f"   最小: {np.min(gat_stat_weights):8.4f} ({gat_statistical_features[np.argmin(gat_stat_weights)]})"
        )
    ...
    if len(gat_emb_weights) > 0:
        ...
        print(
            f"   最大: {np.max(gat_emb_weights):8.4f} ({gat_embedding_features[np.argmax(gat_emb_weights)]})"
        )
        print(
            f"   最小: {np.min(gat_emb_weights):8.4f} ({gat_embedding_features[np.argmin(gat_emb_weights)]})"
        )
        ...
        emb_importance = [
            ...
            for i, (name, weight) in enumerate(
                zip(gat_embedding_features, gat_emb_weights)
            )
        ]
        ...
        for rank, (idx, name, abs_weight) in enumerate(emb_importance[:10], 1):
            ...
            print(
                f"{rank:2d}. {name:20s} | 重み: {actual_weight:8.4f} | 埋め込み次元{dimension}"
            )
        ...
        print(f"\n📊 重みの符号分布:")
        print(
            f"   正の重み: {len(positive_weights):2d}個 (平均: {np.mean(positive_weights) if len(positive_weights) > 0 else 0:6.4f})"
        )
        print(
            f"   負の重み: {len(negative_weights):2d}個 (平均: {np.mean(negative_weights) if len(negative_weights) > 0 else 0:6.4f})"
        )
...
"""
    ...
    if len(gat_stat_weights) > 0:
        ...
        stat_csv_path = (
            project_root / "outputs" / f"gat_statistical_features_{timestamp}.csv"
        )
    ...
    if len(gat_emb_weights) > 0:
        ...
        # 重要度でランキング
        emb_df = emb_df.sort_values("abs_weight", ascending=False).reset_index(
            drop=True
        )
        emb_df["rank_by_importance"] = range(1, len(emb_df) + 1)

        emb_csv_path = (
            project_root / "outputs" / f"gat_embedding_features_{timestamp}.csv"
        )
    ...
    if summary_data:
        ...
        summary_csv_path = (
            project_root / "outputs" / f"gat_features_summary_{timestamp}.csv"
        )
