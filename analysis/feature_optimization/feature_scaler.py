...
class FeatureScaler:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # スケーリング手法の設定
        self.scaling_methods = self.config.get('scaling_methods', {
        ...
        # 特徴量タイプの自動判定閾値
        self.auto_detection_thresholds = self.config.get('auto_detection_thresholds', {
        ...
        # 時系列特徴量のパターン
        self.temporal_patterns = self.config.get('temporal_patterns', [
            'time', 'date', 'hour', 'day', 'week', 'month', 'year',
            'created', 'updated', 'since', 'age', 'duration'
        ])
    ...
    def fit(self, X: Union[np.ndarray, pd.DataFrame], 
            ...
            feature_types: Optional[Dict[str, str]] = None) -> 'FeatureScaler':
        """スケーラーを学習データに適合
        ...
        if len(feature_names) != n_features:
            raise ValueError(f"特徴量名の数({len(feature_names)})とデータの次元数({n_features})が一致しません")
        ...
        # 各特徴量に対するスケーリング戦略の決定
        self.scaling_strategies = self._determine_scaling_strategies(X_array, feature_names)
    ...
    def transform(self, X: Union[np.ndarray, pd.DataFrame]) -> np.ndarray:
        ...
        if not self.is_fitted:
            raise ValueError("スケーラーが学習されていません。先にfit()を呼び出してください。")
        ...
        if X_array.shape[1] != len(self.feature_names):
            raise ValueError(f"入力データの次元数({X_array.shape[1]})が学習時({len(self.feature_names)})と異なります")
    ...
    def fit_transform(self, X: Union[np.ndarray, pd.DataFrame], 
                     feature_names: Optional[List[str]] = None,
                     feature_types: Optional[Dict[str, str]] = None) -> np.ndarray:
        """学習と変換を同時実行
    ...
    def _detect_feature_types(self, X: np.ndarray, feature_names: List[str]) -> Dict[str, str]:
        """特徴量タイプの自動判定
        ...
        for i, name in enumerate(feature_names):
            ...
            # カテゴリカル特徴量の判定
            elif unique_ratio <= self.auto_detection_thresholds['categorical_unique_ratio']:
                feature_types[name] = 'categorical'
            # 数値特徴量の詳細判定
            else:
                # 歪度の計算
                if len(clean_values) > 3:
                    ...
                    iqr = q3 - q1
                    outliers = clean_values[(clean_values < q1 - 1.5 * iqr) | 
                    ...
                    elif outlier_ratio > self.auto_detection_thresholds['outlier_ratio_threshold']:
    ...
    def _determine_scaling_strategies(self, X: np.ndarray, feature_names: List[str]) -> Dict[str, str]:
        """各特徴量のスケーリング戦略を決定
        ...
        for i, name in enumerate(self.feature_names):
            ...
            if strategy == 'onehot':
                ...
                if strategy == 'none':
                    ...
                    if params is not None:
                        min_val, max_val = params['min'], params['max']
                        original_values = scaled_values.flatten() * (max_val - min_val) + min_val
                        inverse_features.append(original_values)
                    else:
                        inverse_features.append(scaled_values.flatten())
    ...
    def generate_scaling_report(self, output_path: Optional[str] = None) -> str:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"次元変化: {scaling_info['n_features_out'] - scaling_info['n_features_in']:+d}\n\n")
            ...
            # OneHotエンコーディングの詳細
            onehot_features = [name for name, strategy in scaling_info['scaling_strategies'].items() 
                             if strategy == 'onehot']
            if onehot_features:
                f.write(f"\n【OneHotエンコーディング詳細】\n")
                for name in onehot_features:
                    encoder = self.encoders[name]
                    categories = encoder.categories_[0]
                    f.write(f"{name}: {len(categories)}カテゴリ → {len(categories)}特徴量\n")
