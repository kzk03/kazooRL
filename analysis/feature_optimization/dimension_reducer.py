...
class DimensionReducer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # 次元削減手法の設定
        self.reduction_methods = self.config.get(
            "reduction_methods",
            {
                "pca": {
        ...
        # 最適次元数決定の設定
        self.auto_dimension_config = self.config.get(
            "auto_dimension_config",
            {
                "min_components": 2,
    ...
    def transform(
        self, X: Union[np.ndarray, pd.DataFrame], method: str = "pca"
    ) -> np.ndarray:
        """次元削減を適用
        ...
        if not self.is_fitted:
            raise ValueError(
                "次元削減器が学習されていません。先にfit()を呼び出してください。"
            )
        ...
        logger.debug(
            f"次元削減変換完了: {X_array.shape[1]} → {X_reduced.shape[1]} ({method})"
        )
        return X_reduced
    ...
    def fit_transform(
        ...
        Returns:
            次元削減後のデータ
        """
        return self.fit(X, feature_names, methods, scale_data).transform(
            X, transform_method
        )
    ...
    def _fit_pca(self, X: np.ndarray) -> None:
        ...
        # PCAの学習
        pca = PCA(
            n_components=n_components, svd_solver=config["svd_solver"], random_state=42
        )
        pca.fit(X)
    ...
    def _fit_truncated_svd(self, X: np.ndarray) -> None:
        ...
        if config["n_components"] == "auto":
            ...
            n_components = min(
                int(
                    max_components * self.auto_dimension_config["max_components_ratio"]
                ),
                50,
            )
        else:
    ...
    def _fit_ica(self, X: np.ndarray) -> None:
        ...
        try:
            ...
            self.reduction_info["ica"] = {
                ...
                "mixing_matrix": (
                    ica.mixing_.tolist() if hasattr(ica, "mixing_") else None
                ),
                "n_iter": ica.n_iter_,
            }
    ...
    def _fit_tsne(self, X: np.ndarray) -> None:
        ...
        if X.shape[0] > 5000:
            logger.warning(
                f"t-SNE: サンプル数が多いため({X.shape[0]})、最初の5000サンプルのみ使用"
            )
            X_sample = X[:5000]
        else:
            X_sample = X
    ...
    def _fit_umap(self, X: np.ndarray) -> None:
        ...
        try:
            ...
            logger.warning(
                "UMAPライブラリが利用できません。pip install umapでインストールしてください。"
            )
        except Exception as e:
    ...
    def _determine_pca_components(self, X: np.ndarray, config: Dict[str, Any]) -> int:
        ...
        if isinstance(config["n_components"], int):
            ...
            max_comp = int(
                X.shape[1] * self.auto_dimension_config["max_components_ratio"]
            )
    ...
    def get_component_interpretation(
        self, method: str = "pca", n_top_features: int = 5
        ...
        for i, component in enumerate(components):
            # 絶対値で重要度をソート
            feature_importance = [
                (self.original_feature_names[j], float(component[j]))
    ...
    def get_optimal_dimensions(
        self, method: str = "pca", variance_threshold: float = 0.95
    ) -> Dict[str, Any]:
        """最適次元数の分析
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            for method, result in summary["reduction_results"].items():
                ...
                try:
                    ...
                    f.write(
                        f"  推奨次元数(95%分散): {optimal_info.get('optimal_dimensions_for_threshold', 'N/A')}\n"
                    )
                    f.write(
                        f"  エルボーポイント: {optimal_info.get('elbow_point_dimensions', 'N/A')}\n"
                    )
                except:
                    pass
            ...
            if "pca" in summary["methods_used"]:
                try:
                    interpretation = self.get_component_interpretation(
