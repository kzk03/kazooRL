...
def generate_test_data(n_samples=1000, n_features=50):
    ...
    for i in range(5):
        categories = np.random.choice([0, 1, 2, 3], n_samples, p=[0.4, 0.3, 0.2, 0.1]).astype(float)
    ...
    y_continuous = (X[:, 0] * 0.5 + X[:, 1] * 0.3 + X[:, 2] * 0.2 + 
...
def main():
    ...
    try:
        ...
        for method, result in selection_summary['selection_results'].items():
            print(f"   - {method}: {result['n_selected']}個選択 ({result['selection_ratio']:.1%})")
        ...
        for method, result in reduction_summary['reduction_results'].items():
            print(f"   - {method}: {result['n_components']}次元 (削減率: {1-result['reduction_ratio']:.1%})")
        ...
        if 'pca' in reduction_summary['methods_used']:
            try:
                interpretation = reducer.get_component_interpretation('pca', n_top_features=3)
                print(f"   - PCA成分解釈:")
                for component, features in list(interpretation.items())[:2]:  # 最初の2成分
        ...
        selection_report = selector.generate_selection_report("test_selection_report.txt")
        reduction_report = reducer.generate_reduction_report("test_reduction_report.txt")
        ...
        X_test_selected_loaded = selector_loaded.transform(X_test_scaled_loaded, method='ensemble')
        X_test_reduced_loaded = reducer_loaded.transform(X_test_selected_loaded, method='pca')
        
        # 結果の一致確認
        scaling_match = np.allclose(X_test_scaled, X_test_scaled_loaded, rtol=1e-10)
        selection_match = np.allclose(X_test_selected, X_test_selected_loaded, rtol=1e-10)
        ...
        # クリーンアップ
        for file in ["test_scaler.pkl", "test_selector.pkl", "test_reducer.pkl",
                    "test_scaling_report.txt", "test_selection_report.txt", "test_reduction_report.txt"]:
            try:
                Path(file).unlink()
            except:
                pass
