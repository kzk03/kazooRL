...
class FeatureSelector:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # 相互作用項生成の設定
        self.interaction_config = self.config.get(
            "interaction_config",
            {
                "degree": 2,  # 多項式の次数
    ...
    def transform(
        self, X: Union[np.ndarray, pd.DataFrame], method: str = "ensemble"
    ) -> np.ndarray:
        """特徴量選択を適用
        ...
        if not self.is_fitted:
            raise ValueError(
                "特徴量選択器が学習されていません。先にfit()を呼び出してください。"
            )
        ...
        if X_array.shape[1] != self.n_features_original:
            raise ValueError(
                f"入力データの次元数({X_array.shape[1]})が学習時({self.n_features_original})と異なります"
            )
        ...
        logger.debug(
            f"特徴量選択変換完了: {X_array.shape[1]} → {X_selected.shape[1]} ({method})"
        )
        return X_selected
    ...
    def fit_transform(
        ...
        Returns:
            選択された特徴量データ
        """
        return self.fit(X, y, feature_names, task_type, methods).transform(
            X, transform_method
        )
    ...
    def _fit_univariate_selection(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        self.selected_features["univariate"] = selector.get_support(
            indices=True
        ).tolist()
        self.feature_scores["univariate"] = selector.scores_.tolist()

        logger.debug(
            f"単変量選択完了: {len(self.selected_features['univariate'])}特徴量選択"
        )
    ...
    def _fit_rfe_selection(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        if config["n_features_to_select"] == "auto":
            n_features = min(
                int(self.n_features_original * 0.3), 30
            )  # 30%または30個の小さい方
        else:
    ...
    def _fit_l1_selection(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        if self.task_type == "classification":
                ...
                for alpha in alphas:
                    ...
                    try:
                        scores = cross_val_score(
                            model, X, y, cv=config["cv"], scoring="accuracy"
                        )
            ...
            if config["alpha"] == "auto":
                model = LassoCV(
                    cv=config["cv"], random_state=42, max_iter=config["max_iter"]
                )
            else:
                model = Lasso(
                    alpha=config["alpha"], random_state=42, max_iter=config["max_iter"]
                )
    ...
    def _fit_importance_selection(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        if config["top_k"] == "auto":
            top_k = min(
                int(self.n_features_original * 0.4), 40
            )  # 40%または40個の小さい方
        else:
            top_k = config["top_k"]
        ...
        selected_indices = np.unique(
            np.concatenate([threshold_selected, top_k_indices])
        ).tolist()
    ...
    def generate_interaction_features(
        ...
        if X_poly.shape[1] > self.interaction_config["max_features"]:
            logger.warning(
                f"相互作用項が多すぎます({X_poly.shape[1]})。最初の{self.interaction_config['max_features']}個のみ使用。"
            )
            X_poly = X_poly[:, : self.interaction_config["max_features"]]
            poly_feature_names = poly_feature_names[
                : self.interaction_config["max_features"]
            ]
    ...
    def get_feature_ranking(self, method: str = "ensemble") -> List[Tuple[str, float]]:
        ...
        # ランキング作成
        ranking = [
            (self.feature_names[i], float(scores[i])) for i in range(len(scores))
        ]
    ...
    def get_selection_summary(self) -> Dict[str, Any]:
        ...
        for method, selected_indices in self.selected_features.items():
            summary["selection_results"][method] = {
                ...
                "selected_features": [
                    self.feature_names[i] for i in selected_indices[:10]
                ],  # 最初の10個
            }
    ...
    def generate_selection_report(self, output_path: Optional[str] = None) -> str:
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            if len(summary["methods_used"]) > 1:
                ...
                f.write(
                    f"選択率: {len(ensemble_features) / summary['n_features_original']:.1%}\n"
                )
