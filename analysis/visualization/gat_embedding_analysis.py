...
def visualize_embeddings_2d(dev_embeddings, task_embeddings, output_dir="outputs"):
    ...
    task_indices = sample_indices[sample_indices >= len(dev_embeddings)] - len(dev_embeddings)
    
    plt.subplot(1, 2, 2)
    if len(dev_indices) > 0:
        dev_tsne_indices = np.where(sample_indices < len(dev_embeddings))[0]
        plt.scatter(tsne_result[dev_tsne_indices, 0], tsne_result[dev_tsne_indices, 1], 
                   alpha=0.6, label='開発者', s=20)
    
    if len(task_indices) > 0:
        task_tsne_indices = np.where(sample_indices >= len(dev_embeddings))[0]
        plt.scatter(tsne_result[task_tsne_indices, 0], tsne_result[task_tsne_indices, 1], 
...
def create_interpretability_report(dev_embeddings, task_embeddings):
    ...
    # 統計的要約
    print("📊 統計的要約:")
    print(f"  - 開発者埋め込み: {dev_embeddings.shape[0]}個 × {dev_embeddings.shape[1]}次元")
    print(f"  - タスク埋め込み: {task_embeddings.shape[0]}個 × {task_embeddings.shape[1]}次元")
