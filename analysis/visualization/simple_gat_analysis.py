...
def analyze_embedding_statistics(embeddings, name="GAT埋め込み"):
    ...
    print(
        f"平均値の範囲: [{np.mean(embeddings, axis=0).min():.3f}, {np.mean(embeddings, axis=0).max():.3f}]"
    )
    print(
        f"標準偏差の範囲: [{np.std(embeddings, axis=0).min():.3f}, {np.std(embeddings, axis=0).max():.3f}]"
    )
...
def create_simple_visualization(dev_embeddings, task_embeddings, output_dir="outputs"):
    ...
    plt.subplot(2, 3, 5)
    plt.scatter(
        dev_embeddings[:, sorted_dims[0]],
        dev_embeddings[:, sorted_dims[1]],
        alpha=0.6,
        label="開発者",
        s=20,
    )
    plt.scatter(
        task_embeddings[:, sorted_dims[0]],
        task_embeddings[:, sorted_dims[1]],
        alpha=0.6,
        label="タスク",
        s=20,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        f"{output_dir}/gat_embedding_simple_analysis.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
