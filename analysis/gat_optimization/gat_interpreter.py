...
class GATInterpreter:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # 解釈設定
        self.interpretation_config = self.config.get('interpretation_config', {
    ...
    def load_data(self, embeddings: Dict[str, torch.Tensor], 
                  basic_features: Optional[Dict[str, np.ndarray]] = None,
                  graph_data: Optional[Any] = None) -> 'GATInterpreter':
        """GAT埋め込みと関連データを読み込み
    ...
    def interpret_dimensions(self, node_type: str = 'dev', 
                           basic_feature_names: Optional[List[str]] = None) -> Dict[str, Any]:
        """各GAT次元の意味解釈
        ...
        if not self.is_fitted or node_type not in self.embeddings:
            raise ValueError(f"データが読み込まれていないか、ノードタイプ '{node_type}' が見つかりません")
        ...
        if node_type in self.basic_features and basic_feature_names:
            results['correlation_analysis'] = self._analyze_correlations_with_basic_features(
    ...
    def _interpret_single_dimension(self, dim_values: np.ndarray, 
                                   dim_idx: int, node_type: str) -> Dict[str, Any]:
        """単一次元の解釈"""
        interpretation = {
            ...
            'semantic_category': self._infer_semantic_category(dim_values, dim_idx, node_type)
        }
    ...
    def _find_extreme_values(self, values: np.ndarray, 
                           percentile: float = 95) -> Dict[str, Any]:
        """極値の特定"""
    ...
    def _infer_semantic_category(self, values: np.ndarray, 
                               dim_idx: int, node_type: str) -> str:
        """セマンティックカテゴリの推定"""
        # 統計的特性に基づく簡単な分類
    ...
    def _cluster_dimensions(self, embeddings: np.ndarray, 
        ...
        try:
            ...
            for cluster_id in range(n_clusters):
                ...
                dimension_clusters[f'cluster_{cluster_id}'] = {
                    ...
                    'center': cluster_centers[cluster_id].tolist() if cluster_centers is not None else None
                }
    ...
    def _analyze_correlations_with_basic_features(self, embeddings: np.ndarray,
        ...
        try:
            ...
            gat_basic_correlations = correlations[:n_gat_dims, n_gat_dims:n_gat_dims+n_basic_features]
            ...
            for gat_dim in range(n_gat_dims):
                high_corr_indices = np.where(np.abs(gat_basic_correlations[gat_dim, :]) >= threshold)[0]
            ...
            return {
                ...
                'mean_absolute_correlation': float(np.mean(np.abs(gat_basic_correlations)))
            }
    ...
    def _find_similarity_patterns(self, embeddings: np.ndarray, 
                                config: Dict[str, Any]) -> Dict[str, Any]:
        """類似性パターンの発見"""
            ...
            if node_i not in used_nodes and node_j not in used_nodes:
                ...
                for k in range(n_nodes):
                    if k not in [node_i, node_j] and k not in used_nodes:
                        sim_to_group = np.mean([similarity_matrix[k, node_i], similarity_matrix[k, node_j]])
    ...
    def _find_cluster_patterns(self, embeddings: np.ndarray, 
                             config: Dict[str, Any]) -> Dict[str, Any]:
        """クラスタパターンの発見"""
        try:
            # K-meansクラスタリング
            ...
            for cluster_id in range(n_clusters):
                ...
                if cluster_size > 1:
                    ...
                    cohesion = np.mean(internal_distances[np.triu_indices_from(internal_distances, k=1)])
                else:
                    cohesion = 1.0
    ...
    def _find_outlier_patterns(self, embeddings: np.ndarray, 
                             config: Dict[str, Any]) -> Dict[str, Any]:
        """異常値パターンの発見"""
        try:
            # 各ノードの近傍との平均類似度を計算
    ...
    def visualize_collaboration_network(self, output_dir: Optional[str] = None) -> Dict[str, str]:
        """協力関係の可視化
        ...
        try:
            ...
            if 'dev' in self.embeddings:
                ...
                if dev_embeddings.shape[1] > 2:
                    if self.interpretation_config['visualization']['dimension_reduction'] == '2d':
                ...
                # 開発者埋め込みプロット
                fig, ax = plt.subplots(figsize=self.interpretation_config['visualization']['plot_size'])
                ...
                embedding_plot_path = output_dir / f'dev_embeddings_2d_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png'
            ...
            # 協力ネットワークの可視化
            collaboration_plot_path = self._create_collaboration_network_plot(output_dir)
            ...
            logger.error("可視化に必要なライブラリ（matplotlib, networkx）がインストールされていません")
            return {}
        except Exception as e:
    ...
    def _create_collaboration_network_plot(self, output_dir: Path) -> Optional[str]:
        ...
        try:
            ...
            if ('dev', 'collaborates', 'dev') in self.graph_data.edge_index_dict:
                collab_edges = self.graph_data.edge_index_dict[('dev', 'collaborates', 'dev')]
            ...
            edges = [(int(collab_edges[0, i]), int(collab_edges[1, i])) 
            ...
            network_plot_path = output_dir / f'collaboration_network_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png'
    ...
    def analyze_feature_correlations(self, node_type: str = 'dev',
                                   basic_feature_names: Optional[List[str]] = None) -> Dict[str, Any]:
        """GAT特徴量と基本特徴量の相関分析
        ...
        feature_names = basic_feature_names or [f'basic_feature_{i}' for i in range(basic_features.shape[1])]
        ...
        try:
            ...
            # 高相関ペアの特定
            threshold = self.interpretation_config['dimension_interpretation']['correlation_threshold']
            
            for gat_dim in range(n_gat):
                high_corr_indices = np.where(np.abs(gat_basic_corr[gat_dim, :]) >= threshold)[0]
                ...
                if len(high_corr_indices) > 0:
                    ...
                    for basic_idx in high_corr_indices:
                        correlations.append({
                            ...
                            'correlation': float(gat_basic_corr[gat_dim, basic_idx]),
                            'abs_correlation': float(np.abs(gat_basic_corr[gat_dim, basic_idx]))
                        })
                    
                    # 絶対値でソート
                    correlations.sort(key=lambda x: x['abs_correlation'], reverse=True)
                    results['top_correlations'][f'gat_dim_{gat_dim}'] = correlations[:10]
    ...
    def _analyze_feature_redundancy(self, gat_embeddings: np.ndarray,
                                  basic_features: np.ndarray,
    ...
    def get_interpretation_summary(self) -> Dict[str, Any]:
        ...
        # 次元解釈結果の要約
        dimension_results = [k for k in self.interpretation_results.keys() if k.startswith('dimension_interpretation')]
        if dimension_results:
            for result_key in dimension_results:
                ...
                summary['results_summary'][f'dimension_interpretation_{node_type}'] = {
                    ...
                    'n_dimension_clusters': result['dimension_clusters'].get('n_clusters', 0),
                    'has_correlation_analysis': bool(result.get('correlation_analysis'))
                }
        
        # パターン分析結果の要約
        pattern_results = [k for k in self.interpretation_results.keys() if k.startswith('graph_patterns')]
        if pattern_results:
            for result_key in pattern_results:
                ...
                summary['results_summary'][f'graph_patterns_{node_type}'] = {
                    'n_similarity_groups': result['similarity_patterns'].get('n_groups', 0),
                    'n_clusters': result['cluster_patterns'].get('n_clusters', 0),
                    'n_outliers': result['outlier_patterns'].get('n_outliers', 0)
                }
        
        # 相関分析結果の要約
        correlation_results = [k for k in self.interpretation_results.keys() if k.startswith('correlation_analysis')]
            ...
            for result_key in correlation_results:
                ...
                if 'correlation_matrix' in result:
                    summary['results_summary'][f'correlation_analysis_{node_type}'] = {
                        'max_correlation': result['correlation_matrix'].get('max_correlation', 0),
                        'mean_abs_correlation': result['correlation_matrix'].get('mean_abs_correlation', 0),
        ...
        try:
            ...
            for result_key, result in self.interpretation_results.items():
                if result_key.startswith('correlation_analysis'):
                    ...
                    if 'correlation_matrix' in result:
                        max_corr = result['correlation_matrix'].get('max_correlation', 0)
                        if max_corr > 0.5:
    ...
    def _write_interpretation_details(self, file_handle, result_data: Dict[str, Any]) -> None:
        """解釈結果詳細をファイルに書き込み"""
        for key, value in result_data.items():
            if isinstance(value, dict):
                ...
                file_handle.write(f"  {key}: [リストデータ - 省略 (長さ: {len(value)})]\n")
            else:
