...
class GATIntegratedOptimizer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # 統合最適化設定
        self.integration_config = self.config.get(
            "integration_config",
            {
                "combination_search": {
    ...
    def load_components(
        self, gat_optimizer: GATOptimizer, gat_interpreter: GATInterpreter
    ...
    def load_features(
        self,
        basic_features: Union[np.ndarray, Dict[str, np.ndarray]],
        ...
        try:
            ...
            if self.basic_features.shape[0] != self.gat_features.shape[0]:
                raise ValueError(
                    f"基本特徴量とGAT特徴量のサンプル数が一致しません: "
                    f"{self.basic_features.shape[0]} vs {self.gat_features.shape[0]}"
                )

            if (
                target_values is not None
                and len(target_values) != self.basic_features.shape[0]
            ):
                raise ValueError(
                    f"ターゲット値とサンプル数が一致しません: "
                    f"{len(target_values)} vs {self.basic_features.shape[0]}"
                )

            self.is_fitted = True
            logger.info(
                f"特徴量読み込み完了: 基本={self.basic_features.shape}, GAT={self.gat_features.shape}"
            )
            return self
    ...
    def find_optimal_combination(
        self,
        basic_feature_names: Optional[List[str]] = None,
        gat_feature_names: Optional[List[str]] = None,
    ) -> Dict[str, Any]:
        ...
        if basic_feature_names is None:
            basic_feature_names = [
                f"basic_feature_{i}" for i in range(self.basic_features.shape[1])
            ]
        if gat_feature_names is None:
            gat_feature_names = [
                f"gat_feature_{i}" for i in range(self.gat_features.shape[1])
            ]
        ...
        if combination_results:
            ...
            results["optimal_combination"] = {
                ...
                "selected_features": combination_results[best_combination][
                    "selected_features"
                ],
                "performance_score": combination_results[best_combination][
                    "performance_score"
                ],
                "feature_count": len(
                    combination_results[best_combination]["selected_features"]
                ),
            }
    ...
    def _exhaustive_combination_search(
        self, features: np.ndarray, feature_names: List[str], config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """全探索による組み合わせ探索"""
    ...
    def _greedy_combination_search(
        self, features: np.ndarray, feature_names: List[str], config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """貪欲法による組み合わせ探索"""
        if self.target_values is None:
        ...
        for step in range(max_features):
            ...
            if best_addition is not None:
                ...
                if best_addition_score <= best_score:
                    logger.info(
                        f"性能向上が見られないため、ステップ {step + 1} で探索終了"
                    )
                    break

                best_score = best_addition_score
                logger.info(
                    f"ステップ {step + 1}: 特徴量 '{feature_names[best_addition]}' を追加 (スコア: {best_addition_score:.4f})"
                )

        return results

    def _genetic_combination_search(
        self, features: np.ndarray, feature_names: List[str], config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """遺伝的アルゴリズムによる組み合わせ探索"""
        if self.target_values is None:
        ...
        for _ in range(population_size):
            ...
            if selected_count > max_selected:
                ...
                to_remove = np.random.choice(
                    selected_indices, size=selected_count - max_selected, replace=False
                )
        ...
        for generation in range(generations):
            ...
            logger.info(
                f"世代 {generation + 1}: 最良スコア = {best_score:.4f}, 特徴量数 = {len(selected_indices)}"
            )
    ...
    def _correlation_based_search(
        ...
        variance_threshold = self.integration_config["redundancy_removal"][
            "variance_threshold"
        ]
        ...
        results = {
            "correlation_based_selection": {
                ...
                "performance_score": float(np.sum(valid_features))
                / len(feature_names),  # 選択された特徴量の割合
    ...
    def _evaluate_feature_combination(self, features: np.ndarray) -> float:
        ...
        try:
            if config["performance_metric"] == "r2":
                ...
                # デフォルトは相関係数の絶対値の平均
                correlations = [
                    np.corrcoef(features[:, i], self.target_values)[0, 1]
    ...
    def _compare_feature_combinations(
        self,
        basic_feature_names: List[str],
        gat_feature_names: List[str],
        optimal_combination: Optional[Dict[str, Any]],
    ) -> Dict[str, Any]:
        """特徴量組み合わせの性能比較"""
        comparison = {}
        ...
        try:
            ...
            if optimal_combination:
                ...
                if baseline_score != 0:
                    improvement = (
                        optimal_combination["performance_score"] - baseline_score
                    ) / abs(baseline_score)
    ...
    def remove_redundant_features(
        ...
        # 結果の整理
        removed_indices = [
            i for i in range(len(feature_names)) if i not in remaining_features
        ]
        ...
        logger.info(
            f"冗長性除去完了: {len(feature_names)} → {len(remaining_features)} 特徴量"
        )

        return results

    def _analyze_feature_redundancy_detailed(
        self, features: np.ndarray, feature_names: List[str], config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """詳細な特徴量冗長性分析"""
        analysis = {}
        ...
        try:
            ...
            if self.target_values is not None:
                mutual_info_scores = mutual_info_classif(
                    features, self.target_values.astype(int), discrete_features=False
                )

                low_info_features = []
                for i, (name, score) in enumerate(
                    zip(feature_names, mutual_info_scores)
                ):
    ...
    def _remove_redundancy_correlation_first(
        self, features: np.ndarray, feature_names: List[str], config: Dict[str, Any]
    ) -> List[int]:
        """相関ベースの冗長性除去"""
        correlation_matrix = np.corrcoef(features.T)
        ...
        # 残存特徴量のインデックス
        remaining_features = [
            i for i in range(n_features) if i not in features_to_remove
        ]

        return remaining_features

    def _remove_redundancy_mutual_info_first(
        self, features: np.ndarray, feature_names: List[str], config: Dict[str, Any]
    ) -> List[int]:
        """相互情報量ベースの冗長性除去"""
        n_features = len(feature_names)
        ...
        if self.target_values is not None:
            try:
                ...
                return self._remove_redundancy_correlation_first(
    ...
    def _analyze_information_content(
    ...
    def adjust_dimensions_dynamically(
        ...
        for n_dims in test_dimensions:
            ...
            if config["adjustment_strategy"] == "performance_based":
                ...
                # デフォルトはPCA
                reduced_features = self._reduce_dimensions_pca(
                    self.gat_features, n_dims
                )
            ...
            # 性能低下率の計算
            performance_loss = (
                (baseline_performance - reduced_performance) / abs(baseline_performance)
            ...
            logger.info(
                f"次元数 {n_dims}: 性能={reduced_performance:.4f}, 低下率={performance_loss:.2%}"
            )

        # 最適次元数の決定
        acceptable_dimensions = [
            dims
            for dims, result in results["dimension_reduction_results"].items()
        ...
        if acceptable_dimensions:
            ...
            results["recommended_reduction"] = results["dimension_reduction_results"][
                optimal_dims
            ]
        else:
            ...
            if results["dimension_reduction_results"]:
                optimal_dims = min(
                    results["dimension_reduction_results"].keys(),
                    key=lambda d: results["dimension_reduction_results"][d][
                        "performance_loss"
                    ],
                )
                results["optimal_dimensions"] = optimal_dims
                results["recommended_reduction"] = results[
                    "dimension_reduction_results"
                ][optimal_dims]
        ...
        logger.info(
            f"動的次元調整完了: 推奨次元数 = {results.get('optimal_dimensions', 'N/A')}"
        )

        return results

    def _reduce_dimensions_performance_based(
        self, features: np.ndarray, target_dims: int
    ) -> Optional[np.ndarray]:
        """性能ベースの次元削減"""
        if self.target_values is None:
            # ターゲット値がない場合はPCAを使用
        ...
        try:
            ...
            reduced_features = selector.fit_transform(
                features, self.target_values.astype(int)
            )
    ...
    def _reduce_dimensions_information_based(
        self, features: np.ndarray, target_dims: int
    ) -> Optional[np.ndarray]:
        """情報量ベースの次元削減"""
        try:
    ...
    def _analyze_dimension_performance_tradeoff(
        self, reduction_results: Dict[int, Dict[str, Any]]
    ) -> Dict[str, Any]:
        """次元数と性能のトレードオフ分析"""
        ...
        performance_losses = [
            reduction_results[d]["performance_loss"] for d in dimensions
        ]
        reduction_ratios = [reduction_results[d]["reduction_ratio"] for d in dimensions]

        analysis = {
            "dimension_performance_correlation": float(
                np.corrcoef(dimensions, performances)[0, 1]
            ),
        ...
        for dims in dimensions:
            ...
            efficiency = result["reduction_ratio"] / (
                result["performance_loss"] + 1e-10
            )
        ...
        if analysis["efficiency_analysis"]:
            most_efficient_dims = max(
                analysis["efficiency_analysis"].keys(),
    ...
    def get_integration_summary(self) -> Dict[str, Any]:
        ...
        if "combination_search" in self.optimization_results:
            ...
            summary["results_summary"]["combination_search"] = {
                ...
                "optimal_combination_found": combo_result["optimal_combination"]
                is not None,
            }
        ...
        if "dimension_adjustment" in self.optimization_results:
            ...
            if dimension_result.get("optimal_dimensions"):
                summary["recommendations"]["optimal_dimensions"] = dimension_result[
                    "optimal_dimensions"
                ]
    ...
    def _write_integration_details(
