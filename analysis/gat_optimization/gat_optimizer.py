...
class GATOptimizer:
    ...
    def load_gat_model(self, model_path: str, graph_data_path: Optional[str] = None) -> 'GATOptimizer':
        """GATモデルと関連データを読み込み
        ...
        try:
            ...
            if not model_path.exists():
                raise FileNotFoundError(f"GATモデルファイルが見つかりません: {model_path}")
            ...
            if graph_data_path:
                ...
                if graph_data_path.exists():
                    ...
                    logger.warning(f"グラフデータファイルが見つかりません: {graph_data_path}")
    ...
    def extract_embeddings(self, graph_data: Optional[Any] = None) -> Dict[str, torch.Tensor]:
        """GATモデルから埋め込みを抽出
    ...
    def analyze_optimal_dimensions(self, 
                                 node_type: str = 'dev',
        ...
        methods = methods or self.optimization_config['dimension_search']['evaluation_methods']
        ...
        for method in methods:
            ...
            if method == 'variance':
                method_results = self._analyze_dimensions_by_variance(embeddings, dimensions_to_test)
            elif method == 'clustering':
                method_results = self._analyze_dimensions_by_clustering(embeddings, dimensions_to_test)
            elif method == 'information':
                method_results = self._analyze_dimensions_by_information(embeddings, dimensions_to_test)
            else:
        ...
        logger.info(f"次元数分析完了: 推奨次元数 = {results.get('recommended_dimension', 'N/A')}")
        
        return results
    
    def _analyze_dimensions_by_variance(self, embeddings: np.ndarray, 
        ...
        if results['dimension_scores']:
            optimal_dim = max(results['dimension_scores'].keys(), 
                            key=lambda d: results['dimension_scores'][d]['score'])
            results['optimal_dimension'] = optimal_dim
        
        return results
    
    def _analyze_dimensions_by_clustering(self, embeddings: np.ndarray, 
                                        dimensions: List[int]) -> Dict[str, Any]:
        """クラスタリング品質ベースの次元数分析"""
        results = {
            'method': 'clustering',
        ...
        min_samples = self.optimization_config['quality_metrics']['silhouette_min_samples']
    ...
    def _analyze_dimensions_by_information(self, embeddings: np.ndarray, 
                                         dimensions: List[int]) -> Dict[str, Any]:
        """情報量ベースの次元数分析"""
        results = {
            'method': 'information',
        ...
        for dim in dimensions:
            ...
            try:
                ...
                for i in range(dim):
                    # ヒストグラムを作成してエントロピー計算
                    hist, _ = np.histogram(reduced_embeddings[:, i], bins=n_bins, density=True)
    ...
    def analyze_attention_weights(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.optimization_config['attention_analysis']['layer_analysis']:
                results['layer_analysis'] = self._analyze_attention_by_layer(attention_weights)
            
            # エッジタイプ別分析
            if self.optimization_config['attention_analysis']['edge_type_analysis']:
                results['edge_type_analysis'] = self._analyze_attention_by_edge_type(attention_weights)
            
            # 総合分析
            results['attention_analysis'] = self._analyze_attention_overall(attention_weights)
            ...
            for name, module in self.gat_model.named_modules():
                ...
                if hasattr(module, 'attention'):
                    attention_weights[name] = {
                        'weights': module.attention.detach().cpu().numpy() if hasattr(module.attention, 'detach') else None,
                        'module_type': type(module).__name__
                    }
                    
            logger.info(f"アテンション重み抽出完了: {len(attention_weights)} モジュール")
    ...
    def _analyze_attention_by_layer(self, attention_weights: Dict[str, Any]) -> Dict[str, Any]:
        """層別アテンション分析"""
        layer_results = {}
    ...
    def _analyze_attention_by_edge_type(self, attention_weights: Dict[str, Any]) -> Dict[str, Any]:
        """エッジタイプ別アテンション分析"""
        edge_type_results = {}
        ...
        for edge_type in edge_types:
            matching_layers = [name for name in attention_weights.keys() if edge_type in name]
            ...
            for layer_name in matching_layers:
                ...
                edge_type_results[edge_type][layer_name] = {
                    ...
                    'attention_concentration': float(np.std(weights) / (np.mean(weights) + 1e-10))
                }
        
        return edge_type_results
    
    def _analyze_attention_overall(self, attention_weights: Dict[str, Any]) -> Dict[str, Any]:
        """アテンション重みの総合分析"""
        all_weights = []
        layer_statistics = {}
        ...
        for layer_name, attention_data in attention_weights.items():
            ...
            layer_statistics[layer_name] = {
                ...
                'contribution_ratio': float(np.sum(weights)) if len(all_weights) > 0 else 0.0
            }
        ...
        return {
            'overall_statistics': {
                ...
                'attention_concentration': float(np.std(all_weights) / (np.mean(all_weights) + 1e-10))
            },
    ...
    def evaluate_embedding_quality(self, node_type: str = 'dev') -> Dict[str, Any]:
        ...
        logger.info(f"埋め込み品質評価完了: 総合スコア = {results['overall_quality_score']:.3f}")
    ...
    def _evaluate_information_quality(self, embeddings: np.ndarray) -> Dict[str, Any]:
        ...
        # 情報の均一性
        information_uniformity = 1.0 / (1.0 + std_information / (mean_information + 1e-10))
    ...
    def get_optimization_summary(self) -> Dict[str, Any]:
        ...
        # 次元分析結果の要約
        dimension_results = [k for k in self.optimization_results.keys() if k.startswith('dimension_analysis')]
        if dimension_results:
            for result_key in dimension_results:
                ...
                if 'recommended_dimension' in result:
                    summary['recommendations'][f'optimal_dimensions_{node_type}'] = result['recommended_dimension']
        ...
        if 'attention_analysis' in self.optimization_results:
            ...
            summary['results_summary']['attention_analysis'] = {
                ...
                'edge_types_analyzed': len(attention_result.get('edge_type_analysis', {}))
            }
            
            if 'attention_analysis' in attention_result:
                overall = attention_result['attention_analysis'].get('overall_statistics', {})
                if overall:
                    summary['recommendations']['attention_concentration'] = overall.get('attention_concentration')
        
        # 品質評価結果の要約
        quality_results = [k for k in self.optimization_results.keys() if k.startswith('quality_evaluation')]
        if quality_results:
            for result_key in quality_results:
                ...
                if result['overall_quality_score'] is not None:
                    summary['recommendations'][f'quality_score_{node_type}'] = result['overall_quality_score']
