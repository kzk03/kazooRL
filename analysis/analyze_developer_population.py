...
def analyze_developer_population():
    ...
    print(f"\n🔄 プロファイルとの重複分析:")
    print(f"  プロファイルに含まれる担当者: {len(assignees_in_profiles)}/{len(actual_assignees)} ({len(assignees_in_profiles)/len(actual_assignees)*100:.1f}%)")
    print(f"  プロファイルに含まれない担当者: {len(assignees_not_in_profiles)}")
    
    if assignees_not_in_profiles:
        print(f"\n❌ プロファイルに含まれない主要担当者:")
        missing_assignee_counts = {name: assignee_counts[name] for name in assignees_not_in_profiles}
        for assignee, count in sorted(missing_assignee_counts.items(), key=lambda x: x[1], reverse=True)[:10]:
    ...
    print(f"  候補に含まれる実際の担当者: {len(assignees_in_profiles & candidates_set)}")
    print(f"  候補に含まれない実際の担当者: {len(assignees_in_profiles - candidates_set)}")
    
    # 候補に含まれない担当者の詳細
    missing_from_candidates = assignees_in_profiles - candidates_set
    if missing_from_candidates:
        print(f"\n⚠️ 推薦候補に含まれない担当者（プロファイルはあり）:")
        missing_candidate_counts = {name: assignee_counts[name] for name in missing_from_candidates if name in assignee_counts}
        for assignee, count in sorted(missing_candidate_counts.items(), key=lambda x: x[1], reverse=True)[:10]:
            # プロファイル内での順位を確認
        ...
        if task.get("assignees"):
            ...
            if any(assignee in candidates_set for assignee in assignees):
                evaluable_tasks.append({
                    ...
                    "task_title": task.get("title", "")[:50],
                    "assignees": assignees,
                    "assignees_in_candidates": [a for a in assignees if a in candidates_set]
                })
    
    print(f"\n✅ 評価可能タスク分析:")
    print(f"  評価可能タスク数: {len(evaluable_tasks)}/{len(test_tasks)} ({len(evaluable_tasks)/len(test_tasks)*100:.1f}%)")
    ...
    max_possible_accuracy = len(assignees_in_profiles & candidates_set) / len(assignees_in_profiles) if assignees_in_profiles else 0
    print(f"  理論的最大精度: {max_possible_accuracy*100:.1f}%")
    
    # 現在の候補選択による制約
    coverage_rate = len(evaluable_tasks) / len([t for t in test_tasks if t.get("assignees")]) if test_tasks else 0
    ...
    return {
        ...
        "missing_from_candidates": list(missing_from_candidates)[:10] if missing_from_candidates else []
    }
