...
class DeveloperFeatureDesigner:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # プログラミング言語リスト
        self.programming_languages = self.config.get(
        ...
        # 技術ドメイン
        self.technical_domains = self.config.get(
            "technical_domains",
            [
                "web",
                "mobile",
        ...
        # フレームワーク・ライブラリ
        self.frameworks = self.config.get(
            "frameworks",
            [
                "react",
                "vue",
    ...
    def _improve_existing_features(
        ...
        # 2. 協力ネットワークの対数変換
        features["dev_collaboration_network_log"] = float(
            np.log1p(collaboration_network_size)
        )
        features["dev_comment_interactions_log"] = float(np.log1p(comment_interactions))

        # 3. 効率性指標（比率計算）
        if total_commits > 0:
            features["dev_lines_per_commit"] = float(
                total_lines_changed / total_commits
            )
        ...
        if total_activity > 0:
            features["dev_collaboration_ratio"] = float(
                collaboration_network_size / total_activity
            )
            features["dev_comment_activity_ratio"] = float(
                comment_interactions / total_activity
            )
        else:
    ...
    def _extract_expertise_features(
        self, developer_data: Dict[str, Any]
    ) -> Dict[str, float]:
        """専門性特徴量の抽出
        ...
        if languages:
            ...
            if total_lang_usage > 0:
                ...
                features["dev_primary_language_strength"] = float(
                    max_lang_usage / total_lang_usage
                )

                # 上位3言語の集中度
                sorted_langs = sorted(languages.values(), reverse=True)
                top3_usage = sum(sorted_langs[:3])
                features["dev_top3_language_concentration"] = float(
                    top3_usage / total_lang_usage
                )
            else:
        ...
        if domain_scores:
            ...
            features["dev_domain_specialization"] = float(
                len([s for s in domain_scores.values() if s > 0.1])
            )
        else:
    ...
    def _extract_activity_pattern_features(
        self, developer_data: Dict[str, Any]
    ) -> Dict[str, float]:
        """活動パターン特徴量の抽出
        ...
        if activity_hours:
            ...
            features["dev_night_activity_ratio"] = float(
                len(night_hours) / len(activity_hours)
            )

            # 営業時間活動比率（9時-17時）
            business_hours = [h for h in activity_hours if 9 <= h <= 17]
            features["dev_business_hours_ratio"] = float(
                len(business_hours) / len(activity_hours)
            )
        else:
        ...
        if activity_days:
            ...
            features["dev_weekday_activity_ratio"] = float(
                len(weekday_activities) / total_activities
            )
            features["dev_weekend_activity_ratio"] = float(
                len(weekend_activities) / total_activities
            )
        ...
        if response_times:
            ...
            features["dev_quick_response_ratio"] = float(
                len(quick_responses) / len(response_times)
            )

            # 応答時間の対数変換
            features["dev_avg_response_time_log"] = float(
                np.log1p(np.mean(response_times))
            )
        else:
        ...
        consistency_factors = [
            ...
            1.0
            / (1.0 + features.get("dev_activity_hour_variance", 1.0) / 100),  # 正規化
    ...
    def _extract_quality_features(
        ...
        if reviews_given > 0:
            features["dev_review_approval_rate"] = float(
                approved_reviews / reviews_given
            )
        else:
        ...
        if review_comments > 0:
            features["dev_review_helpfulness_ratio"] = float(
                helpful_reviews / review_comments
            )
        else:
        ...
        if total_files_touched > 0:
            features["dev_test_coverage_ratio"] = float(
                test_files_touched / total_files_touched
            )
        else:
        ...
        if total_files_touched > 0:
            features["dev_documentation_ratio"] = float(
                doc_files_touched / total_files_touched
            )
        else:
        ...
        quality_components = [
            ...
            features["dev_review_approval_rate"]
            * self.quality_weights["review_approval_rate"],
            (1.0 - features["dev_bug_introduction_rate"])
            * self.quality_weights["bug_introduction_rate"],
            features["dev_review_helpfulness_ratio"]
            * self.quality_weights["code_review_quality"],
        ]
        ...
        if years_experience > 0:
            features["dev_quality_per_experience"] = float(
                features["dev_overall_quality_score"] / years_experience
            )
        else:
            features["dev_quality_per_experience"] = features[
    ...
    def _calculate_domain_expertise(self, touched_files: List[str]) -> Dict[str, float]:
        ...
        for file_path in touched_files:
            ...
            for domain in self.technical_domains:
                ...
                elif domain == "frontend" and any(
                    term in file_path_lower
                    for term in ["ui", "view", "component", "css", "html"]
                ):
                    domain_counts[domain] += 1
                elif domain == "backend" and any(
                    term in file_path_lower
                    for term in ["api", "server", "service", "controller"]
                ):
                    domain_counts[domain] += 1
                elif domain == "database" and any(
                    term in file_path_lower
                    for term in ["db", "sql", "migration", "schema"]
                ):
                    domain_counts[domain] += 1
                elif domain == "testing" and any(
                    term in file_path_lower for term in ["test", "spec", "__test__"]
                ):
                    domain_counts[domain] += 1
        ...
        if total_files > 0:
            domain_scores = {
                domain: count / total_files for domain, count in domain_counts.items()
            }
        else:
