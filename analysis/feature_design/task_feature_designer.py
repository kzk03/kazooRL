...
class TaskFeatureDesigner:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # デフォルト設定
        self.priority_labels = self.config.get(
            "priority_labels",
            {"critical": 5, "high": 4, "medium": 3, "low": 2, "trivial": 1},
        )

        self.technical_terms = self.config.get(
            "technical_terms",
            [
                "API",
                "database",
        ...
        self.urgency_keywords = self.config.get(
            "urgency_keywords",
            [
                "urgent",
                "critical",
    ...
    def design_enhanced_features(
        self, task_data: Dict[str, Any], env_context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, float]:
        """強化されたタスク特徴量を設計
    ...
    def _improve_existing_features(self, task_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        if updated_at:
            try:
                ...
                features["task_days_since_update_log"] = float(
                    np.log1p(max(0, days_since_update))
                )
        ...
        if text_length > 0:
            features["task_code_density"] = float(
                code_blocks / text_length * 1000
            )  # 1000文字あたり
        else:
    ...
    def _extract_urgency_features(
        ...
        if text_length > 0:
            features["task_urgency_keyword_density"] = float(
                urgency_count / text_length * 1000
            )
        else:
        ...
        deadline_mentions = sum(
            text_content.count(keyword) for keyword in deadline_keywords
        )
        features["task_deadline_mentions"] = float(deadline_mentions)

        # 5. マイルストーン関連
        milestone_keywords = ["milestone", "release", "version", "sprint"]
        milestone_mentions = sum(
            text_content.count(keyword) for keyword in milestone_keywords
        )
        features["task_milestone_mentions"] = float(milestone_mentions)

        # 6. ブロッキング課題の特徴量
        blocking_keywords = ["block", "blocker", "blocking", "depends", "dependency"]
        blocking_mentions = sum(
            text_content.count(keyword) for keyword in blocking_keywords
        )
        ...
        if env_context and "blocking_issues" in env_context:
            features["task_blocking_issues_count"] = float(
                len(env_context["blocking_issues"])
            )
        else:
    ...
    def _extract_complexity_features(
        ...
        text_length = len(text_content)
        if text_length > 0:
            features["task_technical_term_density"] = float(
                tech_term_count / text_length * 1000
            )
        else:
        ...
        dependency_count = sum(
            text_content.lower().count(keyword) for keyword in dependency_keywords
        )
        ...
        question_keyword_count = sum(
            text_content.lower().count(keyword) for keyword in question_keywords
        )
    ...
    def _extract_social_attention_features(
        self, task_data: Dict[str, Any]
    ) -> Dict[str, float]:
        """社会的注目度特徴量の抽出
        ...
        for reaction_type in reaction_types:
            count = (
                reactions.get(reaction_type, 0) if isinstance(reactions, dict) else 0
            )
            features[
                f'task_reaction_{reaction_type.replace("+", "plus").replace("-", "minus")}'
            ] = float(count)
