...
class MatchingFeatureDesigner:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # 技術スタック階層
        self.tech_stack_hierarchy = self.config.get('tech_stack_hierarchy', {
        ...
        # 協力強度の重み
        self.collaboration_weights = self.config.get('collaboration_weights', {
        ...
        # 成功指標の重み
        self.success_weights = self.config.get('success_weights', {
            'completion_rate': 0.4,
            'quality_score': 0.3,
            'timeliness': 0.2,
            'satisfaction': 0.1
    ...
    def design_enhanced_features(self, task_data: Dict[str, Any], 
        ...
        # 既存特徴量の改良
        features.update(self._improve_existing_features(task_data, developer_data, env_context))
        
        # 時間的近接性特徴量
        features.update(self._extract_temporal_proximity_features(task_data, developer_data, env_context))
        
        # 技術的適合性特徴量
        features.update(self._extract_technical_compatibility_features(task_data, developer_data))
        
        # 成功履歴特徴量
        features.update(self._extract_success_history_features(task_data, developer_data, env_context))
        
        logger.debug(f"マッチング特徴量設計完了: {len(features)}個の特徴量")
        return features
    
    def _improve_existing_features(self, task_data: Dict[str, Any], 
                                  developer_data: Dict[str, Any],
                                  env_context: Optional[Dict[str, Any]] = None) -> Dict[str, float]:
        """既存特徴量の改良
        ...
        # 1. 協力強度の改良
        collaboration_history = env_context.get('collaboration_history', []) if env_context else []
        ...
        direct_collaborations = [
            ...
            if developer_name in collab.get('participants', []) and task_author in collab.get('participants', [])
        ]
        
        if direct_collaborations:
            # 協力の頻度と最近性を考慮
            recent_collaborations = [
                collab for collab in direct_collaborations
                if self._is_recent(collab.get('date'), days=90)
            ]
            
            collaboration_strength = len(recent_collaborations) * 2.0 + len(direct_collaborations) * 1.0
            features['match_collaboration_strength_enhanced'] = float(np.log1p(collaboration_strength))
        else:
        ...
        if task_required_skills and developer_skills:
            ...
            # 部分一致スキル（類似技術）
            partial_matches = self._calculate_skill_similarity(task_required_skills, developer_skills)
        ...
        if task_files and developer_files:
            ...
            # ディレクトリレベルの経験
            directory_matches = self._calculate_directory_overlap(task_files, developer_files)
            features['match_directory_experience'] = float(directory_matches)
            
            # ファイルタイプの経験
            file_type_matches = self._calculate_file_type_overlap(task_files, developer_files)
    ...
    def _extract_temporal_proximity_features(self, task_data: Dict[str, Any], 
                                           developer_data: Dict[str, Any],
                                           env_context: Optional[Dict[str, Any]] = None) -> Dict[str, float]:
        """時間的近接性特徴量の抽出
        ...
        if recent_collaborations:
            # 最も最近の協力からの日数
            latest_collaboration = max(recent_collaborations, key=lambda x: x.get('date', ''))
            days_since_last_collaboration = self._calculate_days_since(latest_collaboration.get('date'))
            features['match_days_since_last_collaboration'] = float(days_since_last_collaboration)
            features['match_days_since_last_collaboration_log'] = float(np.log1p(days_since_last_collaboration))
            ...
            features['match_recent_collaboration_count'] = float(len(recent_30d_collaborations))
        else:
        ...
        if task_activity_hours and developer_activity_hours:
            ...
            if union_hours:
                features['match_activity_time_overlap'] = float(len(overlap_hours) / len(union_hours))
            else:
                features['match_activity_time_overlap'] = 0.0
            
            # ピーク活動時間の近さ
            task_peak_hour = Counter(task_activity_hours).most_common(1)[0][0] if task_activity_hours else 12
            dev_peak_hour = Counter(developer_activity_hours).most_common(1)[0][0] if developer_activity_hours else 12
            
            hour_distance = min(abs(task_peak_hour - dev_peak_hour), 24 - abs(task_peak_hour - dev_peak_hour))
        ...
        timezone_compatibility = self._calculate_timezone_compatibility(task_timezone, developer_timezone)
        features['match_timezone_compatibility'] = float(timezone_compatibility)
        
        # 4. 応答時間予測
        developer_avg_response_time = developer_data.get('avg_response_time', 24.0)  # 時間
        ...
        features['match_predicted_response_time_log'] = float(np.log1p(predicted_response_time))
        
        # 応答時間適合性（短いほど良い）
        features['match_response_time_fitness'] = float(1.0 / (1.0 + predicted_response_time / 24.0))
        
        return features
    
    def _extract_technical_compatibility_features(self, task_data: Dict[str, Any], 
                                                developer_data: Dict[str, Any]) -> Dict[str, float]:
        """技術的適合性特徴量の抽出
        ...
        developer_architecture_experience = self._extract_developer_architecture_experience(developer_data)
        
        architecture_compatibility = 0.0
        for arch_type in task_architecture_hints:
            if arch_type in developer_architecture_experience:
                architecture_compatibility += developer_architecture_experience[arch_type]
        ...
        developer_experience_level = self._calculate_developer_experience_level(developer_data)
        ...
        features['match_overall_tech_compatibility'] = float(sum(tech_compatibility_components))
        
        return features
    
    def _extract_success_history_features(self, task_data: Dict[str, Any], 
                                         developer_data: Dict[str, Any],
                                         env_context: Optional[Dict[str, Any]] = None) -> Dict[str, float]:
        """成功履歴特徴量の抽出
        ...
        developer_tasks = [
            ...
            if developer_name in task.get('assignees', [])
        ]
        
        # 1. 過去成功率
        if developer_tasks:
            completed_tasks = [task for task in developer_tasks if task.get('status') == 'completed']
            ...
            if recent_tasks:
                recent_completed = [task for task in recent_tasks if task.get('status') == 'completed']
                recent_success_rate = len(recent_completed) / len(recent_tasks)
                features['match_recent_success_rate'] = float(recent_success_rate)
            else:
                features['match_recent_success_rate'] = features['match_historical_success_rate']
        else:
        ...
        for task in developer_tasks:
            # タスクタイプまたはラベルが類似
            if (task.get('type') == current_task_type or 
                len(set(task.get('labels', [])).intersection(current_task_labels)) > 0):
                similar_tasks.append(task)
        
        if similar_tasks:
            similar_completed = [task for task in similar_tasks if task.get('status') == 'completed']
            similar_completion_rate = len(similar_completed) / len(similar_tasks)
            features['match_similar_task_completion_rate'] = float(similar_completion_rate)
        else:
            features['match_similar_task_completion_rate'] = features['match_historical_success_rate']
        
        # 3. 協力満足度
        collaboration_feedback = env_context.get('collaboration_feedback', {}) if env_context else {}
        developer_feedback = collaboration_feedback.get(developer_name, [])
        
        if developer_feedback:
            avg_satisfaction = np.mean([feedback.get('satisfaction', 3.0) for feedback in developer_feedback])
            # 1-5スケールを0-1に正規化
            normalized_satisfaction = (avg_satisfaction - 1.0) / 4.0
            features['match_collaboration_satisfaction'] = float(normalized_satisfaction)
        else:
        ...
        if developer_tasks:
            # 平均品質スコア
            quality_scores = [task.get('quality_score', 0.5) for task in completed_tasks if task.get('quality_score')]
            if quality_scores:
        ...
        # 複数の要因を組み合わせて成功確率を推定
        success_factors = [
            features['match_recent_success_rate'] * self.success_weights['completion_rate'],
            features['match_historical_quality_score'] * self.success_weights['quality_score'],
            features['match_timeliness_rate'] * self.success_weights['timeliness'],
            features['match_collaboration_satisfaction'] * self.success_weights['satisfaction']
        ]
        
        estimated_success_probability = sum(success_factors)
        features['match_estimated_success_probability'] = float(estimated_success_probability)
    ...
    def _extract_required_skills(self, task_data: Dict[str, Any]) -> Set[str]:
        ...
        # タイトル・本文からスキルを抽出
        text_content = (task_data.get('title', '') + ' ' + task_data.get('body', '')).lower()
    ...
    def _calculate_skill_similarity(self, required_skills: Set[str], developer_skills: Set[str]) -> float:
        """スキルの類似度を計算"""
    ...
    def _calculate_weighted_skill_compatibility(self, required_skills: Set[str], 
                                              developer_skills: Set[str]) -> float:
        """重み付きスキル適合度を計算"""
    ...
    def _calculate_directory_overlap(self, task_files: Set[str], developer_files: Set[str]) -> float:
        """ディレクトリレベルの重複を計算"""
        task_dirs = set()
        dev_dirs = set()
    ...
    def _calculate_file_type_overlap(self, task_files: Set[str], developer_files: Set[str]) -> float:
        """ファイルタイプの重複を計算"""
        task_extensions = set()
        dev_extensions = set()
    ...
    def _extract_developer_tech_stack(self, developer_data: Dict[str, Any]) -> Dict[str, Set[str]]:
        """開発者から技術スタックを抽出"""
        tech_stack = {category: set() for category in self.tech_stack_hierarchy.keys()}
    ...
    def _extract_developer_architecture_experience(self, developer_data: Dict[str, Any]) -> Dict[str, float]:
        """開発者のアーキテクチャ経験を抽出"""
        ...
        for file_path in touched_files:
            ...
            if any(term in file_path_lower for term in ['api', 'controller', 'endpoint']):
                architecture_experience['api'] += 0.1
            if any(term in file_path_lower for term in ['frontend', 'ui', 'component', 'view']):
                architecture_experience['frontend'] += 0.1
            if any(term in file_path_lower for term in ['backend', 'server', 'service']):
                architecture_experience['backend'] += 0.1
        
        # スキルから推定
        skills_lower = [skill.lower() for skill in skills]
        
        if any(skill in skills_lower for skill in ['microservices', 'docker', 'kubernetes']):
            architecture_experience['microservices'] += 0.5
        if any(skill in skills_lower for skill in ['aws lambda', 'serverless', 'azure functions']):
            architecture_experience['serverless'] += 0.5
        if any(skill in skills_lower for skill in ['rest api', 'graphql', 'api design']):
            architecture_experience['api'] += 0.5
        
        return architecture_experience
    
    def _calculate_developer_experience_level(self, developer_data: Dict[str, Any]) -> float:
        """開発者の経験レベルを計算（0-1）"""
        factors = []
        ...
        commit_score = min(np.log1p(total_commits) / np.log1p(10000), 1.0)  # 10000コミットで最大
    ...
    def get_feature_names(self) -> List[str]:
        ...
        features = self.design_enhanced_features(sample_task, sample_developer, sample_env)
    ...
    def validate_features(self, features: Dict[str, float]) -> Dict[str, Any]:
        ...
        for name, value in features.items():
            ...
            # 確率・比率特徴量の範囲チェック
            if any(keyword in name for keyword in ['ratio', 'rate', 'probability', 'compatibility', 'similarity']):
