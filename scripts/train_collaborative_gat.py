...
def train_collaborative_gat():
    ...
    print(f"Collaboration network loaded: {dev_network['num_developers']} developers, {dev_network['network_stats']['num_edges']} edges")
    
    # エッジ辞書を準備（協力ネットワークを含む）
    edge_index_dict = {
        ("dev", "writes", "task"): data[("dev", "writes", "task")].edge_index,
        ("task", "written_by", "dev"): data[("dev", "writes", "task")].edge_index.flip([0]),
    ...
    enhanced_data[("dev", "collaborates", "dev")].edge_index = dev_network["dev_collaboration_edge_index"]
    enhanced_data[("dev", "collaborates", "dev")].edge_attr = dev_network["dev_collaboration_edge_weights"]
