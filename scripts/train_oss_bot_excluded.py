...
def filter_bot_tasks(
    backlog: List[Dict], expert_trajectories: List, train_ratio: float = 0.7
) -> tuple:
    """
    ...
    print(
        f"  Train tasks: {len(train_backlog)} (up to {train_tasks[-1]['created_at'] if train_tasks else 'N/A'})"
    )
    print(
        f"  Test tasks: {len(test_backlog)} (from {test_tasks[0]['created_at'] if test_tasks else 'N/A'})"
    )
...
def main(cfg: DictConfig):
    ...
    expert_path = cfg.env.get(
        "expert_trajectories_path",
        cfg.irl.get("expert_path", "data/expert_trajectories.pkl"),
    )
    ...
    human_tasks, human_dev_profiles, filtered_trajectories = (
        filter_bot_tasks_and_developers(backlog, dev_profiles, expert_trajectories)
    )
    ...
    print(
        f"Environment initialized with {len(train_tasks)} tasks and {len(human_dev_profiles)} developers"
    )
