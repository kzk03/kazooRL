...
class SimpleTaskAssignmentEnv(gym.Env):
    ...
    def __init__(self, cfg, backlog_data, dev_profiles_data):
        ...
        self.num_developers = min(
            len(dev_profiles_data), cfg.optimization.get("max_developers", 50)
        )
        self.max_tasks = min(len(backlog_data), cfg.optimization.get("max_tasks", 200))
        ...
        for i, task_dict in enumerate(backlog_data[: self.max_tasks]):
            try:
                ...
                if task_obj.updated_at is None:
                    print(
                        f"⚠️ Task {i} has None updated_at: {task_dict.get('updated_at')}"
                    )
                self.tasks.append(task_obj)
            except Exception as e:
                print(f"⚠️ Failed to create Task object for task {i}: {e}")
                print(
                    f"   Task data keys: {list(task_dict.keys()) if isinstance(task_dict, dict) else 'Not a dict'}"
                )
                conversion_errors += 1
    ...
    def step(self, action):
        """ステップ実行"""
        if self.current_task_idx >= len(self.tasks) or action >= self.num_developers:
            # 終了条件
        ...
        if not hasattr(current_task, "updated_at"):
            print(
                f"⚠️ Task {self.current_task_idx} is not a Task object in step: {type(current_task)}"
            )
        ...
        task_id = (
            ...
            else current_task.get("id", self.current_task_idx)
        )
    ...
    def _get_observation(self):
        ...
        if not hasattr(current_task, "updated_at"):
            print(
                f"⚠️ Task {self.current_task_idx} is not a Task object: {type(current_task)}"
            )
            # 辞書の場合は Task オブジェクトに変換
            if isinstance(current_task, dict):
                current_task = self.Task(current_task)
        ...
        try:
            ...
            dummy_env = type(
                ...
                {
                    "backlog": [
                        t if hasattr(t, "updated_at") else self.Task(t)
                        for t in self.backlog_data[: self.max_tasks]
                    ],
    ...
    def _calculate_reward(self, task, developer):
        ...
        try:
            ...
            if not hasattr(task, "updated_at"):
                print(
                    f"⚠️ Task is not a Task object in reward calculation: {type(task)}"
                )
                # 辞書の場合は Task オブジェクトに変換
...
def main(cfg: DictConfig):
    ...
    def linear_schedule(initial_value: float):
        ...
        def func(progress_remaining: float) -> float:
            ...
            return (
                ...
                + (1 - progress_remaining) * 0.1 * initial_value
            )
    ...
    model = PPO(
        ...
        verbose=1,
        learning_rate=(
            linear_schedule(learning_rate) if use_lr_schedule else learning_rate
        ),
...
def evaluate_simple_model(model, env, cfg):
    ...
    for episode in range(num_episodes):
        ...
        while True:
            ...
            if env.current_task_idx < len(env.tasks):
                ...
                task_id = (
                    task.id
                    if hasattr(task, "id")
                    else task.get("id", env.current_task_idx)
                )
                predicted_dev = (
                    env.developers[action]
        ...
        print(
            f"   Episode {episode + 1}: 報酬={episode_reward:.4f}, 割り当て数={assignments}"
        )
...
def calculate_recommendation_metrics(
    predictions, confidences, dev_distribution, available_developers
):
    """推薦システムとしての評価指標を計算"""
    metrics = {}
    ...
    metrics["recommendation_coverage"] = (
        unique_developers / len(available_developers) if available_developers else 0
    )
    ...
    if confidences:
        ...
        high_confidence_count = sum(
            1 for c in confidences if c >= high_confidence_threshold
        )
        medium_confidence_count = sum(
            1 for c in confidences if c >= medium_confidence_threshold
        )
