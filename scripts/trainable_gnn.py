...
def create_better_training_data(data):
    ...
    print(
        f"負のサンプル数: {neg_pairs_tensor.size(0)} (ランダム:{len(random_neg_pairs)}, 部分:{len(partial_neg_pairs)}, 人気:{len(popular_neg_pairs)})"
    )
    # ▲▲▲【改善点ここまで】▲▲▲
...
def main():
    ...
    model = GNNModel(
        in_channels_dict={"dev": actual_dev_dim, "task": actual_task_dim},
        out_channels=32,
    )
    ...
    for epoch in range(1, 21):
        ...
        try:
            ...
            # ペアの妥当性をチェック
            valid_pairs_mask = (pairs[:, 0] < embeddings["dev"].size(0)) & (
            ...
            if epoch % 5 == 0:
                print(
                    f"[Epoch {epoch:03d}] loss: {loss.item():.4f}, total_loss: {total_loss.item():.4f}, valid_pairs: {valid_pairs.size(0)}"
                )
