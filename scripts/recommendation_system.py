...
def create_recommendation_system():
    ...
    def recommend_tasks_for_developer(dev_idx, top_k=10):
        ...
        recommendations = []
        for rank, (sim, task_idx) in enumerate(
            zip(top_indices.values, top_indices.indices), 1
        ):
            recommendations.append(
                {"rank": rank, "task_idx": task_idx.item(), "similarity": sim.item()}
            )
    ...
    def recommend_developers_for_task(task_idx, top_k=10):
        ...
        recommendations = []
        for rank, (sim, dev_idx) in enumerate(
            zip(top_indices.values, top_indices.indices), 1
        ):
            recommendations.append(
                {"rank": rank, "dev_idx": dev_idx.item(), "similarity": sim.item()}
            )
    ...
    for rec in dev_recommendations:
        print(
            f"{rec['rank']}. Task {rec['task_idx']}: similarity = {rec['similarity']:.4f}"
        )
    ...
    for rec in task_recommendations:
        print(
            f"{rec['rank']}. Developer {rec['dev_idx']}: similarity = {rec['similarity']:.4f}"
        )
