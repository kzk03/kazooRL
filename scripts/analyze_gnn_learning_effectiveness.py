...
def analyze_gnn_learning_effectiveness():
    ...
    print(f"📊 モデルパラメータ数: {sum(p.numel() for p in gnn_extractor.model.parameters())}")
    ...
    print(f"📊 初期開発者埋め込みノルム: {torch.norm(initial_state['dev_emb']).item():.6f}")
    print(f"📊 初期タスク埋め込みノルム: {torch.norm(initial_state['task_emb']).item():.6f}")
    ...
    for i in range(20):  # 20回の強いインタラクション
        ...
        gnn_extractor.record_interaction(task, developer, reward, "strong_positive", simulation_time=sim_time)
        
        # 5回ごとに状態をチェック
        if (i + 1) % 5 == 0:
            current_state = get_specific_embeddings()
            similarity_change = current_state['similarity'] - initial_state['similarity']
            print(f"  インタラクション {i+1}: 類似度 = {current_state['similarity']:.6f} (Δ{similarity_change:+.6f})")
    ...
    print(f"\n📊 埋め込み統計:")
    print(f"  開発者埋め込み - 平均: {dev_embeddings.mean().item():.6f}, 標準偏差: {dev_embeddings.std().item():.6f}")
    print(f"  タスク埋め込み - 平均: {task_embeddings.mean().item():.6f}, 標準偏差: {task_embeddings.std().item():.6f}")
    
    # 類似度分布
    all_similarities = []
    for i in range(min(10, dev_embeddings.shape[0])):
        for j in range(min(10, task_embeddings.shape[0])):
            sim = F.cosine_similarity(dev_embeddings[i].unsqueeze(0), task_embeddings[j].unsqueeze(0)).item()
            all_similarities.append(sim)
    
    import numpy as np
    all_similarities = np.array(all_similarities)
    print(f"  類似度分布 - 平均: {all_similarities.mean():.6f}, 標準偏差: {all_similarities.std():.6f}")
