...
def filter_bot_tasks_and_developers(backlog, dev_profiles, expert_trajectories=None):
        ...
        for episode in expert_trajectories:
            ...
            for step in episode:
                if isinstance(step, dict):
                    ...
                    if (
                        developer
                        and not is_bot_user(developer)
                        and developer in human_dev_profiles
                    ):
    ...
    print(
        f"Human expert trajectory episodes remaining: {len(human_expert_trajectories)}"
    )
...
def main(cfg: DictConfig):
    ...
    print(
        f"  Bot-excluded expert trajectories: data/expert_trajectories_bot_excluded.pkl"
    )
    ...
    if os.path.exists(expert_trajectories_path):
        try:
            ...
            print(
                f"Loaded {len(expert_trajectories)} bot-excluded expert trajectory episodes"
            )
            ...
            if expert_trajectories and expert_trajectories[0]:
                ...
                if "action_details" in sample_step:
                    developer = sample_step["action_details"].get(
                        "developer", "Unknown"
                    )
        ...
        print(
            f"Warning: Bot-excluded expert trajectories file not found: {expert_trajectories_path}"
        )
        print(
            "You can create it by running: python tools/analysis/create_expert_trajectories_bot_excluded.py"
        )

    # Bot除外処理
    human_backlog, human_dev_profiles, human_expert_trajectories, expert_assignments = (
    ...
    print(
        f"Environment initialized with {len(train_tasks)} tasks and {len(human_dev_profiles)} human developers"
    )
    ...
    print(f"Total timesteps: {cfg.rl.total_timesteps}")
    print(
        f"Using bot-excluded expert trajectories: {len(human_expert_trajectories)} episodes"
    )
    ...
    print(
        f"  Expert data: {len(human_expert_trajectories)} episodes with bot exclusion"
    )
