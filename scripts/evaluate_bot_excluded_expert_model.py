...
class BotExcludedExpertModelEvaluator:
    ...
    def __init__(self, cfg: DictConfig, train_ratio: float = 0.7):
        ...
        print(
            f"Loaded {len(self.full_backlog)} tasks and {len(self.full_dev_profiles)} developer profiles"
        )

        # Bot除外済みexpert軌跡の読み込み
        expert_trajectories_path = "data/expert_trajectories_bot_excluded.pkl"
        self.expert_trajectories, self.expert_assignments = (
            load_bot_excluded_expert_data(expert_trajectories_path)
        )
        ...
        self.train_tasks, self.test_tasks = split_tasks_by_time(
            self.human_backlog, train_ratio
        )
        ...
        self.train_expert_assignments = {
            task_id: dev
            for task_id, dev in self.expert_assignments.items()
    ...
    def evaluate(self):
        ...
        model_type = (
            "Bot-Excluded Expert Model" if self.models_loaded else "Random Baseline"
        )
        ...
        print(
            f"  Random Top-1       : {baseline_results.get('random_top_1', 0):.3f} ({baseline_results.get('random_top_1', 0)*100:.1f}%)"
        )
        print(
            f"  Frequent Dev Top-1 : {baseline_results.get('frequent_top_1', 0):.3f} ({baseline_results.get('frequent_top_1', 0)*100:.1f}%)"
        )
