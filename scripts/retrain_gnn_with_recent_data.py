    ...
    for event in events:
        ...
        if event_type in ['PullRequestEvent', 'IssuesEvent', 'PullRequestReviewEvent']:
            ...
            if event_type == 'PullRequestEvent':
                ...
                task_labels = [label.get('name', '') for label in issue.get('labels', [])]
...
def create_modern_graph_data(interactions, task_info, dev_info):
    ...
    min_interactions = 2
    active_devs = {name: info for name, info in dev_info.items() 
                   if info['interactions'] >= min_interactions}
    
    min_dev_interactions = 1  
    active_tasks = {tid: info for tid, info in task_info.items()
    ...
    for dev_name in active_devs.keys():
        ...
        profile = existing_profiles.get(dev_name, {
            'skills': ['general'],
            'touched_files': [],
            'label_affinity': {}
        })
        ...
        feature_vector = [
            ...
            profile.get('label_affinity', {}).get('documentation', 0.0)  # ドキュメント親和性
        ]
    ...
    for task_id in active_tasks.keys():
        ...
        feature_vector = [
            ...
            sum(1 for i in interactions if i['task_id'] == task_id),  # インタラクション数
            len(set(i['dev_name'] for i in interactions if i['task_id'] == task_id)),  # 関与開発者数
