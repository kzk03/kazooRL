...
class BotExcludedRecommendationEvaluator:
    ...
    def __init__(self, cfg: DictConfig, train_ratio: float = 0.7):
        ...
        self.env = OSSSimpleEnv(cfg, self.train_tasks, self.human_dev_profiles)  # Use train_tasks for same observation space
    ...
    def split_data(self):
        ...
        train_tasks, test_tasks = split_tasks_by_time(human_tasks, train_ratio=self.train_ratio)
        ...
        for k in [1, 3, 5]:
            ...
            for task_id, expert_dev in self.test_expert_assignments.items():
                if expert_dev in most_frequent_devs[:k]:
                    correct += 1
            accuracy = correct / len(self.test_expert_assignments) if self.test_expert_assignments else 0.0
