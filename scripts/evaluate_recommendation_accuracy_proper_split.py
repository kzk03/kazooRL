...
class ProperSplitRecommendationEvaluator:
    ...
    def split_data(self):
        ...
        print(f"  Train tasks: {len(self.train_tasks)} (up to {self.train_tasks[-1]['created_at'] if self.train_tasks else 'N/A'})")
        print(f"  Test tasks: {len(self.test_tasks)} (from {self.test_tasks[0]['created_at'] if self.test_tasks else 'N/A'})")
    ...
    def get_model_recommendations(self, max_k: int = 5) -> List[Tuple[str, List[str]]]:
        ...
        for agent_id in self.controller.agent_ids:
            try:
                ...
                obs_tensor = torch.FloatTensor(test_obs).to(self.controller.agents[agent_id].device)
                with torch.no_grad():
        ...
        if not models_loaded:
            print("⚠️  No models loaded successfully. Using random baseline for comparison.")
            # ランダム推薦を生成（botを除外）
            import random
            developer_list = [dev for dev in self.dev_profiles.keys() 
    ...
    def calculate_accuracy(self, recommendations: List[Tuple[str, List[str]]], k_values: List[int] = [1, 3, 5]) -> Dict[str, float]:
        ...
        human_test_assignments = {task_id: dev for task_id, dev in self.test_expert_assignments.items() 
                                if not ('bot' in dev.lower() or dev == 'stale[bot]')}
        
        print(f"Human test expert assignments available for {len(human_test_assignments)} tasks")
        print(f"Bot test assignments excluded: {len(self.test_expert_assignments) - len(human_test_assignments)}")
        ...
        accuracies['bot_test_set_size'] = len(self.test_expert_assignments) - len(human_test_assignments)
    ...
    def analyze_baseline_performance(self):
        ...
        # 開発者の数を取得（botを除外）
        human_developers = [dev for dev in self.dev_profiles.keys() 
                           if not ('bot' in dev.lower() or dev == 'stale[bot]')]
        ...
        human_assignments = {task_id: dev for task_id, dev in self.train_expert_assignments.items() 
                           if not ('bot' in dev.lower() or dev == 'stale[bot]')}
        ...
        for i, (dev, count) in enumerate(dev_counts.most_common(5)):
            print(f"  {i+1}. {dev}: {count} assignments ({count/total_human_assignments*100:.1f}%)")
        ...
        for k in [1, 3, 5]:
            ...
            for task_id, expert_dev in human_test_assignments.items():
                if expert_dev in most_frequent_devs[:k]:
                    correct += 1
            accuracy = correct / len(human_test_assignments) if human_test_assignments else 0.0
        ...
        print(f"  Bot training tasks excluded: {len(self.train_expert_assignments) - total_human_assignments}")
        print(f"  Bot test tasks excluded: {len(self.test_expert_assignments) - len(human_test_assignments)}")
        
        return frequent_dev_accuracies
    
    def evaluate(self):
        """推薦システムの総合評価を実行"""
        print("🎯 Starting proper train/test split recommendation accuracy evaluation...")
        ...
        for i, (task_id, devs) in enumerate(recommendations):
            if task_id in human_test_assignments and sample_count < 5:
                ...
                print(f"  {sample_count+1}. Task {task_id} → {top_3_devs} (Expert: {expert_dev})")
                sample_count += 1
        ...
        if accuracies:
            ...
            print(f"  Random Top-1       : {1.0/len(human_developers):.3f} ({100.0/len(human_developers):.1f}%)")
            print(f"  Frequent Dev Top-1 : {baseline_results.get('frequent_dev_top_1', 0):.3f} ({baseline_results.get('frequent_dev_top_1', 0)*100:.1f}%)")
        else:
            print("❌ Could not calculate accuracy (no test data or valid recommendations)")
        
        # 詳細統計
        print(f"\nDetailed Statistics:")
        print(f"  Human test set size: {accuracies.get('human_test_set_size', 0)}")
        print(f"  Bot test set size (excluded): {accuracies.get('bot_test_set_size', 0)}")
        print(f"  Total test recommendations: {accuracies.get('total_recommendations', len(recommendations))}")
        print(f"  Valid human recommendations: {accuracies.get('total_valid_recommendations', 0)}")
        print(f"  Human train set size: {len([task for task, dev in self.train_expert_assignments.items() if not ('bot' in dev.lower() or dev == 'stale[bot]')])}")
