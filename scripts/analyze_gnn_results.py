...
def analyze_gnn_results():
    ...
    unique_values, counts = torch.unique(
        all_similarities.round(decimals=4), return_counts=True
    )
    ...
    task_similarity_matrix = F.cosine_similarity(
        sample_task_emb.unsqueeze(1), sample_task_emb.unsqueeze(0), dim=2
    )
    ...
    for i in range(min(5, len(high_sim_indices[0]))):
        ...
        print(
            f"  Task {task1_idx} (ID: {task1_id}) - Task {task2_idx} (ID: {task2_id}): {sim:.4f}"
        )
