...
class FullTrainingPipeline:
    ...
    def __init__(self, config_path="configs/base.yaml", quiet=False):
        ...
        # ログファイル設定
        self.log_file = log_dir / f"kazoo_training_{self.start_time.strftime('%Y%m%d_%H%M%S')}.log"
        ...
        try:
            ...
            if not Path("data/developer_collaboration_network.pt").exists():
                ...
                result = subprocess.run([
                    sys.executable, "tools/data_processing/build_developer_network.py"
            ...
            for model_path in required_models:
                if not Path(model_path).exists():
                    self.log(f"❌ 生成されるべきモデルファイルが見つかりません: {model_path}")
                    return False
    ...
    def train_irl(self):
        ...
        try:
            ...
            # IRLの実行
            self.log(f"📊 IRL設定: エポック数={self.cfg.irl.epochs}, 学習率={self.cfg.irl.learning_rate}")
            ...
            if not Path(self.cfg.irl.output_weights_path).exists():
                self.log(f"❌ 学習済み重みファイルが生成されませんでした: {self.cfg.irl.output_weights_path}")
                return False
            
            # 重みの分析
            weights = np.load(self.cfg.irl.output_weights_path)
            self.log(f"📈 学習済み重み統計: 平均={weights.mean():.4f}, 標準偏差={weights.std():.4f}")
    ...
    def _run_irl_training(self):
        ...
        self.log(f"🔄 訓練ループ開始: {len(expert_trajectory_steps)} エキスパートステップ")
        ...
        for epoch in range(self.cfg.irl.epochs):
            ...
            if valid_steps > 0:
                ...
                if (epoch + 1) % 100 == 0 or epoch == 0:
                    self.log(f"📈 エポック {epoch + 1}/{self.cfg.irl.epochs}, 平均損失: {avg_loss:.6f}, 有効ステップ: {valid_steps}")
    ...
    def train_rl(self):
        ...
        try:
            ...
            self.log(f"🎮 RL設定: タイムステップ={self.cfg.rl.total_timesteps}, 学習率={self.cfg.rl.learning_rate}")
    ...
    def generate_summary_report(self):
        ...
        # レポート保存
        report_path = self.log_file.parent / f"kazoo_report_{self.start_time.strftime('%Y%m%d_%H%M%S')}.json"
...
実行時間の目安:
  ...
  合計: 10〜17時間（フル実行）
    ...
    if not args.quiet:
        ...
        skip_list = []
        if args.skip_gnn: skip_list.append("GNN")
        if args.skip_irl: skip_list.append("IRL") 
        if args.skip_rl: skip_list.append("RL")
        if skip_list:
