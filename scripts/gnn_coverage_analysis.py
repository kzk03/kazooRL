...
def analyze_gnn_coverage():
    ...
    print(f"Developers found in GNN: {len(dev_overlap)} ({len(dev_overlap)/len(human_devs)*100:.1f}%)")
    print(f"Missing from GNN: {len(dev_missing)} ({len(dev_missing)/len(human_devs)*100:.1f}%)")
    if dev_missing:
    ...
    print(f"Tasks found in GNN: {len(task_overlap)} ({len(task_overlap)/len(task_ids)*100:.1f}%)")
    print(f"Missing from GNN: {len(task_missing)} ({len(task_missing)/len(task_ids)*100:.1f}%)")
    print()
