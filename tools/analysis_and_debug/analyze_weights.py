...
def analyze_reward_weights(config_path, weights_path):
    ...
    try:
        ...
        print("Please check if your config file has the correct structure for FeatureExtractor.")
        return
    ...
    if len(weights) != len(feature_names):
        print(f"Error: Mismatch between weights ({len(weights)}) and feature names ({len(feature_names)})")
    ...
    df_sorted = df.sort_values(by='Abs_Weight', ascending=False).drop(columns=['Abs_Weight'])
    ...
    print("\n---------------------------------------------")
    print("\n[Interpretation]")
    print(" - Positive weights (大きい正の数): Encourage actions. Developers PREFER tasks with these features.")
    print(" - Negative weights (大きい負の数): Discourage actions. Developers AVOID tasks with these features.")
