...
FAILED_LOG_PATH = os.path.join(
    OUTPUT_DIR, f"{OUTPUT_FILE_BASENAME}_failed_downloads.log"
)
        ...
        while current_date <= END_DATE:
            ...
            if year_month != current_file_month:
                ...
                output_filename = os.path.join(
                    OUTPUT_DIR, f"{output_basename}_{current_file_month}.jsonl"
                )
                print(
            ...
            for hour in range(24):
                ...
                print(
                    f"🔗 {current_date.strftime('%Y-%m-%d')} {hour:02d}時 のデータ取得開始 → {url}"
                )
                ...
                while attempt < MAX_RETRIES:
                    try:
                        ...
                        if e.response.status_code == 404:
                            ...
                            log_failed_download(
                                current_date, hour, url, "Not Found (404)"
                            )
                            download_successful = (
                                True  # 見つからなかったが、エラーとしてリトライはしない
                            )
                            break  # スキップして次の時間へ
                            ...
                            attempt += 1
                            print(
                                f"⚠️ API制限または一時的なサーバーエラー (ステータスコード: {e.response.status_code})。試行 {attempt}/{MAX_RETRIES}: {e}"
                            )
                            ...
                            if retry_after:
                                try:
                                    wait_time = (
                                        int(retry_after) + 5
                                    )  # Retry-Afterに少し余裕を持たせる
                                    current_wait_time = min(
                                        wait_time, MAX_RETRY_WAIT_SECONDS
                                    )
                                    print(
                                        f"「Retry-After」ヘッダーに基づいて {current_wait_time}秒待機します..."
                                    )
                                except ValueError:
                                    print(
                                        f"「Retry-After」ヘッダーの値が無効です。デフォルトで {current_wait_time}秒待機します..."
                                    )
                                ...
                                time.sleep(current_wait_time)
                                current_wait_time = min(
                                    current_wait_time * 2, MAX_RETRY_WAIT_SECONDS
                                )  # Exponential backoff
                        else:
                            # その他のHTTPエラー（5xx系など）
                            attempt += 1
                            print(
                                f"⚠️ その他のHTTPエラー (ステータスコード: {e.response.status_code})。試行 {attempt}/{MAX_RETRIES}: {e}"
                            )
                            print(f"{current_wait_time}秒待機して再試行します...")
                            time.sleep(current_wait_time)
                            current_wait_time = min(
                        ...
                        attempt += 1
                        print(
                            f"⚠️ タイムアウトエラー (試行 {attempt}/{MAX_RETRIES}): {url}"
                        )
                        print(f"{current_wait_time}秒待機して再試行します...")
                        time.sleep(current_wait_time)
                        current_wait_time = min(
                        ...
                        attempt += 1
                        print(
                            f"⚠️ その他のリクエストエラー (試行 {attempt}/{MAX_RETRIES}): {e}"
                        )
                        print(f"{current_wait_time}秒待機して再試行します...")
                        time.sleep(current_wait_time)
                        current_wait_time = min(
                ...
                if not download_successful:
                    print(
                        f"❌ 最大リトライ回数に達しました。ダウンロードをスキップします: {url}"
                    )
                    log_failed_download(
                        current_date,
                        hour,
