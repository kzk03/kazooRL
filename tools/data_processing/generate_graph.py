...
def generate_graph():
    ...
    # JSONLファイル検索
    all_jsonl_files = [p for p in data_dir.glob("**/*.jsonl")] if data_dir.exists() else []
    ...
    task_features = []
    task_meta = []
    all_labels_list = ["bug", "enhancement", "documentation", "question", "performance", "CI/CD"]
    ...
    if not dev_set:
        print("⚠️  JSONLファイルから開発者が見つからないため、dev_profilesから開発者を追加します")
        # dev_profilesから開発者を追加
        for dev_name in list(dev_profiles.keys())[:10]:  # 最初の10人
            dev_set.add(dev_name)
            
        # バックログタスクにダミーの作成者を設定
        for i, task in enumerate(backlog_tasks[:min(50, len(list(dev_profiles.keys())) * 3)]):
    ...
    def validate_edges(edge_list, max_dev_idx, max_task_idx, edge_name):
        ...
        for dev_idx, task_idx in edge_list:
            if 0 <= dev_idx < max_dev_idx and 0 <= task_idx < max_task_idx:
                ...
                print(f"Warning: Invalid edge in {edge_name}: dev={dev_idx} (max={max_dev_idx-1}), task={task_idx} (max={max_task_idx-1})")
        return valid_edges

    writes_edges = validate_edges(writes_edges, len(dev_names), len(task_ids), "writes")
    reviews_edges = validate_edges(reviews_edges, len(dev_names), len(task_ids), "reviews")
    assigned_edges = validate_edges(assigned_edges, len(dev_names), len(task_ids), "assigned")
    ...
    if writes_edges:
        ...
        data[("task", "written_by", "dev")].edge_index = data[("dev", "writes", "task")].edge_index.flip([0])
    else:
        data[("task", "written_by", "dev")].edge_index = torch.empty((2, 0), dtype=torch.long)
    ...
    if writes_edges:
        ...
        print(f"  - dev->task edges: dev range [0, {dev_writes_edge[0].max().item()}], task range [0, {dev_writes_edge[1].max().item()}]")
        print(f"  - task->dev edges: task range [0, {task_written_edge[0].max().item()}], dev range [0, {task_written_edge[1].max().item()}]")
        print(f"  - Expected: dev nodes [0, {len(dev_names)-1}], task nodes [0, {len(task_ids)-1}]")
    # ▲▲▲【デバッグ情報ここまで】▲▲▲
    # ▲▲▲【修正箇所ここまで】▲▲▲
