...
def main():
    ...
    # 4. 逆強化学習（IRL）
    if not run_command(
        "python scripts/train_irl.py", "逆強化学習（Inverse Reinforcement Learning）"
    ):
        return False

    # 5. 強化学習（RL）
    if not run_command(
        "python scripts/train_oss.py", "強化学習（Reinforcement Learning）"
    ):
        return False

    # 6. 評価
    if not run_command("python scripts/eval_oss.py", "学習済みモデルの評価"):
